...
void va_log(int pri, char *format, ...)
{
    va_list args;
...
//marco
static int external_port_is_used(struct UPNPUrls * urls,
                             struct IGDdatas * data,char *ext_port,char *proto)
{
	int r;
	int i = 0;
	char index[6];
	...
	do 
	{
		...
		/*	printf("%2d %s %5s->%s:%-5s '%s' '%s' %s\n",
			       i, protocol, extPort, intClient, intPort,
			       desc, rHost, duration);*/
			if( strcmp(proto,protocol)==0 && strcmp(ext_port,extPort)==0 )
			{
				return 1;
			}
		}
                               ...
                               const char * leaseDuration)
{
	...
	if(r!=UPNPCOMMAND_SUCCESS)
	{
		fprintf(stderr, "%s:Can't get external ip address\n", __FUNCTION__);
        return;
	}
	...
	if(intClient[0]) {
		...
		va_log(ALOG_DEBUG | LOG_NOTICE,"external %s:%s %s is redirected to internal %s:%s (duration=%s)\n",
		       externalIPAddress, eport, proto, intClient, intPort, duration);
	}
}
...
#ifdef BUSYBOX_WITH_UPNPC_MAIN//marco
struct igd_info
{
	char protocol[8];//UDP/TCP
	char external_port[8];
	char internal_port[8];
	char desc[128];
	char lease[8];
	char command;
};
int readcfg(char *file,struct igd_info *list,int total)
{
	...
	for(i=0;i<total;i++)
	{
		...
		sscanf(buf,"Protocol:%s\n",list[i].protocol);
		
		fgets(buf,sizeof(buf),fptr);
		sscanf(buf,"Ext_Port:%s\n",list[i].external_port);
		
		fgets(buf,sizeof(buf),fptr);
		sscanf(buf,"Int_Port:%s\n",list[i].internal_port);
...
void show_usage()
{
	printf("upnpc [-m ip] [-t timeout] [-l no.of entry] [-f config file path]\n");
	printf("-m :intf ip to send SSDP\n");
	printf("-t:Time to update IGD port-mapping in sec\n");
	printf("-l:Total no. of entry in config file\n");
	printf("-f:Path of the config File\n");
}
int upnpc_main(int argc, char ** argv);
int main(int argc, char ** argv)
{
	char *file_path=NULL,*multicastif=NULL;
	char  *minissdpdpath=NULL;
	char lanaddr[64];	/* my ip address on the LAN */
	const char * rootdescurl = 0;
	
	int retry_cnt=0, ipv6 = 0;
	int total_entry=0 ,timeout=0;//in sec
	int error = 0, index=0,i=0;
	
	struct igd_info *list=NULL;
	struct UPNPDev * devlist = 0;
	/* command line processing */
	
	if(strcmp(argv[0],"upnpc")==0)
	{
		return  upnpc_main(argc,argv);
	}
	if(argc<8)
	{
		show_usage();	
	...
	list=malloc( total_entry*sizeof(struct igd_info) );
	if(list==NULL)
	{
		printf("MALLOC() failed\n");	
		return 0;
	}
	if( readcfg(file_path,&list[0],total_entry) )
	{
		while(1)
		{
			if( rootdescurl
		  			...
		  			|| (i = UPNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr))))
				{
					switch(i) 
					{
						...
						default:
							...
							printf("Try to send discovery again\n");
					...
					{
						switch(list[index].command)
						{
							case 'a':
//marco,before add,query the port-mapping list first
//if we found th external port is used, delete the entry 
//then add ours one			
								//+++ mark by siyou, because external_port_is_used() will query each upnp entry, it will take a long long time
								//when there are so many ports open by upnp client.
								//if(external_port_is_used(&urls, &data,list[index].external_port, list[index].protocol))
								{
									RemoveRedirect(&urls, &data, list[index].external_port, list[index].protocol);
								}\
								SetRedirectAndTest(&urls, &data,
				                  multicastif, list[index].internal_port,
				                 list[index].external_port, list[index].protocol,
				                 list[index].lease);
...
//if we found th external port is used, delete the entry 
//then add ours one			
				if(external_port_is_used(&urls, &data,commandargv[2], commandargv[3]))
				{
					RemoveRedirect(&urls, &data, commandargv[2], commandargv[3]);
				}
				SetRedirectAndTest(&urls, &data,
...
FAILED:	
	if(retcode && retry_cnt<MAX_RETRY_CNT)
	{
		printf("retry_cnt %d\n",retry_cnt);
		retry_cnt++;	
		retcode=0;
