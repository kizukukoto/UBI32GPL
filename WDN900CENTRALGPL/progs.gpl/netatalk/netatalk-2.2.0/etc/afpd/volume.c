...
#ifdef ALPHA_CUSTOMIZE /* change priviledge behavior to RW > RO */
                /* handle read-only behaviour. semantics:
                 * 1) neither the rolist nor the rwlist exist -> rw
                 * 2) rwlist exists -> rw if user is in it.
                 * 3) rolist exists -> rw unless user is not in rwlist. */
                if (parent_or_child == 1
                    &&
                    ((options[VOLOPT_FLAGS].i_value & AFPVOL_RO) == 0)
                    &&
                    ((accessvol(options[VOLOPT_ROLIST].c_value, obj->username) == 1)))
						if(accessvol(options[VOLOPT_RWLIST].c_value, obj->username) != 1)
							options[VOLOPT_FLAGS].i_value |= AFPVOL_RO;
