...
printf("justin:PAM_ABORT:called pam_start: [%s] [%s] [%p] [%p]"
       ,service_name, user, pam_conversation, pamh);	       
	...
	_pam_drop((*pamh)->user);
	_pam_drop(*pamh);
printf("justin:PAM_ABORT:failed to initialize environment");	
	return PAM_ABORT;
    }
	...
	_pam_drop((*pamh)->user);
	_pam_drop(*pamh);
printf("justin:PAM_ABORT:failed to initialize handlers");		
	return PAM_ABORT;
    }
