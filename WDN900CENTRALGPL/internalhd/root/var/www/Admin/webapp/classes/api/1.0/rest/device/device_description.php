...
require_once('deviceDescription.php');
...
class Device_description{
    ...
    function get($urlPath, $queryParams=null, $ouputFormat='xml'){
        ...
        $device = new DeviceDescripton();
        $result = $device->getDescription();

        if($result !== NULL){
            $xml = new NasXmlWriter();
            $xml->push('device_description');
            $xml->element('machine_name', $result['machine_name']);
            $xml->element('machine_desc', $result['machine_desc']);
            $xml->pop();
            echo $xml->getXml();
			$this->logObj->LogData('OUTPUT', get_class($this),  __FUNCTION__,  'SUCCESS');
		} else {
            //Failed to collect info
			$this->logObj->LogData('OUTPUT', get_class($this),  __FUNCTION__,  'SERVER-ERROR');
    ...
    function put($urlPath, $queryParams=null, $ouputFormat='xml'){
		...
		$device = new DeviceDescripton();
        $result = $device->modifyDescription($changes);
