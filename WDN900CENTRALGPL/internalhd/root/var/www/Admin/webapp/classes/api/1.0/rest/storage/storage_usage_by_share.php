...
require_once('storageUsageByShare.php');
require_once('logMessages.php');

class Storage_usage_by_share{
	var $logObj;

    function Storage_usage_by_share(){
    ...
    function get($urlPath, $queryParams=null, $ouputFormat='xml'){
        ...
        $storageUsageObj = new StorageUsageByShare();
        $result = $storageUsageObj->getStorageUsage();

        if($result !== NULL){
            $xml = new NasXmlWriter();
            $xml->push('storage_usage_by_share');
            $xml->push('shares');
