 ...
 *
 * IMPORTANT!  There is a parallel verison of this file in the kernel in arch/ubicom32/include/asm.  They must agree
 *
...
struct profile_header {
	...
	u32_t perf_counters[PROFILE_COUNTERS];		/* contents of the CPU performance counters */
	u8_t perf_config[PROFILE_COUNTERS];		/* what is being counted */
	u32_t sample_stack_words;	/* number of stack words in the sample */
};
...
struct profile_header_maps {
	u16_t magic;			/* magic number and last packet bit */
	u16_t count;
	u32_t page_shift;
};
...
struct profile_map {
	u16_t start;		/* start page number of segment, relative to start of OCM (Max 256 MB on IP7K or 1 GB on IP8K, plus 256 KB OCM) */
	u16_t type_size;	/* type (5 bits) of the segment and size (12 bits) in pages. A size of 0 means 4K pages */
};
