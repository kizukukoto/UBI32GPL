...
typedef struct {
	...
	unsigned int scnt, ocnt;
	unsigned char offset, tcp_flags;
	unsigned short window, nullnull, length16;
} usbmux_tcp_header;
...
typedef struct {
    ...
    pthread_t thread;
    pthread_t reader;
    int reader_quit; // hack flag to signal quit to the reader

    int fd;
    struct sockaddr_in peeraddr;
    socklen_t addrlen;
    iphone_umux_client_t usb;
