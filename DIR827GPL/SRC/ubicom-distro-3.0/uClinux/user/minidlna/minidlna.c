	...
	else
	{
		for(i=0; i<num_options; i++)
		{
			...
			case UPNPMODEL_NUMBER:
				...
				break;
			case UPNPMODEL_URL:
				strncpy(modelurl, ary_options[i].value, MODELNUMBER_MAX_LEN);
				modelurl[MODELNUMBER_MAX_LEN-1] = '\0'; 
				break;
...
main(int argc, char * * argv)
{
	...
	if( sqlite3_threadsafe() && sqlite3_libversion_number() >= 3005001 &&
	    GETFLAG(INOTIFY_MASK))
	{
		pthread_attr_t	pa;
		pthread_attr_init(&pa);
		pthread_attr_setstacksize(&pa, 48*1024);
		if (pthread_create(&inotify_thread, &pa, start_inotify, NULL) )
	...
	/*
	 * Below code line is commented out because of the CTT 1.5 tool's Discovery
	 * 2.0.0.0 and 2.0.0.1 test case failures. When the dlna device is booted,
	 * CTT test tool assume that device will send out initial notify message but
	 * due to the below line CTT is getting the ByeBye message, so it fails. As
	 * a consequence of this below line is commented out.
	 *
	 */
