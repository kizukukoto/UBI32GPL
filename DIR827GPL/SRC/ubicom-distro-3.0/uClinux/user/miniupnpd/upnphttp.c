/* $Id: upnphttp.c,v 1.3 2009/06/04 02:21:34 jimmy_huang Exp $ */
/* Project :  miniupnp
...
{
	DEBUG_MSG("%s (%d): begin , h->respflags = %02x\n",__FUNCTION__,__LINE__,h->respflags);
...
ProcessHTTPSubscribe_upnphttp(struct upnphttp * h, const char * path)
{
	...
	if(!h->req_Callback && !h->req_SID) {
		DEBUG_MSG("%s (%d): Missing h->req_Callback or h->req_SID, return 412 !!\n",__FUNCTION__,__LINE__);
...
/* Server:, SID:; Timeout: Second-(xx|infinite) */
		if(h->req_Callback) {
			DEBUG_MSG("%s (%d): go upnpevents_addSubscriber() \n",__FUNCTION__,__LINE__);
			...
			DEBUG_MSG("%s (%d): h->respflags = %02x\n",__FUNCTION__,__LINE__,h->respflags);
			DEBUG_MSG("%s (%d): h->req_SID = %s\n",__FUNCTION__,__LINE__,h->req_SID);
			DEBUG_MSG("%s (%d): h->req_SIDLen = %d\n",__FUNCTION__,__LINE__,h->req_SIDLen);
			DEBUG_MSG("%s (%d): req_Timeout = %d\n",__FUNCTION__,__LINE__,h->req_Timeout);
			DEBUG_MSG("%s (%d): go BuildResp_upnphttp(h,0,0) \n",__FUNCTION__,__LINE__);
			BuildResp_upnphttp(h, 0, 0);
		} else {
			DEBUG_MSG("%s (%d): subscription renew\n",__FUNCTION__,__LINE__);
			/* subscription renew */
			/* Invalid SID
...
with HTTP error 412 Precondition Failed. */
			if(renewSubscription(h->req_SID, h->req_SIDLen, h->req_Timeout) < 0) {
				DEBUG_MSG("%s (%d): h->respflags = %02x\n",__FUNCTION__,__LINE__,h->respflags);
				DEBUG_MSG("%s (%d): renewSubscription failed, return 412\n",__FUNCTION__,__LINE__);
				BuildResp2_upnphttp(h, 412, "Precondition Failed", 0, 0);
			} else {
				h->respflags = FLAG_TIMEOUT;
				...
				DEBUG_MSG("%s (%d): h->req_SID = %s\n",__FUNCTION__,__LINE__,h->req_SID);
				DEBUG_MSG("%s (%d): h->req_SIDLen = %d\n",__FUNCTION__,__LINE__,h->req_SIDLen);
				DEBUG_MSG("%s (%d): renewSubscription response with req_Timeout = %d\n",__FUNCTION__,__LINE__,h->req_Timeout);
				//jimmy test for CD_ROUTER 202
				h->respflags |= FLAG_SID;
				DEBUG_MSG("%s (%d): go BuildResp_upnphttp(h,0,0) \n",__FUNCTION__,__LINE__);
				//-------------
				BuildResp_upnphttp(h, 0, 0);
		...
		DEBUG_MSG("%s (%d): go SendResp_upnphttp() \n",__FUNCTION__,__LINE__);
		SendResp_upnphttp(h);
		DEBUG_MSG("%s (%d): go CloseSocket_upnphttp() \n",__FUNCTION__,__LINE__);
		CloseSocket_upnphttp(h);
	}
	DEBUG_MSG("%s (%d): end \n",__FUNCTION__,__LINE__);
}
...
ProcessHTTPUnSubscribe_upnphttp(struct upnphttp * h, const char * path)
{
	...
	if(upnpevents_removeSubscriber(h->req_SID, h->req_SIDLen) < 0) {
		DEBUG_MSG("%s (%d): upnpevents_removeSubscriber failed, return 412\n",__FUNCTION__,__LINE__);
		BuildResp2_upnphttp(h, 412, "Precondition Failed", 0, 0);
	} else {
		DEBUG_MSG("%s (%d): upnpevents_removeSubscriber success\n",__FUNCTION__,__LINE__);
		BuildResp_upnphttp(h, 0, 0);
	}
	SendResp_upnphttp(h);
	CloseSocket_upnphttp(h);
...
ProcessHttpQuery_upnphttp(struct upnphttp * h)
{
	...
	{
		h->req_command = EPost;
		DEBUG_MSG("%s (%d): h->respflags = %02x, call ProcessHTTPPOST_upnphttp()\n",__FUNCTION__,__LINE__,h->respflags);
                     ...
                     int bodylen)
{
	...
	/*	Date:	2009-06-03
	*	Name:	jimmy huang
	*	Reason:	When renew Subscription, h->req_SID would be not only SID field
	*			SID: ...
	*			HOST ...
	*			And the weird thing is that snprintf()
	*			ex: 
	*				a = snprintf(buf,4,"1234567");
	*				buf will be "123"
	*				but a will be 7 !!
	*			So we need to copy explicitily length of "SID: xxxx-xxxxx-xxx...."
	*	Note:	41 means the length of "uuid:11111111-1111-1111-1111-111111111111"
	*			length of "SID: uuid:11111111-1111-1111-1111-111111111111\r\n"
	*			would be 41 + strlen("SID: \r\n")
	*/
#if 1 //def ENABLE_EVENTS
	char tmp_sid[42] = {0};
	if(h->req_SID){
		strncpy(tmp_sid,h->req_SID,41);
		tmp_sid[41] = '\0';
	}
#endif
	//---
	...
	DEBUG_MSG("%s (%d): begin\n",__FUNCTION__,__LINE__);
	DEBUG_MSG("%s (%d): htpresphead = \n%s \n\n",__FUNCTION__,__LINE__,httpresphead);
	DEBUG_MSG("%s (%d): respmsg = \n%s \n\n",__FUNCTION__,__LINE__,respmsg);
	DEBUG_MSG("%s (%d): bodylen = %d \n",__FUNCTION__,__LINE__,bodylen);
	...
	DEBUG_MSG("%s (%d): h->res_buf_alloclen = %d \n",__FUNCTION__,__LINE__,h->res_buf_alloclen);
	/*
		Be careful, text/html or text/xml may cuase some control unhappy...
	*/
	...
	/* Additional headers */
#ifdef ENABLE_EVENTS
	DEBUG_MSG("%s (%d): h->respflags (%02x) & FLAG_TIMEOUT(%02x) = %d  \n"
			,__FUNCTION__,__LINE__,h->respflags,FLAG_TIMEOUT,(h->respflags & FLAG_TIMEOUT));
	if(h->respflags & FLAG_TIMEOUT) {
		h->res_buflen += snprintf(h->res_buf + h->res_buflen,
		                          h->res_buf_alloclen - h->res_buflen,
		                          "Timeout: Second-");
		if(h->req_Timeout) {
			DEBUG_MSG("%s (%d): h->req_Timeout = %d\n",__FUNCTION__,__LINE__,h->req_Timeout);
			h->res_buflen += snprintf(h->res_buf + h->res_buflen,
			                          h->res_buf_alloclen - h->res_buflen,
			                          "%d\r\n", h->req_Timeout);
		} else {
	...
	if(h->respflags & FLAG_SID) {
		/*	Date:	2009-06-03
		*	Name:	jimmy huang
		*	Reason:	When renew Subscription, h->req_SID would be 
		*			SID: ...
		*			HOST ...
		*			So we need to copy explicitily length of "SID: xxxx-xxxxx-xxx...."
		*	Note:	
		*			1.
		*			41 means the length of "uuid:11111111-1111-1111-1111-111111111111"
		*			length of "SID: uuid:11111111-1111-1111-1111-111111111111\r\n"
		*			would be 41 + strlen("SID: \r\n")
		*			2.
		*			This way will cut off the rest data of SID after len 41
		*/
		/*
  		h->res_buflen += snprintf(h->res_buf + h->res_buflen,
  		                          h->res_buf_alloclen - h->res_buflen,
