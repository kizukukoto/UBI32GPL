...
{
	char cAddress[INET6_ADDRSTRLEN]="", cMask[INET6_ADDRSTRLEN]="";
	switch(p->type)
	{
	case UPNPPERM_ALLOW:
		printf("allow ");
	...
	inet_ntop(AF_INET6, &(p->address), cAddress, INET6_ADDRSTRLEN); // IPv6 modification
	printf("%s/", cAddress); // IPv6 modification
	inet_ntop(AF_INET6, &(p->mask), cMask, INET6_ADDRSTRLEN); // IPv6 modification
	printf("%s ", cMask); // IPv6 modification
