...
#ifdef MINIUPNPD_DEBUG_UPNPHTTP
...
sendDummyDesc(struct upnphttp * h)
{
	static const char xml_desc[] = "<?xml version=\"1.0\"?>\r\n"
		...
		"  <serviceStateTable />"
		"</scpd>\r\n";
	BuildResp_upnphttp(h, xml_desc, sizeof(xml_desc)-1, 2);
	SendResp_upnphttp(h);
	CloseSocket_upnphttp(h);
}
#endif
...
sendXMLdesc(struct upnphttp * h, char * (f)(int *))
{
	char * desc;
	int len;
	DEBUG_MSG("get the desc required\n");
	desc = f(&len);
	if(!desc)
	{
		...
		BuildResp2_upnphttp(h, 500, "Internal Server Error",
		                    error500, sizeof(error500)-1, 6);
	}
	else
	{
		DEBUG_MSG("sendXML done\n");
		BuildResp_upnphttp(h, desc, len, 2);
	}
	SendResp_upnphttp(h);
	CloseSocket_upnphttp(h);
	free(desc);
...
/* Sends the presentation page generated by the parameter */
static void
sendHTMLpage(struct upnphttp * h, char * (f)())
{
	char * desc;
	int len;
	desc = f();
	...
	if(!desc)
	{
		static const char error500[] = "<HTML><HEAD><TITLE>Error 500</TITLE>"
			"</HEAD><BODY>Internal Server Error</BODY></HTML>\r\n";
		syslog(LOG_ERR, "Failed to generate presentation page");
		BuildResp2_upnphttp(h, 500, "Internal Server Error",
			error500, sizeof(error500)-1, 6);
	}
	else
	...
	if((h->req_buflen - h->req_contentoff) >= h->req_contentlen)
	{
		...
		else
		{
			...
			BuildResp2_upnphttp(h, 400, "Bad Request",
			                    err400str, sizeof(err400str) - 1, 6);
			SendResp_upnphttp(h);
			CloseSocket_upnphttp(h);
		}
	}
...
ProcessHTTPSubscribe_upnphttp(struct upnphttp * h, const char * path)
{
	...
	if(!h->req_Callback && !h->req_SID)
	{
		...
		BuildResp2_upnphttp(h, 412, "Precondition Failed", 0, 0, 6);
		SendResp_upnphttp(h);
		CloseSocket_upnphttp(h);
	}
	else
...
#ifdef DEBUG
				printf("\tSID generated for callback(%.*s): %s\n", h->req_CallbackLen, h->req_Callback, sid);
#endif
				h->respflags |= FLAG_SID;
				h->req_SID = sid;
				h->req_SIDLen = strlen(sid);
			}
			BuildResp_upnphttp(h, 0, 0, 4);
		}
		else
		{
			/* subscription renew */
...
#ifdef DEBUG
			printf("\tRenewing subscription for : %.*s\n", h->req_SIDLen, h->req_SID);
#endif
...
ProcessHttpQuery_upnphttp(struct upnphttp * h)
{
	...
	else if(strcmp("GET", HttpCommand) == 0)
	{
		...
		if(strcasecmp(PRESENTATION_PATH, HttpUrl) == 0)
		{
			sendHTMLpage(h, genPresentationPage);
		}
...
static const char httprespheadDesc[] =
	"%s %d %s\r\n"
	//"CONTENT-LANGUAGE: \r\n"
	"CONTENT-LENGTH: %d\r\n"
...
static const char httprespheadCont[] =
	"%s %d %s\r\n"
	"CONTENT-LENGTH: %d\r\n"
	"CONTENT-TYPE: %s; charset=\"utf-8\"\r\n"
	"DATE: %s\r\n"
...
static const char httprespheadEven[] =
	"%s %d %s\r\n"
	"DATE: %s\r\n"
	"SERVER: " MINIUPNPD_SERVER_STRING "\r\n"
	"SID: %s\r\n"
	"CONTENT-LENGTH: %d\r\n" // Required in the UDA 1.1
	"TIMEOUT: Second-%s\r\n"
	;
...
BuildHeader_upnphttp(struct upnphttp * h, int respcode,
                     const char * respmsg,
                     int bodylen, int phase)
{
	int templen;
	char tmp[8], expire_time[32];
	...
	{
		case 2:// Description
			// printf("\tDescription mode response\n");
			h->res_buflen = snprintf(h->res_buf, h->res_buf_alloclen,
					httprespheadDesc, h->HttpVer, respcode, respmsg, bodylen,
					(h->respflags&FLAG_HTML)?"text/html":"text/xml", expire_time);
			break;
		case 3:// Control
			// printf("\tControl mode response\n");
			h->res_buflen = snprintf(h->res_buf, h->res_buf_alloclen, 
					httprespheadCont, h->HttpVer, respcode, respmsg, bodylen,
					(h->respflags&FLAG_HTML)?"text/html":"text/xml", expire_time);
			break;
		case 4:// Eventing
			// printf("\tEventing mode response\n");
			if(h->req_Timeout)
				snprintf(tmp, sizeof(h->req_Timeout), "%d", h->req_Timeout);
			else
				strcpy(tmp, "infinite");
			h->res_buflen = snprintf(h->res_buf, h->res_buf_alloclen, 
					httprespheadEven, h->HttpVer, respcode, respmsg, expire_time,
					h->req_SID, bodylen, tmp);
			break;
		default:
			// printf("\tDefault mode response\n");
			h->res_buflen = snprintf(h->res_buf, h->res_buf_alloclen, 
				httpresphead, h->HttpVer, respcode, respmsg, bodylen,
				(h->respflags&FLAG_HTML)?"text/html":"text/xml", expire_time);
			/* Additional headers */
#ifdef ENABLE_EVENTS
...
BuildResp2_upnphttp(struct upnphttp * h, int respcode,
                    const char * respmsg,
                    const char * body, int bodylen, int phase)
{
#ifdef DEBUG
	printf("\t--> Building & Sending an %d %s message.\n", respcode, respmsg);
#endif
	BuildHeader_upnphttp(h, respcode, respmsg, bodylen, phase);
	if(body)
...
BuildResp_upnphttp(struct upnphttp * h,
                        const char * body, int bodylen, int phase)
{
	BuildResp2_upnphttp(h, 200, "OK", body, bodylen, phase);
}
