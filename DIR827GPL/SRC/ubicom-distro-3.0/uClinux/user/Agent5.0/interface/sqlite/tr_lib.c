...
{
    /* Because we implement the library by sqlite, so we just record the database file path here.
     * If you implement the library in other technologies such as XML or plain text file, maybe
...
{
    /* Open the database file if it has not been opened yet. The increase the reference count by 1.*/
    /* If you implement XML and have loaded the XML file into memory already, you need to do nothing
     * except to increase the reference count.*/

    /*! 
     * \note In any technology, this function MUST implement a LOCK mechanism to lock the MOT. For
     * example, a file lock to lock the database file or XML file. If some other process has hold
...
/*!
 * \return 0 on success, 1 on node not exist, -1 on error
 */
