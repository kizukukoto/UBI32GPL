...
extern char crs_username[80]; //defined in tcp.c
extern char crs_password[80]; //defined in tcp.c
...
static void notify_interval_changed(const char *new)
{
	int i;
...
static void min_keepalive_changed(const char *new)
{
	int s;
...
static void max_keepalive_changed(const char *new)
{
	int s;
...
static void enabled_changed(const char *new)
{
	int old = enabled;
...
static void server_address_changed(const char *new)
{
...
static void server_port_changed(const char *new)
{
...
void slave_destroy(struct sched *sc)
{
	if(sc) {
		...
		if(sc->pdata) { //Detach with master
			struct sched *master;
...
static int get_error_code(char *m)
{
	...
	while(tmp < m + STUN_MESSAGE_LEN && NEXT_ATTR(tmp) <= m + STUN_MESSAGE_LEN) {
		if(UNIT_TYPE(tmp) == 0x0009) { //ERROR-CODE
			unsigned char *data;
			int res;
...
static void get_mapped_address(char *m, struct sockaddr_in *nb)
{
	if(UNIT_TYPE(m) == 0x0101) {
		...
		tmp = (FIRST_ATTR(m));
		while(tmp < m + STUN_MESSAGE_LEN && NEXT_ATTR(tmp) <= m + STUN_MESSAGE_LEN) {
			if(UNIT_TYPE(tmp) == 0x0001 && UNIT_LEN(tmp) == 8) { //MAPPED-ADDRESS
		...
		if(m[0] == 0 || m[0] == 1) {
			...
			switch(UNIT_TYPE(m)) {
				case 0x0111:
					...
					break;
				case 0x0101:
					get_mapped_address(m, &binding);
					//memset(&local, 0, sizeof(local));
					local_len = sizeof(local);
					war_getsockname(sc->fd, (struct sockaddr *)&local, &local_len);

					//lib_start_session();/*ingore*/
					war_snprintf(m , sizeof(m), "%s:%d", inet_ntoa(binding.sin_addr), ntohs(binding.sin_port));
					lib_start_session();
					if(lib_resolve_node(UDP_CRS_ADDRESS, &node) == 0) {
						lib_set_value(node, m);
					}
					add_inform_parameter(UDP_CRS_ADDRESS, 1);
					behind_nat = binding.sin_addr.s_addr != local.sin_addr.s_addr || binding.sin_port != local.sin_port;
					if(nat_detected != behind_nat) {
						if(lib_resolve_node(NAT_DETECTED, &node) == 0) {
							lib_set_value(node,  behind_nat ? "true" : "false");
						}
	...
	if(res > 0) {
		...
		if(m[0] == 0 || m[0] == 1) {
			if(UNIT_TYPE(m) == 0x0111) {
				...
				if(nb.sin_addr.s_addr != binding.sin_addr.s_addr ||
						nb.sin_port != binding.sin_port) {
					node_t node;
					binding.sin_port = nb.sin_port;
					binding.sin_addr.s_addr = nb.sin_addr.s_addr;
					//lib_start_session();/*ingore*/
					war_snprintf(m , sizeof(m), "%s:%d", inet_ntoa(binding.sin_addr), ntohs(binding.sin_port));
					tr_log(LOG_DEBUG, "Binding changed, new binding: %s", m);
					lib_start_session();
					if(lib_resolve_node(UDP_CRS_ADDRESS, &node) == 0) {
						tr_log(LOG_DEBUG, "set value....\n");
						lib_set_value(node, m);
					}
					add_inform_parameter(UDP_CRS_ADDRESS, 1);
					lib_end_session();
...
int launch_udp_listener()
{
	...
	if(first) {
		...
		if(lib_start_session() > 0) {
			...
			if(res == 0)
				notify_interval_changed(buf);
			else {
				lib_end_session();
				return -1;
			}
	
			res = lib_resolve_node(UDP_CRS_ADDRESS, &node);
			if(res != 0) {
				lib_end_session();
				return -1;
			}

			war_snprintf(buf, sizeof(buf), "%s:%d", addr, port);
			if(lib_set_value(node, buf) != 0) { //Set the UDPConnectionRequestAddress to the locale address
				lib_end_session();
				return -1;
			}
