...
extern char ca_cert[256];
#else
static char ca_cert[256];
#endif
...
static void wib_on_writable(struct sched *sc)
{
	...
	if(wib->next_step == WIB_NEXT_HTTP_HEADER) {
	    push_buffer(&(wib->outbuf), "GET %s HTTP/1.1\r\n"
		    ...
		    "Connection: %s\r\n"
		    "Content-Length: 0\r\n"
		    "\r\n", wib->conn.path, wib->conn.host, wib->conn.port, "keep-alive");
 ...
 * \param original certificate and length
...
{ 
    ...
    for(i = 0; i < loop; i++){
	fwrite((base64+i*76), 1, 76,  fp);
	fwrite("\n", 1, 1, fp);
    }
    fwrite((base64+i*76), 1, res,  fp);
    fwrite("\r\n", 1, 2, fp);
...
{
	int res = 0;
	int p_len; 	/*stroe after decrpt data length*/
	...
	unsigned char username_flag = 0, password_flag = 0, url_flag = 0, cert_flag = 0;/*Flag for duplicate TLV*/
	node_t node;
			...
			case 0x00: /*TLV nonce*/
				...
				if(tlv_type == 0x200 || tlv_type == 0x201) {
					...
					if(tlv_type == 0x200 && username_flag == 0) {
						...
						res = lib_resolve_node(SESSION_AUTH_USERNAME, &node);
						if(res == 0) {
							res = lib_set_value(node, (const char*)tlv_value);
							if(res != 0) {
								tr_log(LOG_ERROR, "set username error[exit]");
							}
						}
						username_flag = 1;
					} else if(tlv_type == 0x201 && password_flag == 0){
						tr_log(LOG_DEBUG, "it is password = %s", tlv_value);
						res = lib_resolve_node(SESSION_AUTH_PASSWORD, &node);
						if(res == 0) {
						    res = lib_set_value(node, (const char*)tlv_value);
						    if(res != 0) {
							tr_log(LOG_ERROR, "set password error[exit]");
	    						}
						}
						password_flag = 1;
					} else {
			...
			case 0x202:
				if(url_flag == 0) {
					...
					*(tlv_value + tlv_length) = '\0';
					...
					res = lib_resolve_node(ACS_URL, &node);
					if(res == 0) {
						res = lib_set_value(node, (const char*)tlv_value);
		    				if(res != 0) {
							tr_log(LOG_ERROR, "set URL error[exit]");
						}
					}
					free(tlv_value);
...
static void wib_on_readable(struct sched *sc)
{
	...
	if(wib->http.msg_type != HTTP_RESPONSE) {
	    ...
	    int res_code = atoi(wib->http.start_line.response.code);
...
/*redirect test*/
		...
		case 200:
			...
			if(ct == NULL || war_strncasecmp(ct, "application/vnd.wmf.bootstrap", 29)) {
				...
				if(res == -1 ||(cnt = fread(body_data, 1, body_len, (FILE *)(wib->http.body))) != body_len) {
				    tr_log(LOG_ERROR, "fread return size = %d, fread error: %s", cnt, war_strerror(war_geterror()));
				    if(body_data)
                                        free(body_data);
				    sc->need_destroy = 1;	
				    return ;
