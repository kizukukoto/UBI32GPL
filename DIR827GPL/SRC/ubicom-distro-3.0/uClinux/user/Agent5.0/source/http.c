		...
		if(h->inlen > 0) {
			...
			if(nl) {
				*nl = '\0';
				nl += 2;
				/*!
				 * \note the space size of outbuf MUST not smaller than h->inbuf
				 */
...
static http_recv_status_t http_recv_body(struct http *h, struct connection *conn)
{
	...
	if(h->msg_type == HTTP_RESPONSE) {
		response_code = atoi(h->start_line.response.code);
	}
	...
	for(;;) {
				...
				if(h->block_len == h->inlen) {
					if(h->inlen > 2) {
						if(response_code == 200) {
							...
							if(h->body_type == HTTP_BODY_BUFFER)
								...
								res = fwrite(h->inbuf, 1, h->inlen - 2, (FILE *)(h->body));
							else
								res = h->inlen - 2;
						} else {
							res = h->inlen - 2;
					...
					if(response_code == 200) {
						...
						if(h->body_type == HTTP_BODY_BUFFER)
							...
							res = fwrite(h->inbuf, 1, h->inlen, (FILE *)(h->body));
						else
							res = h->inlen;
					} else {
						res = h->inlen;
					...
					if(response_code == 200) {
						...
						if(h->body_type == HTTP_BODY_BUFFER)
							...
							res = fwrite(h->inbuf, 1, h->block_len - 2, (FILE *)(h->body));
						else
							res = h->block_len - 2;
					} else {
						res = h->block_len - 2;
					...
					if(response_code == 200) {
						if(h->inlen > 1) {
							...
							if(h->body_type == HTTP_BODY_BUFFER)
								...
								res = fwrite(h->inbuf, 1, h->inlen - 1, (FILE *)(h->body));
							else
								res = h->inlen - 1;
						}
					} else {
		...
		if(h->block_len > 0) {
			res = tr_conn_recv(conn, h->inbuf, MIN(sizeof(h->inbuf) - 1, h->block_len));
			if(res > 0) {
				h->inlen += res;
				h->inbuf[h->inlen] = '\0';
				h->bytes_received += res;
...
char *http_get_header(struct http *h, const char *name)
{
	...
	for(i = 0; i < h->header_count; i++) {
		if(war_strncasecmp(h->headers[i], name, len) == 0) {
			return h->headers[i] + len;
		}
	}
...
static int http_parse_digest_chal(struct http *h, char *left)
{
	...
	while(http_next_arg(&left, &name, &value, ',') == 0) {
		if(war_strcasecmp(name, "domain") == 0) {
			...
			if(war_strcasecmp(value, "auth") == 0) {
				h->chal.qop = QOP_AUTH;
			} else {
