...
#ifdef MAIL_CLIENT_DEBUG
#define DEBUG_MSG(fmt, arg...)       {printf("%s [%d]: ", __FILE__, __LINE__); printf(fmt, ##arg);}
//we cannot use the below syntax, because of translation. Therefore, we use the above hack as a workaround.
    ...
    *lmtp_error_msgs = NULL;
    ...
    if (read_recipients)
    {
		if ((e = msmtp_read_recipients(f, list_last(recipients), &tmpfile, 
				errstr)) != EX_OK)
		{
		    syslog(LOG_INFO,"SMTP client send Email unsuccessfully !!!\n");
		    goto error_exit;   
		}
	...
	syslog(LOG_INFO,"SMTP (Email) server %s is at IP address %s:%d\n", acc->host, server_address, acc->port);
...
send_without_auth:
    ...
    *mailsize = 0;
	...
	syslog(LOG_INFO,"SMTP client send Email successfully\n");
	printf("msmtp_sendmail OK \n");
	_system("rm -f %s", LOG_FILE_FULL);
	_system("rm -f %s", LOG_FILE_MAIL);
