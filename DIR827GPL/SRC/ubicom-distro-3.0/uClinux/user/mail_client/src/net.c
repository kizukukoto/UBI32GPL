...
int net_puts(int fd, const char *s, size_t len, char **errstr)
{
	...
	if ((ret = send(fd, s, len, 0)) < 0)
	{
		if (errno == EINTR)
			...
			*errstr = xasprintf(_("network write error: %s"), _("the operation timed out"));
		else
			*errstr = xasprintf(_("network write error: %s"), strerror(errno));
