...
static int cgi_create_env(server *srv, connection *con, plugin_data *p, buffer *cgi_handler) {
	...
	if (srv_sock->is_ssl) {
		...
		if (NULL != (ds = (data_string *)array_get_element(con->request.headers, "SOAPAction"))){
			...
			cgi_env_add(&env, CONST_STR_LEN("SOAPACTION"), soap_string, strlen(soap_string));
		}	
		//log_error_write(srv, __FILE__, __LINE__, "ss", "soap_string:", soap_string);
		
//add for web file access html5
		cgi_env_add(&env, CONST_STR_LEN("WS_ACTION"), CONST_STR_LEN(ws_action));		

		//if(strlen(ws_action) > 0){
			data_string *ds;
			if (NULL != (ds = (data_string *)array_get_element(con->request.headers, "Cookie"))){				
				cgi_env_add(&env, CONST_STR_LEN("Cookie"), ds->value->ptr, strlen(ds->value->ptr));			
			}
		//}
//end for web file access html5
