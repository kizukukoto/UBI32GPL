...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	if (!static_lease_ip) {
		...
		if((hostname = get_option(oldpacket, DHCP_HOST_NAME)) != NULL)
			memcpy(host, hostname, *(hostname - 1));

		if (!add_lease(packet.chaddr, packet.yiaddr, server_config.offer_time, host)) {
...
int sendACK(struct dhcpMessage *oldpacket, uint32_t yiaddr)
{
	...
	add_lease(packet.chaddr, packet.yiaddr, lease_time_align, host);
