...
#if ENABLE_FEATURE_HTTPD_BASIC_AUTH && !ENABLE_FEATURE_HTTPD_GRAPH_AUTH
...
remaping_url(const char *purl)
{
	...
	struct {
		...
		{ "/version.txt", "/cgi/ssi/version.txt" },
		{ "/chklst.txt", "/cgi/ssi/tmp/chklst.txt" },
		{ "/wlan.txt", "/cgi/ssi/tmp/wlan.txt" },
		{ "/reject.html", "/cgi/ssi/tmp/index.html" },
		{ "/wl.txt", "/cgi/ssi/wl.txt" },
		{ NULL, NULL }
	};
...
//#ifdef GRAPHICAL_AUTH
	...
	auth_setenv_from_file(config->rmt_ip_str);
#endif
	return 1;
#else
...
#endif /* ENABLE_FEATURE_HTTPD_BASIC_AUTH */
	...
	while (1) {
		...
		*/	
		if (nfound <= 0) {
			...
			close(inFd);
			if (WIFEXITED(status))
				DEBUG_MSG("piped has exited with status=%d\n", WEXITSTATUS(status));
			if (WIFSIGNALED(status))
				DEBUG_MSG("piped has exited with signal=%d\n", WTERMSIG(status));
			break;
		}
#if ENABLE_FEATURE_HTTPD_SSL
...
static int sendFile(char *url)
{
	...
	DEBUG_MSG("sending file '%s' content-type: %s\n",
...
static int checkPermIP(void)
{
	...
	for (cur = config->ip_a_d; cur; cur = cur->next) {
		DEBUG_MSG("checkPermIP: '%s' ? ", config->rmt_ip_str);
		DEBUG_MSG("'%u.%u.%u.%u/%u.%u.%u.%u'\n",
