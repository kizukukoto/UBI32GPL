...
<script language="JavaScript">
    ...
    function send_request()
    {
        ...
        if (get_by_id("qos_traffic_shaping").checked) {
            ...
            if (qos_uplink > 2048 || 20 > qos_uplink || isNaN(qos_uplink)|| get_by_id("qos_uplink").value == "") {
                alert(at_alert_1);
                return false;
            }
        }
        ...
        for (var i = 0; i < rule_max_num; i++) {
            ...
            var local_start_port = get_by_id("local_start_port" + i).value;
            var local_end_port = get_by_id("local_end_port" + i).value;
            var remote_start_port = get_by_id("remote_start_port" + i).value;
            var remote_end_port = get_by_id("remote_end_port" + i).value;
            ...
            if (get_by_id("name" + i).value != "") {
				...
				if (!check_ip_order(temp_local_start_ip_obj, temp_local_end_ip_obj)) {
	                alert(msg[IP_RANGE_ERROR]);
	                return false;
	            }
	            if (!check_ip_order(temp_remote_start_ip_obj, temp_remote_end_ip_obj)) {
	                alert(msg[IP_RANGE_ERROR]);
	                return false;
	            }
                //check port
                if ((get_by_id("protocol_select"+i).selectedIndex ==1) || (get_by_id("protocol_select"+i).selectedIndex ==2) || (get_by_id("protocol_select"+i).selectedIndex ==3)){
                    if (!check_positive_integer(local_start_port) || local_start_port < 0 || local_start_port > 65535) {
                        alert(YM59);
                        get_by_id("local_start_port"+i).focus();
                        get_by_id("local_start_port"+i).select();
                        return false;
                    }
                    else if (!check_positive_integer(local_end_port) || local_end_port < 0 || local_end_port > 65535) {
                        alert(YM60);
                    ...
                    if (!check_positive_integer(remote_start_port) || remote_start_port < 0 || remote_start_port > 65535) {
                        alert(YM61);
                        get_by_id("remote_start_port"+i).focus();
                        get_by_id("remote_start_port"+i).select();
                        return false;
                    }
                    else if (!check_positive_integer(remote_end_port) || remote_end_port < 0 || remote_end_port > 65535) {
                        alert(YM62);
