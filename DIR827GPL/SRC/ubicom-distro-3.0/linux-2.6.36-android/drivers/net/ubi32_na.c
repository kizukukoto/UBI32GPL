...
#if defined(CONFIG_SUPPORT_VLAN_IGMP)
			if ( skb->data[12] == 0x81 && skb->data[13] == 0x00 &&  // VLAN
				 skb->data[0x10] == 0x08 && skb->data[0x11] == 0x00 && // IPV4
				  skb->data[0x1b] == 0x02 && // IGMP
				  skb->data[0x22] == 0xe0 && skb->data[0x23] == 0x00 && 
				  skb->data[0x24] == 0x00 && skb->data[0x25] == 0x01  // 224.0.0.1 dst 

				  // maybe check ip == 224.0.0.1
				) {
				asm volatile (
				"	move.2	%0, 14(%1)	\n\t"
				"	move.2	14(%1), 10(%1)	\n\t"
...
struct MY_PHY_REG {
	...
	uint32_t result;
		...
		if( cmd == (SIOCDEVPRIVATE+1)) {
			...
			reg = (struct MY_PHY_REG *) rq->ifr_ifru.ifru_slave;
			if( reg->cmd == 1) {
				reg->value = athrs17_reg_read(reg->addr);
				reg->result = 0;
			}
			else if( reg->cmd == 2) {
				athrs17_reg_write(reg->addr,reg->value);
				reg->result = 0;
			}
		}
