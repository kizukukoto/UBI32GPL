...
static int ip6mr_send_icmp_info(struct mr6_table *mrt,struct sk_buff *pkt, mifi_t mifi, int assert)
{
...
int ip6_mr_process_icmp(struct sk_buff *skb)
{
	...
	if( vif >= 0 ) 
		ip6mr_send_icmp_info(mrt,skb,vif,199);
...
// 2011.06.03
	vif = ip6mr_find_vif(mrt,skb->dev);
	if (vif >= 0 && mrt->vif6_table[vif].flags & MIFF_NO_ORG ) {
		cache = ip6mr_cache_find(mrt,&ipv6_Zero, &ipv6_hdr(skb)->daddr);
