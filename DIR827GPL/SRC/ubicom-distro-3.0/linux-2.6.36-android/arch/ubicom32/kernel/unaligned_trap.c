...
static DEFINE_SPINLOCK(unaligned_lock);
...
int unaligned_emulate(struct pt_regs *regs)
{
	...
	set_fs(KERNEL_DS);
	result = copy_from_user(&inst, (void *)(regs->pc), sizeof(inst));
	set_fs(old_fs);
	...
	if (src_operand) {
		 ...
		 */
		if (likely(four_byte)) {
			if (__copy_from_user_no_check(&source_data, (void *)(src_address), 4)) {
				/*
			...
			if (__copy_from_user_no_check(&s_data, (void *)(src_address), 2)) {
				/*
				 * The source address is bad. Change the fault to TRAP_CAUSE_SRC1_DECOEE_ERR.
				 */
				regs->trap_cause = (1 << TRAP_CAUSE_SRC1_DECODE_ERR);
		...
		if ((s1 >> 8) == 0) {
			...
			if (s1 < 0x40) {
				...
				spin_lock_irqsave(&unaligned_lock, flags);
				...
				*((int *)&unaligned_emulate_get_src) |= (inst & 0x7ff);
				...
				spin_unlock_irqrestore(&unaligned_lock, flags);
			}
		}
	}
	...
	*((unsigned int *)&trap_emulate) = emulate_inst;
	...
	if (dest_operand) {
		 ...
		 */
		if (four_byte) {
			if (__copy_to_user_no_check((void *)(dest_address), (void *)&result, 4)) {
				/*
			...
			if (__copy_to_user_no_check((void *)(dest_address), (void *)&s_data, 2)) {
				/*
				 * The destination address is bad. Change the fault to TRAP_CAUSE_DST_DECODE_ERR.
				 */
				regs->trap_cause = (1 << TRAP_CAUSE_DST_DECODE_ERR);
