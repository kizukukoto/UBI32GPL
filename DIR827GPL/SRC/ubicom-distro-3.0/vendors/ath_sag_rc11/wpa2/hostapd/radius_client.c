		     ...
		     int sock, int sock6, int auth)
{
	...
	if (connect(sel_sock, addr, addrlen) < 0) {
		printf("radius server %s connect failed \n",inet_ntoa(((struct sockaddr_in *)addr)->sin_addr));
...
/*  Date: 2010-01-06
*   Name: jimmy huang
*   Reason: When hostapd has been assign 2 radius authentication server
*			Once the first is not reachable (different subnet, no default route)
*			It will failed when connect(), and the failover mechanism is not implement
*			in that part, thus the second radius server will never be used
*			So, we force it to retry 3 times (RADIUS_CLIENT_INIT_AUTH_MAX_RETRIES)
*			If failed, using second server
*   Note:
*			Not using eloop_register_timeout()
*/
#define RADIUS_CLIENT_INIT_AUTH_MAX_RETRIES 3
	struct hostapd_radius_server *next, *old;
	int init_auth_failover = 0;
	int init_ret = 0;
	int retry_count = 0;
...
INIT_RETRY:
	...
	if (next > &(conf->auth_servers[conf->num_auth_servers - 1])){
		printf("%s %d, All radius server are unreachable ! \n",__func__,__LINE__);
		return -1;
	}
...
*/
	ok = 0;
	for(retry_count = 0; retry_count < RADIUS_CLIENT_INIT_AUTH_MAX_RETRIES ; retry_count++){
		init_ret = radius_change_server(radius, next, old,
	...
	if(!ok){
		printf("%s %d, radius_change_server failed, try next server \n",__func__,__LINE__);
		old = next;
		goto INIT_RETRY;
	}
