...
extern void save_to_nvram_for_single_mbssid(const char *iface, const char *data);
#define save_to_nvram(data) save_to_nvram_for_single_mbssid(hapd->conf->iface, data)
#else
extern void save_to_nvram(const char *);
#endif

extern int check_need_lock(void);
				       ...
				       const u8 *uuid_e)
{
	...
	/* Cameo : WPS Success for GUI*/
	system("echo \"1\" > /var/tmp/wps_result");
	system("echo \"0\" > /var/tmp/mp_wps");
#ifdef PLC_SUPPORT
	system("cli plc led sync blink stop");
	system("cli plc led sync light-on-direct");
	system("int6k -i br0 -R");
#endif
        wpa_printf(MSG_CAMEO, "CAMEO: hostapd_wps_reg_success_cb");
}
...
/* NickChou For A/G Concurrent Model to run the same WSC_athX.conf, restart_wlan() relpace reload_config() */
	wpa_printf(MSG_CAMEO, "CAMEO: wps_reload_config: ??? restart_wlan ???");
	//system("rc restart &");	
}
 ...
 * */
	...
	wpa_printf(MSG_CAMEO, "CAMEO: hapd_wps_cred_cb: WPS_STATE_CONFIGURED\n");
...
/* Robert 2011/6/29 */
	...
	/* 2012.01.16, According to D-Link request, user just do. we must be disabled wps pin. */
	//if (check_need_lock())
		fprintf(nconf, "config_methods=push_button\n");
	
	fprintf(nconf, "ssid=");
	for (i = 0; i < cred->ssid_len; i++)
		fputc(cred->ssid[i], nconf);
	fprintf(nconf, "\n");
	save_to_nvram("wps_state=2");
	sprintf(tmp_data, "ssid=%s", cred->ssid);
	wpa_printf(MSG_CAMEO, "CAMEO: save_to_nvram: [%s]\n", tmp_data);	
	save_to_nvram(tmp_data);
	...
	if (wpa) {
		...
		sprintf(tmp_data, "wpa=%d", wpa);
		save_to_nvram(tmp_data);
		fprintf(nconf, "wpa_key_mgmt=");
		sprintf(tmp_data, "wpa_key_mgmt=");
		prefix = "";
		if (cred->auth_type & (WPS_AUTH_WPA2 | WPS_AUTH_WPA)) {
			fprintf(nconf, "WPA-EAP");
			sprintf(tmp_data, "%sWPA-EAP", tmp_data);
			prefix = " ";
		}
		if (cred->auth_type & (WPS_AUTH_WPA2PSK | WPS_AUTH_WPAPSK)) {
			fprintf(nconf, "%sWPA-PSK", prefix);
			sprintf(tmp_data, "%sWPA-PSK", tmp_data);
		}
		fprintf(nconf, "\n");
		save_to_nvram(tmp_data);

		fprintf(nconf, "wpa_pairwise=");
		sprintf(tmp_data, "wpa_pairwise=");
		prefix = "";
		if (cred->encr_type & WPS_ENCR_AES) {
			fprintf(nconf, "CCMP");
			sprintf(tmp_data, "%sCCMP", tmp_data);
			prefix = " ";
		}
		if (cred->encr_type & WPS_ENCR_TKIP) {
			fprintf(nconf, "%sTKIP", prefix);
			sprintf(tmp_data, "%sTKIP", tmp_data);
		}
		fprintf(nconf, "\n");
		save_to_nvram(tmp_data);
		...
		if (cred->key_len >= 8 && cred->key_len < 64) {
			...
			fprintf(nconf, "\n");
			sprintf(tmp_data, "wpa_passphrase=%s", cred->key);
			save_to_nvram(tmp_data);
		} else if (cred->key_len == 64) {
			fprintf(nconf, "wpa_psk=");
			for (i = 0; i < cred->key_len; i++)
				fputc(cred->key[i], nconf);
			sprintf(tmp_data, "wpa_psk=%s", cred->key);
			save_to_nvram(tmp_data);
			fprintf(nconf, "\n");
		} else {
	...
	/*
	NickChou 2012.01.03 add ap_pin and config_methods
        Disable WPS PIN Method for Brute Forcing WPS
	*/
		     str_starts(buf, "ap_pin=") ||
		     str_starts(buf, "config_methods=") ||
		     str_starts(buf, "wpa_passphrase=")))
		{
#if 0 /* According to D-Link request, user just do pin. We will be */
			if (str_starts(buf, "ap_pin") || str_starts(buf, "config_methods")) {
				/*It need to check when to lock disable wps pin by Robert */
				if (check_need_lock())
	...
	*/
	save_to_nvram("setup_wizard=0");
	
	wpa_printf(MSG_DEBUG, "WPS: AP configuration updated");

	/* nvram commit */
	save_to_nvram("done");
	wpa_printf(MSG_CAMEO, "CAMEO: hapd_wps_cred_cb: nvram commit");
...
static void hostapd_wps_reenable_ap_pin(void *eloop_data, void *user_ctx)
{
	...
	{
		wpa_printf(MSG_DEBUG, "WPS: hostapd_wps_reenable_ap_pin: ap_setup_locked");
		return;
	}	

	/*
	NickChou, 2012.2.21 from Atheros patch
	brute-force attack WPS PIN
	*/
	if (hapd->ap_pin_failures_consecutive >= WPS_PIN_MAX_RETIME)
	{
		wpa_printf(MSG_CAMEO, "CAMEO: WPS LOCK Enabled indefinitely");		
		return;
	}	
...
static int wps_pwd_auth_fail(struct hostapd_data *hapd, void *ctx)
{
...
#if 0 //NickChou mark for AP PIN disabled indefinitely
	...
	*/
	...
	if (!hapd->conf->ap_setup_locked &&
	    hapd->ap_pin_failures_consecutive >= WPS_PIN_MAX_RETIME) 
	{
		/*
		...
		wpa_printf(MSG_DEBUG, "WPS LOCK: AP PIN disabled indefinitely");
		save_to_nvram("wps_lock=1");
		save_to_nvram("done");/* nvram commit */
				 ...
				 struct wps_context *wps)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: hostapd_wps_upnp_init: upnp_wps_device_init\n");
