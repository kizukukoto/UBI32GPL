				   ...
				   struct sta_info *sta, int authorized)
{
	...
	if (authorized) {
		if (!ap_sta_is_authorized(sta)) {
			...
			/*Robert, Maybe it need to re-boot. 2011-8-19*/
			restart_wlan();
		}
					  ...
					  const u8 *eap, size_t len)
{
	...
	/*
	* Robert Chen, 2012.01.13
	* Fix bug and to support and patch to mac auth ability on WPA/WPA2-EAP
 	* */
	if ((hapd->conf->mac_auth == hapd->conf->radius->auth_server->index) || (hapd->conf->mac_auth == 3 )){
		...
		printf("Enable MACAddr Auth. => add Calling-Station-Id\n");
	} else {
		printf("Disable MACAddr Auth. => Do not add Calling-Station-Id\n");
	}
...
int ieee802_1x_init(struct hostapd_data *hapd)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: ieee802_1x_init: start");	
	if ((hapd->conf->ieee802_1x || hapd->conf->wpa) &&
	...
	wpa_printf(MSG_CAMEO, "CAMEO: ieee802_1x_init: end");
	return 0;
}
