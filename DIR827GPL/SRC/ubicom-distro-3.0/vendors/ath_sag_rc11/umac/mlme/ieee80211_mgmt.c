                        ...
                        struct ieee80211_ie_intolerant_report *intol_ie)
{
    struct ieee80211com *ic = ni->ni_ic;
    int i, j;
    u_int8_t intol_chan  = 0, center_chan = 0;
        ...
        */
        ...
        for (j = 0; j < ic->ic_nchans; j++) {
            if (intol_chan == ic->ic_channels[j].ic_ieee) {
               if (ni->ni_vap->iv_cur_mode == IEEE80211_MODE_11NG_HT40PLUS) {
                       center_chan = ieee80211_chan2ieee(ic, ic->ic_curchan) + 2;
               }
               else {
                       center_chan = ieee80211_chan2ieee(ic, ic->ic_curchan) - 2;
               }
               if ( intol_chan <= (center_chan + 5) &&  intol_chan >= (center_chan -5) ) {
                IEEE80211_NOTE(ni->ni_vap, IEEE80211_MSG_ACTION, ni,
                               "%s: Found intolerant channel %d under center channel %d",
                               __func__, intol_chan, center_chan);
                return 1;
            }
        }
    }
