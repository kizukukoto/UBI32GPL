...
ar5416TxReqIntrDesc(struct ath_hal *ah, void *ds)
{
    ...
    OS_FLUSH_CACHE_SINGLE(&ads->ds_ctl0);
}
...
ar5416FillKeyTxDesc(struct ath_hal *ah, void *ds, HAL_KEY_TYPE keyType)
{
    ...
    //OS_FLUSH_CACHE_SINGLE(&ads->ds_ctl6);
    return AH_TRUE;
}
...
#endif
	OS_FLUSH_AR5416_TX_DESC(ads, 1);
	if (ads != last_ads) OS_FLUSH_AR5416_TX_DESC(last_ads, 1);
}
...
ar5416Set11nAggrFirst(struct ath_hal *ah, void *ds, u_int aggrLen)
{
    ...
    OS_FLUSH_CACHE_SINGLE(&ads->ds_ctl1);
    OS_FLUSH_CACHE_SINGLE(&ads->ds_ctl6);
}
                                                  ...
                                                  u_int burstDuration)
{
    ...
    OS_FLUSH_CACHE_SINGLE(&ads->ds_ctl2);
}
...
ar5416Set11nRifsBurstMiddle(struct ath_hal *ah, void *ds)
{
    ...
    ads->ds_ctl0 &= ~AR_TxIntrReq;
    OS_FLUSH_CACHE_SINGLE(&ads->ds_ctl0); /* ctl0 & ctl1 in the same cache line */
}
