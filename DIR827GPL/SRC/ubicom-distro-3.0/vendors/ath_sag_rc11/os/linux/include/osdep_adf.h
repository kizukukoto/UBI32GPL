...
#ifdef __ubicom32__
static inline void * OS_MACSET(void *_mac, u8 _c)
{
    u8 *mac = _mac;
    mac[0] = _c;
    mac[1] = _c;
...
{
    const u16 *mac1 = _mac1;
    const u16 *mac2 = _mac2;
    return ((mac1[0] ^ mac2[0]) | (mac1[1] ^ mac2[1]) | (mac1[2] ^ mac2[2]));
}
#else
#define OS_MACCPY(_mac1, _mac2)         OS_MEMCPY(_mac1, _mac2, 6)
#define OS_MACCMP(_mac1, _mac2)         OS_MEMCMP(_mac1, _mac2, 6)
#define OS_MACSET(_mac1, _c)            OS_MEMSET(_mac1, _c, 6)
#endif
...
#define spin_trylock_bh(x) _spin_trylock_bh(x)
...
#define BUS_DMA_UBICOM32_FROMDEVICE BUS_DMA_TODEVICE
...
os_mesgq_handler(void *timer_arg)
{
    ...
    while(mesg) {
         ...
         */
        spin_lock_bh(&(queue->ev_handler_lock));
        queue->handler(queue->ctx,mesg->mesg_type,mesg->mesg_len, msg);
        spin_unlock_bh(&(queue->ev_handler_lock));
