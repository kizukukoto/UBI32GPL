...
int do_log(unsigned char *pattern_host,unsigned char *pattern_url,int accept_url, u32 src_ip)
{
	...
	if(accept_url)
		printk(KERN_INFO "%s accessed from %s\n",logUrl,ntoa(src_ip));
	else
		printk(KERN_INFO "%s blocked for %s\n",logUrl,ntoa(src_ip));
...
static bool urlfilter_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if(*(tmpinfo.url + info_host_len ) == '%')
	{
		need_log=1;

//		if(*(tmpinfo.host + info_host_len - 1) == '1')
		if(*(tmpinfo.url + info_host_len + 1) == '1')
			accept_url=1;
		else
			accept_url=0;
...
//printk("[%s] %d\n",tmpinfo.url,accept_url);
	if(strstr(tmpinfo.url, "ALL"))
	{
		if(need_log)
