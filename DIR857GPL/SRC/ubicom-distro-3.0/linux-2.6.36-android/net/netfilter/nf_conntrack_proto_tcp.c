...
//Albert add for spi feature Ubicom needs flag so it can fix NA SPI deadlock
int sysctl_spi_enable __read_mostly = 0;
EXPORT_SYMBOL_GPL(sysctl_spi_enable);
...
static int ip6_ct_tcp_be_liberal __read_mostly = 0;
			  ...
			  u_int8_t pf)
{
	...
	if (before(seq, sender->td_maxend + 1) &&
		...
		if (sender->flags & IP_CT_TCP_FLAG_BE_LIBERAL ||
		    ...
		    (pf==PF_INET6 && ip6_ct_tcp_be_liberal)) { // support simple security REC-32
			res = true;
		}
...
static struct ctl_table tcp_sysctl_table[] = {
	...
	{
		.procname       = "ip6_conntrack_tcp_be_liberal",
		.data           = &ip6_ct_tcp_be_liberal,
		.maxlen         = sizeof(unsigned int),
		.mode           = 0644,
		.proc_handler   = proc_dointvec,
	},
