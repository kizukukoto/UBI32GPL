...
ar9300FillKeyTxDesc(struct ath_hal *ah, void *ds, HAL_KEY_TYPE keyType)
{
    ...
    ads->ds_ctl17 = SM(keyType, AR_EncrType);
    //OS_FLUSH_CACHE_SINGLE(&ads->ds_ctl17);
    return AH_TRUE;
}
...
ar9300SetDescLink(struct ath_hal *ah, void *ds, u_int32_t link)
{
    ...
    OS_FLUSH_AR9300_TXC_DESC(ads, 1);
}
...
ar9300ClearDestMask(struct ath_hal *ah, void *ds)
{
    ...
    OS_FLUSH_CACHE_SINGLE(&ads->ds_ctl11);
}
#endif
...
//                        ads->ds_info,ads->status1,ads->status8);
    ...
    if ((ads->status3 & (AR_ExcessiveRetries|AR_Filtered|AR_FIFOUnderrun|AR_TxTimerExpired
                        |AR_DescCfgErr|AR_TxDataUnderrun|AR_TxDelimUnderrun)) ||
        (ads->status8 & AR_TxOpExceeded)) {
    ...
    if ((ahp->ts_tail & 7) == 0) {
        struct ar9300_txs *ads_8ago = &ahp->ts_ring[(ahp->ts_tail - 8) & (ahp->ts_size-1)];
        OS_FLUSH_AR9300_TX_DESC8(ads_8ago); //Ubicom32: include memset zero
    }
...
ar9300SetPAPRDTxDesc(struct ath_hal *ah, void *ds, int chainNum)
{
    ...
    OS_FLUSH_CACHE_SINGLE(&ads->ds_ctl12);
}
HAL_STATUS
    ...
    u_int flags)
{
    ...
    ads->ds_ctl22 = 0;
#ifdef AH_DEBUG
    ads->ds_ctl10 &= ~AR_TxPtrChkSum; /* ubicom32: force checksum error for debugging */
#endif
}
...
#endif
    OS_FLUSH_AR9300_TXC_DESC(ads, 1);
    if (ads != last_ads) OS_FLUSH_AR9300_TXC_DESC(last_ads, 1);
}
...
ar9300Set11nBurstDuration(struct ath_hal *ah, void *ds, u_int burstDuration)
{
    ...
    OS_FLUSH_CACHE_SINGLE(&ads->ds_ctl13);
}
