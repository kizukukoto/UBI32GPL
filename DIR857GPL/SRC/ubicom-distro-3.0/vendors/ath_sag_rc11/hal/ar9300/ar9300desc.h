...
#if defined(__ubicom32__) && defined(IP8000)
/*
 * This is used by RX descriptor and individual TX status descriptor invalidation.
 */
static inline void OS_FLUSH_AR9300_DESC(void *addr, int sync)
{
    asm volatile (
    " flush   (%[addr]) \n\t"
    " flush 32(%[addr]) \n\t"
...
static inline void OS_FLUSH_AR9300_TXC_DESC(void *addr, int sync)
{
    asm volatile (
    " flush   (%[addr]) \n\t"
    " flush 32(%[addr]) \n\t"
...
/* flush and sync 8 TX status descriptors together for cache line alignment */
static inline void OS_FLUSH_AR9300_TX_DESC8(void *_addr)
{
    HALASSERT(!((int)_addr & (L1_CACHE_BYTES - 1)));
    asm volatile (
    " .rept 9                  \n\t"
    " movea   (%[addr])4++, #0 \n\t"
    " .rept 6                  \n\t"
...
#else
#define OS_FLUSH_AR9300_DESC(_addr, sync)
#define OS_FLUSH_AR9300_TXC_DESC(_addr, sync)
...
#else
#define OS_FLUSH_AR9300_TX_DESC8(_addr)
#endif
#endif
