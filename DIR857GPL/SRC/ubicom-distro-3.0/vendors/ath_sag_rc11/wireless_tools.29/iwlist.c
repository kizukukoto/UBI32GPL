		...
		int		buflen)
{
 ...
 iw_print_ie_wpa2(unsigned char *	iebuf,
		int		buflen,char * wpa)
{
  int			ielen = iebuf[1] + 2;
  int			offset = 2;	/* Skip the IE id, and the length. */
  ...
  for(i = 0; i < cnt; i++)
	...
	{
		strcat(wpa,iw_ie_cypher_name[iebuf[offset+3]]);
 	}
      offset+=4;
    }
...
#ifdef ATH_GENERAL_WDS_EXTAP
static inline void
iw_print_gen_ie2(unsigned char *	buffer,
		int		buflen, char * total)
{
  int offset = 0;
  char wpa[32];
  ...
  while(offset <= (buflen - 2))
	...
	case 0xdd:	/* WPA1 (and other) */
	case 0x30:	/* WPA2 */
	  iw_print_ie_wpa2(buffer + offset, buflen,wpa);
	  break;
...
static inline void
print_scanning_token2(struct stream_descr *	stream,	/* Stream of events */
		     struct iw_event *		event,	/* Extracted token */
		     struct iwscan_state *	state,
		     struct iw_range *	iw_range,	/* Range info */
		     int		has_range)
{
  ...
  FILE *fd=NULL;
  fd=fopen("/var/tmp/site_survey","a+");
  /* Now, let's decode the event */
  switch(event->cmd)
    {
    case SIOCGIWAP:
    ...
    case SIOCGIWMODUL:
      break;
    case IWEVQUAL:
	  //fprintf(fd,",%g\t",(((&event->u.qual)->level) / 2.0) - 110.0);
	  //fprintf(fd,",%d\t,%d\t",(&event->u.qual)->qual,(iw_range->max_qual).qual);
	  //fprintf(fd,",%g\t",((((&event->u.qual)->level) / 2.0) - 110.0));
	  fprintf(fd,",%d\t",get_percent((&event->u.qual)->level));
      break;
#ifndef WE_ESSENTIAL
    case IWEVGENIE:
	{
	 ...
	 memset(total,'\0',sizeof(total));
      /* Informations Elements are complex, let's do only some of them */
      iw_print_gen_ie2(event->u.data.pointer, event->u.data.length,total);
	  if(strlen(total)!=0)
		fprintf(fd,",%s",total);
	}
      break;
		    ...
		    int		count)		/* Args count */
{
  ...
  int remove_err;
  remove_err=unlink("/var/tmp/site_survey");
  ...
  if(wrq.u.data.length)
      ...
      do
	{
	  ...
	  if(ret > 0)
	    print_scanning_token2(&stream, &iwe, &state,
				 &range, has_range);
	}
      while(ret > 0);
      printf("\n");
...
#ifdef ATH_GENERAL_WDS_EXTAP
  { "list",		print_scanning_info2,	-1, "[essid NNN] [last]" },
#endif
