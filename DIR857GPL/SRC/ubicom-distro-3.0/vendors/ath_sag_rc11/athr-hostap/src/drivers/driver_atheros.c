...
atheros_set_ieee8021x(void *priv, struct wpa_bss_params *params)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: atheros_set_ieee8021x:  finished");
...
atheros_set_sta_authorized(void *priv, const u8 *addr, int authorized)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: atheros_set_sta_authorized: IEEE80211_IOCTL_SETMLME");
...
atheros_sta_clear_stats(void *priv, const u8 *addr)
{
	...
	memcpy(mlme.im_macaddr, addr, IEEE80211_ADDR_LEN);	
	wpa_printf(MSG_CAMEO, "CAMEO: atheros_sta_clear_stats: IEEE80211_IOCTL_SETMLME");
		   ...
		   int reason_code)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: atheros_sta_deauth: IEEE80211_IOCTL_SETMLME");
	ret = set80211priv(drv, IEEE80211_IOCTL_SETMLME, &mlme, sizeof(mlme));
	if (ret < 0) {
		     ...
		     int reason_code)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: atheros_sta_disassoc: IEEE80211_IOCTL_SETMLME");
	ret = set80211priv(drv, IEEE80211_IOCTL_SETMLME, &mlme, sizeof(mlme));
	if (ret < 0) {
...
atheros_init(struct hostapd_data *hapd, struct wpa_init_params *params)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: atheros_init");
	...
	wpa_printf(MSG_CAMEO, "CAMEO: atheros_init: linux_set_iface down");
