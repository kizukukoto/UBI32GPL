...
/*
NickChou add to control WPS LED, 20110627
Using gpio_simple_config_led_read/write in 2.6.31\kernels\mips-linux-2.6.31\arch\mips\atheros\gpio.c
*/
void wps_led_control(int led_state)
{
	switch(led_state)
	{
		case 1:
			wpa_printf(MSG_CAMEO, "CAMEO: wps_led_control: WPS LED OFF\n");
			system("echo 1  > /proc/simple_config/simple_config_led");
			break;
		case 2:
			wpa_printf(MSG_CAMEO, "CAMEO: wps_led_control: WPS LED ON\n");
			system("echo 2  > /proc/simple_config/simple_config_led");
			break;
		case 3:
			wpa_printf(MSG_CAMEO, "CAMEO: wps_led_control: WPS LED Blinking\n");
			system("echo 3  > /proc/simple_config/simple_config_led");	
			break;
		default:
			wpa_printf(MSG_CAMEO, "CAMEO: wps_led_control: Unknown\n");
	}
}
			  ...
			  int timeout)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: wps_registrar_add_pin: wps_led_control\n");
	wps_led_control(3); 
...
static void wps_registrar_pbc_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: wps_registrar_pbc_completed: wps_led_control\n");
#ifdef PLC_SUPPORT
	wps_led_control(2);
#else
	wps_led_control(1);
	...
	wpa_printf(MSG_CAMEO, "CAMEO : echo 0 to wps_result to inform gui fail");
	system("echo \"0\" > /var/tmp/wps_result");
	/* For MP to detect wps behavior */
	system("echo \"0\" > /var/tmp/mp_wps");
				...
				const u8 *p2p_dev_addr)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: wps_registrar_button_pushed: wps_led_control\n");
	wps_led_control(3);
	...
	eloop_cancel_timeout(wps_registrar_set_selected_timeout, reg, NULL);
	eloop_cancel_timeout(wps_registrar_pbc_timeout, reg, NULL);
	if (access("/tmp/wps_pbc.stop", F_OK) == 0) {
		tout = 3;	/* for stop plc syn pbc */
		unlink("/tmp/wps_pbc.stop");
	}
	eloop_register_timeout(WPS_PBC_WALK_TIME, 0, wps_registrar_pbc_timeout,
			       reg, NULL);
	
	wpa_printf(MSG_CAMEO, "CAMEO: timeout: %d MP will check this value if wps_pbc start.\n", tout);
	system("echo \"1\" > /var/tmp/mp_wps");

	/* if mp_test_start.txt exists then we can't allow client incoming during MP test,
	 * so I exit here
	 *
	 * Fred */
#ifdef PLC_SUPPORT
	if (access("/var/tmp/mp_test_start.txt", F_OK) == 0)
		exit(-1);
#endif
	return 0;
}
...
static void wps_registrar_pin_completed(struct wps_registrar *reg)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: wps_registrar_pin_completed: wps_led_control\n");
#ifdef PLC_SUPPORT
	wps_led_control(2);
#else
	wps_led_control(1);
...
int wps_build_cred(struct wps_data *wps, struct wpabuf *msg)
{
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->wps->ap &&
		...
		/*Name: CK Liau
		 *Date: 2011-08-10
		 *r[47] for base64_encode generate random 63 key length locate at util/base64.c
		 */
		u8 r[47];
		...
		/* NickChou modify for Dlink Spec, using default SSID + MAC
			Robert Chen, Follow Wi-Fi Protected Setup Spec. v1.07. 
			The format od ssid is dlinkxxxx that xxxx express the last two mac of address.
		*/
		char cameo_ssid[33];
		sprintf(cameo_ssid, "%s%02x%02x", wps->wps->ssid, wps->wps->dev.mac_addr[4], wps->wps->dev.mac_addr[5]);
		wpa_printf(MSG_CAMEO, "CAMEO: wps_build_cred: Creating Dlink SSID: %s\n", cameo_ssid);	
		os_memcpy(wps->cred.ssid, cameo_ssid, strlen(cameo_ssid));
		os_memcpy(wps->wps->ssid, cameo_ssid, strlen(cameo_ssid));
		wps->cred.ssid_len = strlen(cameo_ssid);
		wps->wps->ssid_len = strlen(cameo_ssid);
	} 
...
use_provided:
	...
	wpa_printf(MSG_CAMEO, "CAMEO: wps_build_cred: wps_build_credential\n");	
						 ...
						 const struct wpabuf *msg)
{
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->new_psk &&
		...
		wpa_printf(MSG_CAMEO, "CAMEO: wps_process_wsc_done: WPS_STATE from WPS_STATE_NOT_CONFIGURED to WPS_STATE_CONFIGURED\n");
		...
		/* Robert , create temp file to restart wifi from unconfig to config*/
		system("echo \"1\" > /var/tmp/wps_result");
		system("echo \"\" > /tmp/wps_restart_wlan");
	}
	...
	wpa_printf(MSG_CAMEO, "CAMEO: wps_process_wsc_done: wps_cb_reg_success\n");
					       ...
					       void *timeout_ctx)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: wps_registrar_set_selected_timeout: wps_led_control\n");
#ifdef PLC_SUPPORT
	wps_led_control(2);
#else
	wps_led_control(1);
