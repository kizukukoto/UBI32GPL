...
static int wps_workaround_cred_key(struct wps_credential *cred)
{
	if (cred->auth_type & (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK) &&
		 ...
		 */
		 ...
		 /*
		 NickChou, We can see this issue when Vista act as Wire ER 
		 ( cred->key[cred->key_len - 1] is NULL=0x00 )
		 */
		wpa_printf(MSG_DEBUG, "WPS: Workaround - remove NULL termination from ASCII passphrase");
		cred->key_len--;
		wpa_hexdump_ascii(MSG_CAMEO, "WPS: Network Key", cred->key, cred->key_len); 
