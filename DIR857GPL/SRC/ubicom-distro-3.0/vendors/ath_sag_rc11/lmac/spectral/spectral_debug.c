...
void reg_dump(struct ath_softc *sc)
{
    ...
    for (reg_addr=0x9800;reg_addr<0xa47d;) {
        SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL, "0x%x=0x%x\n",
                        reg_addr, 
                        OS_REG_READ(ah, reg_addr));
        reg_addr+=4;
    }
...
#define NUM_REGS 9
    ...
    SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL, "0x%x=0x%x\n",
                        reg_addr[i], 
                        OS_REG_READ(ah, reg_addr[i]));
    }
#undef NUM_REGS
}

void
printNoiseFloor(struct ath_hal *ah)
{
...
#define AR9280_PHY_CCA_THRESH62_S   12
    int16_t nf, orig_nf;
    SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL2,"\nthresh62=0x%x\n", (u_int8_t)MS(OS_REG_READ(ah, AR_PHY_CCA), AR9280_PHY_CCA_THRESH62));
    nf = MS(OS_REG_READ(ah, AR_PHY_CCA), AR9280_PHY_MINCCA_PWR);
    orig_nf=nf;
    if (nf & 0x100)
            nf = 0 - ((nf ^ 0x1ff) + 1);
    SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL2,"NF calibrated [ctl] [chain 0] is %d orig_nf=%d\n", nf, orig_nf);
...
#define AR_OBS_BUS_1 0x806c
        ...
        SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL, "0x%x=0x%x\n", AR_OBS_BUS_1, OS_REG_READ(sc->sc_ah, AR_OBS_BUS_1));
        SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL, "0x%x=0x%x\n", 0x0008, OS_REG_READ(sc->sc_ah, 0x0008));
        ...
        SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL2,"this scan phy err = %d\n", (int)(sc->sc_phy_stats[sc->sc_curmode].ast_rx_phyerr - sc->sc_spectral->cached_phy_err));
        ...
        if (sc->sc_spectral->num_spectral_data < spectral->params.ss_count || spectral->params.ss_count == 128) {
            ...
            sc->sc_spectral->cached_phy_err = sc->sc_phy_stats[sc->sc_curmode].ast_rx_phyerr;
