...
ath_rx_init(ath_dev_t dev, int nbufs)
{
    ...
    do {
        ...
        TAILQ_FOREACH(bf, &sc->sc_rxbuf, bf_list) {
            wbuf = ATH_ALLOCATE_RXBUFFER(sc, sc->sc_rxbufsize);
            if (wbuf == NULL) {
                error = -ENOMEM;
                break;
            }
            ...
            bf->bf_mpdu = wbuf;
            bf->bf_buf_addr[0] = wbuf_map_single(sc->sc_osdev, wbuf, BUS_DMA_UBICOM32_FROMDEVICE,
...
ath_allocRxbufsForFreeList(struct ath_softc *sc)
{
    ...
    while ((bf = sc->sc_rxfreebuf) != NULL) {
        if ((new_wbuf = ATH_ALLOCATE_RXBUFFER(sc, sc->sc_rxbufsize)) != NULL) {
            ...
            bf->bf_buf_addr[0] = wbuf_map_single(sc->sc_osdev, new_wbuf, BUS_DMA_UBICOM32_FROMDEVICE,
                                              OS_GET_DMA_MEM_CONTEXT(bf, bf_dmacontext));
            if (bf->bf_buf_addr[0]) {
                bf->bf_mpdu = new_wbuf;
        ...
        ieee80211_rx_status_t *rx_status, u_int8_t *chainreset)
{
    ...
    OS_UNMAP_SINGLE(sc->sc_osdev, bf->bf_buf_addr[0],
                    sc->sc_rxbufsize, BUS_DMA_UBICOM32_FROMDEVICE,
                    OS_GET_DMA_MEM_CONTEXT(bf, bf_dmacontext));
#endif
        ...
        if (0 == rxs.rs_status) {
            ...
            if ((frm->i_fc[1] & IEEE80211_FC1_DIR_MASK) ==
                ...
                if ((frm->i_fc[1] & IEEE80211_FC1_DIR_MASK) ==
                    ...
                    ATH_ADDR_COPY(sc->sc_ah->ah_rxLastAggrBssid, frm->i_addr2);
                } else {
                    ATH_ADDR_COPY(sc->sc_ah->ah_rxLastAggrBssid, frm->i_addr1);
                }
            }
        }
#endif
    ...
    for (bf = bf_first; bf; bf = TAILQ_NEXT(bf, bf_list)) {
        ...
        if (rxs.rs_status & ~HAL_RXERR_KEYMISS) {
            ...
            if ((wh->i_fc[1] & IEEE80211_FC1_DIR_MASK) ==
                ...
                if ((wh->i_fc[1] & IEEE80211_FC1_DIR_MASK) ==
                    IEEE80211_FC1_DIR_FROMDS) {
                    ATH_ADDR_COPY(sc->sc_ah->ah_rxLastAggrBssid, wh->i_addr2);
                } else {
                    ATH_ADDR_COPY(sc->sc_ah->ah_rxLastAggrBssid,wh->i_addr1);
                }
            }
#endif
        }
        ...
        if (!(bf->bf_status & ATH_BUFSTATUS_SYNCED)) {
            OS_SYNC_SINGLE(sc->sc_osdev,
                       bf->bf_buf_addr[0], wbuf_get_len(wbuf), BUS_DMA_UBICOM32_FROMDEVICE,
                       OS_GET_DMA_MEM_CONTEXT(bf, bf_dmacontext));
            bf->bf_status |= ATH_BUFSTATUS_SYNCED;
        }
#endif
