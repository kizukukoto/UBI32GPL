...
ath_alloc_skb(u_int size, u_int extra_reserve, u_int align)
{
    struct sk_buff *skb;
    u_int off;
    ...
    if (skb != NULL) {
        /*
         * __LINUX_UBICOM32_ARCH__: cache line size value is power-of-2
         * So use and instead of modulo operation.
         */
        off = (align - ((unsigned long) skb->data)) & (align - 1);
        skb_reserve(skb, off + extra_reserve);
    }
    return skb;
}
...
ath_rxbuf_alloc(struct ath_softc *sc, u_int32_t len)
{
    ...
    if (sc->sc_opmode == HAL_M_MONITOR) {
        ...
        /*
         * Align to cache line. (__LINUX_UBICOM32_ARCH__: cache line size value is power-of-2)
         */
         ...
         * in rx'd frames.
         */
        skb = ath_alloc_skb(len + sc->sc_rxreserve, sc->sc_rxreserve, 64/*sc->sc_cachelsz*/); // Ubicom IP8K PCIe work-around
        if (skb == NULL) {
            DPRINTF(sc, ATH_DEBUG_ANY,
...
static int wbuf_start_dma(wbuf_t wbuf, sg_t *sg, u_int32_t n_sg, void *arg)
{
...
ath_rxbuf_alloc(struct ath_softc *sc, u_int32_t len)
{
    ...
    if (sc->sc_opmode == HAL_M_MONITOR) {
         ...
         * in rx'd frames.
         */
        nbf = adf_nbuf_alloc(NULL, len + sc->sc_rxreserve, sc->sc_rxreserve, 64/*sc->sc_cachelsz*/); // Ubicom IP8K PCIe work-around
        if (nbf == NULL) {
            DPRINTF(sc, ATH_DEBUG_ANY,
...
static int wbuf_start_dma(adf_nbuf_t nbf, sg_t *sg, u_int32_t n_sg, void *arg)
{
