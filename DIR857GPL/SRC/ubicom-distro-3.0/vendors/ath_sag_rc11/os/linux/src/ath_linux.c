...
#if ATH_THREAD
static int cpuid = 0;
static int ath_txtread_enqueue(struct ath_softc_net80211 *scn, struct sk_buff *skb);
#endif
...
#if ATH_THREAD
    struct ath_softc_net80211 *scn = ath_netdev_priv(dev);
    return ath_txtread_enqueue(scn, skb);
#else
...
#if ATH_THREAD
static void
ath_txthread_process_txq(struct ath_softc_net80211 *scn)
{
       osdev_t osdev = scn->sc_osdev;
       struct net_device *dev =  osdev->netdev;
       struct sk_buff *skb;

       while ((skb = skb_dequeue(&scn->ath_thread_tx_queue)) != NULL) {
               if(lmac_run_aponly(skb, dev))
                       ath_netdev_hardstart_aponly(skb,dev);
               else
                       ath_netdev_hardstart_generic(skb, dev);
       }
}

static int
ath_tx_thread_fn(void *data)
{
       ...
       while (1) {
               /* Call dpc for resched or schedule from interrupt */
               wait_event_interruptible_timeout(scn->ath_tx_wqh,
                               skb_queue_len(&scn->ath_thread_tx_queue),
                               1);

               if (kthread_should_stop())
                       break;

               ath_txthread_process_txq(scn);
       }

       skb_queue_purge(&scn->ath_thread_tx_queue);
...
static int
ath_txtread_enqueue(struct ath_softc_net80211 *scn, struct sk_buff *skb)
{
       skb_queue_tail(&scn->ath_thread_tx_queue, skb);
       wake_up_interruptible(&scn->ath_tx_wqh);
...
#if ATH_THREAD
    init_waitqueue_head(&scn->ath_tx_wqh);
    skb_queue_head_init(&scn->ath_thread_tx_queue);

    scn->ath_tx_thread = kthread_create(ath_tx_thread_fn, scn, "ath_tx_thread");
    if (IS_ERR(scn->ath_tx_thread)) {
       printk(KERN_ERR "%s: WARNING failed to create ath_tx_thread\n", __FUNCTION__);
       goto bad4;
    }
    cpuid = cpumask_next(cpuid, cpu_online_mask);
    if (cpuid >= nr_cpu_ids) {
       cpuid = cpumask_first(cpu_online_mask);
    }
    kthread_bind(scn->ath_tx_thread, cpuid);
    wake_up_process(scn->ath_tx_thread);
#endif
...
#if ATH_THREAD
    if (scn->ath_tx_thread) kthread_stop(scn->ath_tx_thread);
    scn->ath_tx_thread = NULL;
#endif
