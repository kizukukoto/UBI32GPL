...
/* UBICOM_PORT */
static int
ieee80211_ioctl_siwpriv_getmacaddr(struct net_device *dev,
    struct iw_request_info *info,
    struct iw_point *data, char *extra)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
...
#define MAX_WX_STRING  80 /* UBICOM_PORT */
...
static int
ieee80211_ioctl_siwpriv(struct net_device *dev,
    struct iw_request_info *info,
    struct iw_point *data, char *extra)
{
    debug_print_ioctl(dev->name, SIOCSIWPRIV, "siwpriv") ;

    if (data->length && extra) {
        if (strnicmp(extra, "START", strlen("START")) == 0) {
            printk("%s: Received START\n", __func__);
        }
        if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
            printk("%s: Received SCAN-ACTIVE\n", __func__);
        } else
        if (strnicmp(extra, "SCAN-PASSIVE", strlen("SCAN-PASSIVE")) == 0) {
            printk("%s: Received SCAN-PASSIVE\n", __func__);
        } else
        if (strnicmp(extra, "RSSI", strlen("RSSI")) == 0) {
            printk("%s: Received RSSI\n", __func__);
        } else
        if (strnicmp(extra, "LINKSPEED", strlen("LINKSPEED")) == 0) {
            printk("%s: Received LINKSPEED\n", __func__);
        } else
        if (strnicmp(extra, "MACADDR", strlen("MACADDR")) == 0) {
            printk("%s: Received MACADDR\n", __func__);
            ieee80211_ioctl_siwpriv_getmacaddr(dev, info, data, extra);
        } else
        if (strnicmp(extra, "COUNTRY", strlen("COUNTRY")) == 0) {
            printk("%s: Received COUNTRY\n", __func__);
        } else
        if (strnicmp(extra, "STOP", strlen("STOP")) == 0) {
            printk("%s: Received STOP\n", __func__);
        } else
        if (strnicmp(extra, "POWERMODE", strlen("POWERMODE")) == 0) {
            printk("%s: Received POWERMODE\n", __func__);
        } else
        {
            printk("%s: Sending OK: Unknown cmd %s\n", __func__, extra);
            snprintf(extra, data->length, "OK");
            data->length = strlen("OK") + 1;
		       ...
		       union iwreq_data *wrqu, char *extra)
{
	...
	if(wrqu->data.length > 0) {
		...
		for(i = 0; i < wrqu->data.length; i++)
			IEEE80211_ADDR_COPY(spydata->spy_address[i], address[i].sa_data);
		/* Reset stats */
		       ...
		       union iwreq_data *wrqu, char *extra)
{
	...
	for(i = 0; i < spydata->spy_number; i++) 	{
		IEEE80211_ADDR_COPY(address[i].sa_data, spydata->spy_address[i]);
        ...
        u_int8_t *src_addr, u_int8_t *bssid, ieee80211_xmit_status *ts)
{
    ...
    IEEE80211_ADDR_COPY(act_cb->dst_addr, dst_addr);
    IEEE80211_ADDR_COPY(act_cb->src_addr, src_addr);
    IEEE80211_ADDR_COPY(act_cb->bssid, bssid);
...
get_sta_info(void *arg, wlan_node_t node)
{
    ...
    //NickChou
    si->isi_nrates = wlan_node_get_nrates(node);
    ...
    struct iw_request_info *info, struct iw_point *erq, char *data)
{
    ...
    IEEE80211_ADDR_COPY(mlme.im_macaddr, wextmlme->addr.sa_data);
    ...
    struct iw_request_info *info, struct iw_point *erq, char *extra)
{
    ...
    if (ext->alg == IW_ENCODE_ALG_NONE) {
        ...
        dk.idk_keyix = kid;
        IEEE80211_ADDR_COPY(&dk.idk_macaddr, ext->addr.sa_data);
    ...
    * can be implemented in terms of ieee80211_ioctl_setkey */
    if (ext->alg == IW_ENCODE_ALG_WEP &&
        IEEE80211_ADDR_EQ(ext->addr.sa_data, "\xff\xff\xff\xff\xff\xff")) {
    ...
    if (ext->ext_flags & IW_ENCODE_EXT_SET_TX_KEY) {
        ...
        IEEE80211_ADDR_COPY(kr.ik_macaddr, ext->addr.sa_data);
    }
...
static const iw_handler ieee80211_handlers[] = {
    ...
    (iw_handler) ieee80211_ioctl_siwpriv,   /* SIOCSIWPRIV */  /* UBICOM_PORT */
