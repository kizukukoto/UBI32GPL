...
#define LIST_STATION_ALLOC_SIZE 24*1024
	...
	(void) memset(&a_time, '\0', sizeof(a_time));
	...
	printf("%-17.17s %4s %4s %4s %4s %4s %6s %6s %4s %5s %3s %8s %6s %8s %s\n"
		, "ADDR"
		, "AID"
		, "CHAN"
		, "RATE"
		...
		, "STATE"
	        , "HTCAPS"
               , "A_TIME"
               , "NEGO_RATES" /* NickChou add 07.8.15 for STA using 11g or 11b ... */
	);
	cp = buf;
	do {
		...
		printf("%s %4u %4d %3dM %4d %4d %6d %6d %-4.4s %-5.5s %3x %8x %-6.6s %-8.8s %-10d"
			...
			, a_time
			, si->isi_nrates /*12=>11g, 11n, 4=>11b*/
		);
