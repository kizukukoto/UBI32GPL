...
DeletePortMappingRange(struct upnphttp * h, const char * action)
{
	...
	const char * start_ext_port, * end_ext_port, * protocol, * manage;
	unsigned short seport, eeport;

	ParseNameValue(h->req_buf + h->req_contentoff, h->req_contentlen, &data);
	start_ext_port = GetValueFromNameValueList(&data, "NewStartPort");
	end_ext_port = GetValueFromNameValueList(&data, "NewEndPort");
	protocol = GetValueFromNameValueList(&data, "NewProtocol");
	manage = GetValueFromNameValueList(&data, "NewManage");

	seport = (unsigned short)atoi(start_ext_port);
	eeport = (unsigned short)atoi(end_ext_port);

	if(!start_ext_port || !end_ext_port || !protocol)
	{
	...
	if (seport < 1024 || eeport < 1024)
	{
		syslog(LOG_INFO, "Client tried to access pinhole with port < 1024 and is not authorized to do it");
		SoapError(h, 606, "Action not authorized");
		return;
	}

	syslog(LOG_INFO, "%s: external port, start: %hu - end: %hu, protocol: %s",
		action, seport, eeport, protocol);

	r = -1; // TODO function needs to be implemented
...
GetListOfPortMappings(struct upnphttp * h, const char * action)
{
	...
	static const char resp[] =
		"<u:DeletePortMappingRangeResponse "
		"xmlns:u=\"urn:schemas-upnp-org:service:WANIPConnection:1\">"
		"<NewPortListing>%s</NewPortListing>"
		"</u:DeletePortMappingRangeResponse>";

	struct NameValueParserData data;
	const char * start_ext_port, * end_ext_port, * protocol, * manage, * number_entries;
	unsigned short seport, eeport;
	char port_listing[64];
	...
	protocol = GetValueFromNameValueList(&data, "NewProtocol");
	manage = GetValueFromNameValueList(&data, "NewManage");
	number_entries = GetValueFromNameValueList(&data, "NewNumberOfPorts");
...
GetExternalIPAddress(struct upnphttp * h, const char * action)
{
	...
	if(use_ext_ip_addr)
	{
		strncpy(ext_ip_addr, use_ext_ip_addr, INET6_ADDRSTRLEN);//IPv6 Modification
	}
	else if(getifaddr(ext_if_name, ext_ip_addr, INET6_ADDRSTRLEN) < 0)//IPv6 Modification
	{
		syslog(LOG_ERR, "Failed to get ip address for interface %s",
			ext_if_name);
		strncpy(ext_ip_addr, "::", INET6_ADDRSTRLEN);//IPv6 Modification
	}
#else
	int i, j;
	strncpy(ext_ip_addr, "::", INET6_ADDRSTRLEN);//IPv6 Modification
	for(i = 0; i<n_lan_addr; i++)
	{
		...
		{
			if( (h->clientaddr.s6_addr[j] & lan_addr[i].mask.s6_addr[j])
		   	== (lan_addr[i].addr.s6_addr[j] & lan_addr[i].mask.s6_addr[j]))// IPv6 Modification
			{
				strncpy(ext_ip_addr, lan_addr[i].ext_ip_str, INET6_ADDRSTRLEN);//IPv6 Modification
				break;
			}
		}
	}
...
AddPortMapping(struct upnphttp * h, const char * action)
{
	...
	char formatedAddr[INET6_ADDRSTRLEN]; //IPv6 Modification
			...
			for(p = ai; p; p = p->ai_next)//ptr = hp->h_addr_list; ptr && *ptr; ptr++)
		   	{
				...
				fprintf(stderr, "upnpsoap / AddPortMapping: assuming int addr = %s", int_ip);
	...
	if(GETFLAG(SECUREMODEMASK))
	{
		...
		{
			inet_ntop(AF_INET6, &(h->clientaddr), formatedAddr, sizeof(struct in6_addr)); //IPv6 Modification
			syslog(LOG_INFO, "Client %s tried to redirect port to %s",
			       formatedAddr, int_ip); //IPv6 Modification
			ClearNameValueList(&data);
	...
	syslog(LOG_INFO, "%s: ext port %hu to [%s]:%hu protocol %s for: %s", //IPv6 Modification
...
GetSpecificPortMappingEntry(struct upnphttp * h, const char * action)
{
	...
	char int_ip[40]; // IPv6 Modification
	char desc[64];

	ParseNameValue(h->req_buf + h->req_contentoff, h->req_contentlen, &data);
	//DisplayNameValueList(h->req_buf + h->req_contentoff, h->req_contentlen);
	...
	{
		syslog(LOG_INFO, "%s: rhost='%s' %s %s found => [%s]:%u desc='%s'", //IPv6 Modification
		       action,
...
GetGenericPortMappingEntry(struct upnphttp * h, const char * action)
{
	...
	unsigned short eport, iport;
	const char * m_index;
	char protocol[4], iaddr[40]; // IPv6 Modification
	char desc[64];
	struct NameValueParserData data;
...
GetFirewallStatus(struct upnphttp * h, const char * action)
{
	...
	bodylen = snprintf(body, sizeof(body), resp,
		...
		firewallEnabled, inboundPinholeAllowed, action);
	BuildSendAndCloseSoapResp(h, body, bodylen);
}
...
DataVerification(struct upnphttp * h, char * int_ip, unsigned short * int_port, const char * protocol, char * leaseTime)
{
	...
	else if (atoi(protocol) != IPPROTO_UDP && atoi(protocol) != IPPROTO_TCP) //&& atoi(protocol) != IPPROTO_UDPLITE)
	{
...
soapMethods[] =
{
	...
	{ "AddAnyPortMapping", AddAnyPortMapping}, // Added for certification purpose
	{ "DeletePortMapping", DeletePortMapping},
	{ "DeletePortMappingRange", DeletePortMappingRange}, // Added for certification purpose
	{ "ForceTermination", ForceTermination},  // Added for certification purpose
	...
	{ "GetListOfPortMappings", GetListOfPortMappings}, // Added for certification purpose
	{ "GetStatusInfo", GetStatusInfo},
	{ "RequestConnection", RequestConnection}, // Added for certification purpose
	{ "SetConnectionType", SetConnectionType}, // Added for certification purpose
...
ExecuteSoapAction(struct upnphttp * h, const char * action, int n)
{
	...
	if(p)
	{
		...
		while(soapMethods[i].methodName)
		{
			...
			if(strncmp(p, soapMethods[i].methodName, methodlen) == 0)
			{
...
SoapError(struct upnphttp * h, int errCode, const char * errDesc)
{
	...
	BuildResp2_upnphttp(h, 500, "Internal Server Error", body, bodylen, 6);
	SendResp_upnphttp(h);
	CloseSocket_upnphttp(h);
}
