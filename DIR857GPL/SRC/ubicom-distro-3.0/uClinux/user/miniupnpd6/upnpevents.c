...
struct upnp_event_notify {
	...
	int sent;
	const char * path;
	char addrstr[40]; // IPv6 Modification
	char portstr[8];
};
...
/* Subscriber list */
LIST_HEAD(listhead, subscriber) subscriberlist = LIST_HEAD_INITIALIZER(subscriberlist);
...
newSubscriber(const char * eventurl, const char * callback, int callbacklen)
{
	...
	{
		printf("newSubscriber null argument error\n");	
		return NULL;
	}
	...
	{
		printf("\tEventing request not supported\n");
		free(tmp);
		return NULL;
	}
                         ...
                         int timeout)
{
	...
	if(LIST_EMPTY(&subscriberlist))
		printf("addSubscriber: the subscriberlist is empty.\n");
...
{
	struct subscriber * sub;
	printf("\tStarting Renewing subscription.\n");
	if(!LIST_EMPTY(&subscriberlist))
	{
	...
	{
		printf("\tThe subscriberlist is empty.\n");
	}
	printf("\tTrying the for loop.\n");
...
upnpevents_removeSubscriber(const char * sid, int sidlen)
{
	...
	if(!sid)
	{
		printf("\tremoverSubscriber: Not valid SID");
		return -1;
	}
	if(LIST_EMPTY(&subscriberlist))
		printf("\tremoveSubscriber: the subscriberlist is empty.\n");
	for(sub = subscriberlist.lh_first; sub != NULL; sub = sub->entries.le_next)
	{
		printf("\t\t Current SID: %s\n", sub->uuid);
		printf("\t\t  Compare to: %s\n", sid);
...
upnp_event_create_notify(struct subscriber * sub)
{
	...
	obj->sub = sub;
	obj->state = ECreated;
	obj->s = socket(PF_INET6, SOCK_STREAM, 0); // IPv6 Modification
	if(obj->s<0)
	{
...
error:
	if(obj->s >= 0)
	{
		printf("Error fcntl, closing socket.\n");
		close(obj->s);
	}
	free(obj);
}
...
upnp_event_notify_connect(struct upnp_event_notify * obj)
{
	...
	const char * p;
	unsigned short port;
	struct sockaddr_in6 addr; // IPv6 Modification
	if(!obj)
		return;
	...
	p = obj->sub->callback;
	p += 8;	/* http://[ */  // IPv6 Modification
	while(*p != '/' && *p != ']')  // IPv6 Modification
		obj->addrstr[i++] = *(p++);
	obj->addrstr[i] = '\0';
	if(*p == ']')  // IPv6 Modification
	{
		p+=1;  // IPv6 Add
		obj->portstr[0] = *p;
		i = 1;
	...
	obj->path = p;
	addr.sin6_family = AF_INET6;  // IPv6 Modification
	inet_pton(AF_INET6, obj->addrstr, &(addr.sin6_addr)); // IPv6 Modification
	addr.sin6_port = htons(port); // IPv6 Modification
	if(IN6_IS_ADDR_LINKLOCAL(&(addr.sin6_addr)))
	{
		printf("\tlinklocal mode with addr: %s\n", obj->addrstr);
		addr.sin6_scope_id = linklocal_index;
	}
	syslog(LOG_DEBUG, "[%s]: '%s' %hu '%s'", "upnp_event_notify_connect", // IPv6 Modification
...
{
	int i;
	syslog(LOG_DEBUG, "%s: sending event notify message to [%s]%s", // IPv6 Modification
