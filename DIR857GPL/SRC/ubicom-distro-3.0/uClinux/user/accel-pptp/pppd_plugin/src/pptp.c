...
static int pptp_fd;
//int call_ID; /* Ubicom: Modified 20091113*/
...
//	call_ID=src_addr.sa_addr.pptp.call_id; /* Ubicom: Modified 20091113*/
...
static int open_callmgr(int call_id,struct in_addr inetaddr, char *phonenr,int window)
{
    ...
    /* Make address */
    callmgr_name_unixsock(&where, inetaddr, localbind, call_id);
    for (i = 0; i < NUM_TRIES; i++)
    {
        if (connect(fd, (struct sockaddr *) &where, sizeof(where)) < 0)
        {
            /* couldn't connect.  We'll have to launch this guy. */
/* Ubicom: Modified 20091113 To port the packet for uClinux. These lines are moved from launch_callmgr() function*/
		...
		/* fork and launch call manager process */
			...
			// close(pty_fd);
			//close(gre_fd);
			execle("/bin/pptp_callmgr", "pptp_callmgr", inet_ntoa(inetaddr), "--call_id",call,"--window",win,NULL, environ);
			fatal("execle() to launch call manager failed.");
			_exit(1);
                }
                default: /* parent */
