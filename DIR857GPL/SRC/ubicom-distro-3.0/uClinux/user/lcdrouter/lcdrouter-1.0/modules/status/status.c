...
static 
void prev_menu (struct menu_instance *mi, void *app_data)
{
...
void M_(app_instance_draw_static_components)(struct M_(app_instance) *app)
{
	...
	if ( res != DFB_OK ) {
		DEBUG_ERROR("menu_instance_alloc failed");
	}
	...
	menu_instance_set_menu_data(app->mi, (struct menu_data_instance*)populate_and_create_status());
	menu_instance_set_on_prev_menu(app->mi, prev_menu, app);
...
{
	APP_VERIFY_MAGIC(app);
	header_instance_set_text(app->hi, "Status");
