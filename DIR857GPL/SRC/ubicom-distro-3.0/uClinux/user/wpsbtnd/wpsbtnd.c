...
int invoke_soft_wps(int wps_count)
{
...
//	}
	...
	if (fork() == 0) {
#endif
		printf("Launching hostapd_cli... \n");

                if(wps_count==0)
                {
		        const char *argv[5] = { "hostapd_cli", "-i", "ath0", "wps_pbc", NULL };
		        execve("/sbin/hostapd_cli", argv, NULL);
		}
		else
		{	
		        const char *argv1[5] = { "hostapd_cli", "-i", "ath1", "wps_pbc", NULL };
		        execve("/sbin/hostapd_cli", argv1, NULL);
		}	
#if defined(__uClinux__) && !defined(IP8000)
		_exit(0);
#else
	...
	if (fork() == 0) {
		...
		printf("Launching wpatalk... \n");
                ...
                if(wps_count==0)
                {
		        const char *argv[4] = { "wpatalk", "ath0", "configthem", NULL };
		        execve("/sbin/wpatalk", argv, NULL);	
		}
		else
		{	
		        const char *argv1[4] = { "wpatalk", "ath1", "configthem", NULL };
		        execve("/sbin/wpatalk", argv1, NULL);	
		}	
#if defined(__uClinux__) && !defined(IP8000)
		_exit(0);
#else
...
int invoke_soft_wps_pin(int wps_count)
{
	/* launch wpatalk first time */
//	if (wps_count == 1) {
//#if defined(__uClinux__) && !defined(IP8000) 
...
//	}
	...
	if (fork() == 0) {
		...
		printf("Launching hostapd_cli... \n");
                ...
                if(wps_count==0)
                {
		        const char *argv[6] = { "hostapd_cli", "-i", "ath0", "wps_pin", "any", "29679126", NULL };
		        execve("/sbin/hostapd_cli", argv, NULL);	
		}
		else
		{	
		        const char *argv1[6] = { "hostapd_cli", "-i", "ath1", "wps_pin", "any", "29679126", NULL };
		        execve("/sbin/hostapd_cli", argv1, NULL);	
		}	
#if defined(__uClinux__) && !defined(IP8000)
		_exit(0);
	...
	if (fork() == 0) {
		...
		printf("Launching wpatalk... \n");
                ...
                if(wps_count==0)
                {
		        const char *argv[5] = { "wpatalk", "ath0", "configme", "pin=29679126", NULL };
		        execve("/sbin/wpatalk", argv, NULL);	
		}
		else
		{	
		        const char *argv1[5] = { "wpatalk", "ath1", "configme", "pin=29679126", NULL };
		        execve("/sbin/wpatalk", argv1, NULL);	
		}	
#if defined(__uClinux__) && !defined(IP8000)
		_exit(0);
  ...
  while (1) 
    {
      ...
      for (i = 0; i < rd / sizeof(struct input_event); i++)
	    ...
	    //	    if(ev[i].code == 59 && ev[i].value == 1) {
	    if(ev[i].code == 466 && ev[i].value == 1) {
	      printf("WPS BUTTON PRESSED!\n");
	      wpsbtn_runwps = 1;
	      wps_utton_wipnp_time = ev[i].time.tv_sec;
		  system("echo 1 > /var/tmp/wps_tmp");
	    }
	    else if(ev[i].type == 1 && ev[i].code == 466 && ev[i].value == 0 && wpsbtn_runwps == 1) {
		printf("WPS BUTTON RESET!\n");
		wpsbtn_runwps = 0;
      		fp = fopen("/var/tmp/WPSNoNo","r");
	      	k = 0;
	      	if(fp != NULL)
	      	{
			fscanf(fp, "%d", &k);
			fclose(fp);
	      	...
	      	if(k != 1)
	      	{
	      		if((ev[i].time.tv_sec - wps_utton_wipnp_time) < 5)
	      		{
	      			fp = fopen("/var/tmp/wpstalk_tmp","r");
	      			k = 0;
	      			if(fp != NULL)
	      			{
					fscanf(fp, "%d", &k);
					fclose(fp);
	      			...
	      			if(1) //(k != 1)
	      			{
	      				printf("WPS BUTTON runs WPS talk!\n");
		      			system("echo 1 > /var/tmp/wpstalk_tmp");
	      				invoke_soft_wps(0);
#ifdef ATH_AR938X
	      				invoke_soft_wps(1);
#endif
	      		...
	      		{
	      			printf("WPS BUTTON runs WiPNP!\n");
				system("/sbin/gpio POWER_LED s_blink&");
		      		system("nvram WiPNP&");
	      		}
	      	}
	      	else
...
//	    if(ev[i].code == 139 && ev[i].value == 32) {
//	      printf("WPS BUTTON (PIN) PRESSED!\n");
//	      invoke_soft_wps_pin(++wps_count);
//	    }
