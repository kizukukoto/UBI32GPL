...
char *get_nextdir(DIR *dir)
{
	int ret;
	int idx = 0;
	char *linebuf = NULL, *p;
	int linebufsz = 256;
	char buff[sizeof(struct dirent) + 256];
	struct dirent *dent = (struct dirent *)buff, *result;
	struct stat st;

#if _POSIX_C_SOURCE >= 1 || _XOPEN_SOURCE || _BSD_SOURCE || _SVID_SOURCE || _POSIX_SOURCE 
	if (readdir_r(dir, dent, &result)) return NULL;
	...
	if (stat(result->d_name, &st)) return -1;

	linebuf = xrealloc(linebuf, linebufsz);

	p = linebuf;
	if (S_ISDIR(st.st_mode))  *p++ = 'd';
	else                      *p++ = '-';

	if (S_IRUSR & st.st_mode) *p++ = 'r';
	else                      *p++ = '-';
	if (S_IWUSR & st.st_mode) *p++ = 'w';
	else                      *p++ = '-';
	if (S_IXUSR & st.st_mode) *p++ = 'x';
	else                      *p++ = '-';

	if (S_IRGRP & st.st_mode) *p++ = 'r';
	else                      *p++ = '-';
	if (S_IWGRP & st.st_mode) *p++ = 'w';
	else                      *p++ = '-';
	if (S_IXGRP & st.st_mode) *p++ = 'x';
	else                      *p++ = '-';

	if (S_IROTH & st.st_mode) *p++ = 'r';
	else                      *p++ = '-';
	if (S_IWOTH & st.st_mode) *p++ = 'w';
	else                      *p++ = '-';
	if (S_IXOTH & st.st_mode) *p++ = 'x';
	else                      *p++ = '-';

	*p++ = ' ';

	ret = snprintf(p, 16, "%d ", st.st_nlink);
	if (ret < 0) {*p++ = '1'; *p++ = ' ';}
	else p += ret;

	ret = snprintf(p, 16, "%d ", st.st_uid);
	if (ret < 0) {*p++ = '0'; *p++ = ' ';}
	else p += ret;

	ret = snprintf(p, 16, "%d ", st.st_gid);
	if (ret < 0) {*p++ = '0'; *p++ = ' ';}
	else p += ret;

	ret = snprintf(p, 16, "%lld ", (long long)st.st_size);
	if (ret < 0) {*p++ = '0'; *p++ = ' ';}
	else p += ret;

	strncpy(p, "JAN 1 1970 ", 11);
	p += 11;

	for (; result->d_name[idx] != '\0'; idx++) {
		/* grow the line buffer as necessary */
		if (p == &linebuf[linebufsz]) {
			int off;
			off = p - linebuf;
			linebufsz += 256;
			linebuf = xrealloc(linebuf, linebufsz);
	...
	if (linebuf) {
		int off;
		off = p - linebuf;
		linebuf = xrealloc(linebuf, (p - linebuf) + 1);
		linebuf[off] = '\0';
	}
	return linebuf;
}
