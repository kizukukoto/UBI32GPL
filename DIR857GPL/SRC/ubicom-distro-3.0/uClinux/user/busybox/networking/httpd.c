...
#endif
#if ENABLE_FEATURE_HTTPD_MAX_PRIORITY
#include <sched.h>
#endif
//#define DEBUG 1
#define DEBUG 0
...
#define DEBUG_MSG(fmt, args...)	cprintf("XXX %s(%d) "fmt"\n", __FUNCTION__, __LINE__, ## args)
#endif

static const char default_path_httpd_conf[] ALIGN1 = "/etc";
static const char httpd_conf[] ALIGN1 = "httpd.conf";
/* FredHung 2010/7/1, let browser don't cache anything */
...
#if ENABLE_FEATURE_HTTPD_SSL
#define do_ssl		  (G.do_ssl           )
#define cipher		  (G.cipher	      )
#define cert		  (G.cert	      )
#define ssl_ctx		  (G.ssl_ctx	      )
#define ssl		  (G.ssl	      )
#endif
#if ENABLE_FEATURE_HTTPD_RANGES
...
#define INDEX_PAGE	CONFIG_FEATURE_HTTPD_GRAPHICAL_AUTH_INDEX
extern char *httpd_auth_session_timeout(const char *, char *, char *);
#else
#define INDEX_PAGE	"index.asp"
#endif
...
#if ENABLE_FEATURE_HTTPD_SSL
static ssize_t ssl_safe_read(int fd, void *buf, size_t count)
{
	ssize_t n;

	do {
		n = SSL_read(ssl, buf, count);
	} while (n < 0 && errno == EINTR);

	return n;
}

static ssize_t ssl_safe_write(int fd, const void *buf, size_t count)
{
	ssize_t n;

	do {
		n = SSL_write(ssl, buf, count);
	} while (n < 0 && errno == EINTR);
...
static ssize_t safe_write_hook(int fd, const void *buf, size_t count)
{
#if ENABLE_FEATURE_HTTPD_DEBUG
	DEBUG_MSG();
#endif
#if ENABLE_FEATURE_HTTPD_SSL
	if (do_ssl == 1 && ssl != NULL)
		return ssl_safe_write(fd, buf, count);
	else
...
static ssize_t full_write_hook(int fd, const void *buf, size_t len)
{
	ssize_t cc;
	ssize_t total;
...
static ssize_t safe_read_hook(int fd, void *buf, size_t count)
{
#if ENABLE_FEATURE_HTTPD_SSL
	if (do_ssl == 1 && ssl != NULL)
		return ssl_safe_read(fd, buf, count);
	else
...
static void send_headers(int responseNum)
{
	...
	if (error_page && access(error_page, R_OK) == 0) {
		...
		full_write_hook(STDOUT_FILENO, iobuf, len);
		if (DEBUG)
	...
	if (full_write_hook(STDOUT_FILENO, iobuf, len) != len) {
		if (verbose > 1)
			bb_perror_msg("error");
		log_and_exit();
	}
...
static int get_line(void)
{
	...
	while (1) {
		if (hdr_cnt <= 0) {
			hdr_cnt = safe_read_hook(STDIN_FILENO, hdr_buf, sizeof(hdr_buf));
			if (hdr_cnt <= 0)
				break;
			hdr_ptr = hdr_buf;
		}
...
static NOINLINE void cgi_io_loop_and_exit(int fromCgi_rd, int toCgi_wr, int post_len)
{
...
#if ENABLE_FEATURE_HTTPD_DEBUG
	DEBUG_MSG("fromCgi_rd: '%d' toCgi_wr: '%d' post_len: '%d'",
		fromCgi_rd,
		toCgi_wr,
		post_len);
#endif
	...
	while (1) {
...
#if ENABLE_FEATURE_HTTPD_DEBUG
			DEBUG_MSG("toCgi_wr: '%d' hdr_ptr: '%s', hdr_cnt: '%d'",
				toCgi_wr,
				hdr_ptr,
				hdr_cnt);
#endif
			//count = safe_write_hook(toCgi_wr, hdr_ptr, hdr_cnt);
		...
		if (pfd[0].revents) {
			...
			count = safe_read_hook(STDIN_FILENO, hdr_buf, sizeof(hdr_buf));
			if (count > 0) {
				hdr_cnt = count;
				hdr_ptr = hdr_buf;
				post_len -= count;
		...
		if (pfd[FROM_CGI].revents) {
			...
			if (out_cnt >= 0) {
				...
				//count = safe_read_hook(fromCgi_rd, rbuf + out_cnt, PIPE_BUF - 8);
				count = safe_read(fromCgi_rd, rbuf + out_cnt, PIPE_BUF - 8);
				if (count <= 0) {
					/* eof (or error) and there was no "HTTP",
					 * so write it, then write received data */
					if (out_cnt) {
						full_write_hook(STDOUT_FILENO, HTTP_200, sizeof(HTTP_200)-1);
						full_write_hook(STDOUT_FILENO, rbuf, out_cnt);
					}
				...
				if (out_cnt >= 8 && memcmp(rbuf, "Status: ", 8) == 0) {
					/* send "HTTP/1.0 " */
					if (full_write_hook(STDOUT_FILENO, HTTP_200, 9) != 9)
						break;
					rbuf += 8; /* skip "Status: " */
					count = out_cnt - 8;
					...
					if (memcmp(rbuf, HTTP_200, 4) != 0) {
						...
						if (full_write_hook(STDOUT_FILENO, HTTP_200, sizeof(HTTP_200)-1) != sizeof(HTTP_200)-1)
							break;
					}
					/* Commented out:
				...
				//count = safe_read_hook(fromCgi_rd, rbuf, PIPE_BUF);
				count = safe_read(fromCgi_rd, rbuf, PIPE_BUF);
				if (count <= 0)
					break;  /* eof (or error) */
			}
			if (full_write_hook(STDOUT_FILENO, rbuf, count) != count)
				break;
			if (DEBUG)
...
static NOINLINE void send_file_and_exit(const char *url, int what)
{
	...
	while ((count = safe_read(fd, iobuf, IOBUF_SIZE)) > 0) {
		...
		n = full_write_hook(STDOUT_FILENO, iobuf, count);
		if (count != n)
			break;
...
static int check_user_passwd(const char *path, const char *user_and_passwd)
{
	...
	extern char *hnap_query;	/* @hnap_query := /HNAP1/ */
	extern int httpd_ext_account_verify(const char *);

	if (strcmp(path, hnap_query) == 0 &&
	    httpd_ext_account_verify(user_and_passwd) != 0)
			return 0;
#endif
...
/*
 * Handle an http request if target file do not need to authorized
 * */
static int handle_url_incoming(const char *tptr, int authorized)
{
	struct {
		const char *tg;	/* full URL */
		int policy;	/* 0: allow, -1: don't care, otherwise is deny */
	} *p, ul[] = {
		{ "someone.xml", 1 },
		{ NULL, -1 }
	};
...
extern char *httpd_ext_url_mapping(char *, char *);
#endif
...
#define prequest request_GET
...
#endif
#if ENABLE_FEATURE_HTTPD_HNAP_SUPPORT
	extern char *hnap_query;        /* httpd_ext.c */
#endif
#if ENABLE_FEATURE_HTTPD_MAPPING_URL
	/* used for httpd_ext_url_mapping function */
	char urlcopy_map[128];

	bzero(urlcopy_map, sizeof(urlcopy_map));
#endif
	...
	if (do_ssl == 1) {
		...
		if (cert[0] != '\0') {
			if (SSL_CTX_use_certificate_file(ssl_ctx, cert, SSL_FILETYPE_PEM) == 0 ||
			    SSL_CTX_use_PrivateKey_file(ssl_ctx, cert, SSL_FILETYPE_PEM) == 0 ||
			    SSL_CTX_check_private_key(ssl_ctx) == 0)
			{
#if ENABLE_FEATURE_HTTPD_DEBUG
				DEBUG_MSG("Load pem error");
#endif
		...
		ssl = SSL_new(ssl_ctx);
		//if (SSL_set_fd(ssl, config->accepted_socket) != 1) {
		if (SSL_set_fd(ssl, STDIN_FILENO) != 1) {
#if ENABLE_FEATURE_HTTPD_DEBUG
			DEBUG_MSG("SSL_set_fd err");
#endif
			exit(1);
		}
...
#if ENABLE_FEATURE_HTTPD_DEBUG
			DEBUG_MSG("SSL_accept no list:%d err Return", ssl_ret);
#endif
			switch ((ssl_ret = SSL_get_error(ssl, ssl_ret)))
			{
			case SSL_ERROR_SSL:
#if ENABLE_FEATURE_HTTPD_DEBUG
				DEBUG_MSG("SSL protocol error");
#endif
				break;
			case SSL_ERROR_ZERO_RETURN:
#if ENABLE_FEATURE_HTTPD_DEBUG
				DEBUG_MSG("SSL ZERO RETURN ERROR?");
#endif
				break;
			case SSL_ERROR_SYSCALL:
#if ENABLE_FEATURE_HTTPD_DEBUG
				DEBUG_MSG("CALL back return");
#endif
				break;
			default:
#if ENABLE_FEATURE_HTTPD_DEBUG
				DEBUG_MSG("SSL default ERROR: %d", ssl_ret);
#endif
				;
			}
			exit(1);
	...
	*tptr = '\0';
	...
	strcpy(urlcopy, urlp);
#if ENABLE_FEATURE_HTTPD_DEBUG
	DEBUG_MSG("urlp: '%s' urlcopy: '%s'", urlp, urlcopy);
#endif
	...
	*++urlp = '\0';       /* terminate after last character */
	...
	if (strcmp(bb_basename(urlcopy), httpd_conf) == 0 || !ip_allowed) {
        ...
        /* if particular url do not need to authenticate,
	 * this function will return 0
	 * */
	authorized = handle_url_incoming(urlcopy + 1, authorized);
...
#if ENABLE_FEATURE_HTTPD_HNAP_SUPPORT
	if (strcmp(hnap_query, urlcopy) == 0 && strcmp(prequest, "GET") == 0)
		authorized = 1;
#if ENABLE_FEATURE_HTTPD_DEBUG
	DEBUG_MSG("hnap_query: '%s' urlcopy: '%s' authorized: '%d'",
		hnap_query,
		urlcopy,
		authorized);
#endif
	...
	if (proxy_entry != NULL) {
		...
		header_ptr[1] = '\n';
		header_ptr += 2;
		write_hook(proxy_fd, header_buf, header_ptr - header_buf);
...
#if ENABLE_FEATURE_HTTPD_MAPPING_URL
#if ENABLE_FEATURE_HTTPD_DEBUG
	DEBUG_MSG("tptr: '%s' urlcopy: '%s' urlcopy_map: '%s'",
		tptr,
		urlcopy,
		urlcopy_map);
#endif
	tptr = httpd_ext_url_mapping(urlcopy, urlcopy_map);
#if ENABLE_FEATURE_HTTPD_DEBUG
	DEBUG_MSG("tptr: '%s' urlcopy: '%s' urlcopy_map: '%s'",
		tptr,
		urlcopy,
...
#endif
/* @urlcopy     := /chklst.txt
 * @urlcopy_map := /cgi/ssi/chklst.txt
 * @tptr        := cgi/ssi/chklst.txt
 * */
#endif

#if ENABLE_FEATURE_HTTPD_GRAPHICAL_AUTH
	tptr = httpd_auth_session_timeout(rmt_ip_str, urlcopy, urlcopy_map);
#if ENABLE_FEATURE_HTTPD_DEBUG
	DEBUG_MSG("rmt_ip_str: '%s' tptr: '%s' urlcopy_map: '%s'",
		rmt_ip_str,
		tptr,
		urlcopy_map);
#endif
#endif
/* Here, urlcopy_tmp and urlcopy must including first '/' charactor,
 * ex: /cgi/ssi/login_pic.asp
 * ex: /chklst.txt
 * */
#if ENABLE_FEATURE_HTTPD_CGI
	if (strncmp(tptr, "cgi-bin/", 8) == 0 ||
	    strncmp(tptr, "cgi/", 4) == 0) {
		if (tptr[8] == '\0') {
			/* protect listing "cgi-bin/" */
			send_headers_and_exit(HTTP_FORBIDDEN);
		}
#if ENABLE_FEATURE_HTTPD_MAPPING_URL
		send_cgi_and_exit(urlcopy_map, prequest, length, cookie, content_type);
#else
...
{
	int argc_i = 0;
	const char *pidfile = HTTPD_PIDFILE;
	char *argv_copy[argc + 2];

	for (; argc_i < argc; argc_i++)
		if (strcmp(argv[argc_i], "-k") == 0)
...
int httpd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	//USE_FEATURE_HTTPD_SSL(const char *cert;)
	...
	if (maxpri == -1) {
#if ENABLE_FEATURE_HTTPD_DEBUG
		DEBUG_MSG("sched_get_priority_max failed");
#endif
		return 0;
	}
...
#if ENABLE_FEATURE_HTTPD_DEBUG
		DEBUG_MSG("sched_setscheduler failed");
#endif
		return 0;
	}
#endif	/* ENABLE_FEATURE_HTTPD_MAX_PRIORITY */
...
#if ENABLE_FEATURE_HTTPD_DEBUG
			DEBUG_MSG("Certificate %s Not Found!", cert);
#endif
			return 0;
		}
	} else {
...
//			setenv_long("SERVER_PORT", ???);
	...
	setenv1("SERVER_PORT", bind_addr_or_port);
#endif
