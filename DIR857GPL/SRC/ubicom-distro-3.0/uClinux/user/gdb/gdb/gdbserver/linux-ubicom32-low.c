...
static void
ubicom32_fill_regset (void *buf)
{
  int i;

  for (i = 0; i < UBICOM32_NUM_REGS; i++) {
    collect_register (i, (char *) buf + ubicom32_regmap[i]);
  }
}

static void
ubicom32_store_regset (const void *buf)
{
  int i;

  for (i = 0; i < UBICOM32_NUM_REGS; i++) {
    supply_register (i, (const char *) buf + ubicom32_regmap[i]);
  }
}
...
#ifdef HAVE_PTRACE_GETREGS
  { PTRACE_GETREGS, PTRACE_SETREGS, sizeof(struct pt_regs),
    GENERAL_REGS,
    ubicom32_fill_regset, ubicom32_store_regset },
#endif /* HAVE_PTRACE_GETREGS */
  { 0, 0, -1, -1, NULL, NULL }
};

static const unsigned char ubicom32_breakpoint[] = { 0xAA, 0xBB, 0xCC, 0xDD };
