...
/*
    Name  : Jery Lin added for 404 error spec 
    Description : For different wan types, have different wan interfaces
                  Add new argument to distinct which wan interface need to check it's ip address
*/
#ifdef CONFIG_WEB_404_REDIRECT
	#define LOPT_W4R_WAN_IF 290
	// W4R_WAN_IF define in forward.c
	extern char W4R_WAN_IF[IFNAMSIZ];
#endif
...
#ifdef CONFIG_WEB_404_REDIRECT
    {"w4r-wan-interface", 1, 0, LOPT_W4R_WAN_IF },
#endif
    { NULL, 0, 0, 0 }
  };
...
//   char *open_mode;
//   char *open_lanmac;
  { 'F', ARG_ONE, "path", gettext_noop("Specify path to file of hosts to preload from."), NULL },
  { 'G', ARG_ONE, "path", gettext_noop("Specify path to file of hosts to preload from."), NULL },
  { 'J', ARG_ONE, "path", gettext_noop("Specify path to file of hosts to preload from."), NULL },
#endif
...
#endif
#ifdef CONFIG_WEB_404_REDIRECT
    case LOPT_W4R_WAN_IF:
      {
        if (strlen(arg) < IFNAMSIZ){
          strcpy(W4R_WAN_IF, arg);
	  printf("W4R_WAN_IF=%s\n",W4R_WAN_IF);
	}
        break;
      }
