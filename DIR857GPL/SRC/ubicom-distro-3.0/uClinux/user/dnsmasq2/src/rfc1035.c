...
extern char W4R_LAN_IF[IFNAMSIZ]; //define in forward.c
#define sin_addr(s) (((struct sockaddr_in *)(s))->sin_addr)

// Get IPv6 Link-Local Addr of specify interface(*ifname) save to *addr6
// If success return 0; else return -1;
static int get_v6_lladdr(unsigned char *ifname, struct in6_addr *addr6)
{
	FILE *fp;
	char str_addr[40];
	...
	{
		printf("can't open /proc/net/if_inet6\n");
		return -1;
	}
		...
		devname) != EOF)
	{
		if (strcmp(ifname, devname) == 0)
		{
			...
			for (i=0; i<16; i++) {
				...
				addr6->s6_addr[i] = (unsigned char)ap;
			}
			fclose(fp);
			return 0;
		}
...
// Get IPv4 Addr of specify interface(*ifname) save to *addr
// If success return 0; else return -1;
int get_v4_addr(unsigned char *ifname, struct in_addr *addr)
{
	int sockfd;
	struct ifreq ifr;
...
#ifdef CONFIG_WEB_404_REDIRECT
// Add qtype for distinction between F_IPV4 and F_IPV6
size_t setup_reply(HEADER *header, size_t qlen,
		struct all_addr *addrp, unsigned short flags, unsigned long ttl, unsigned short qtype)
#else
size_t setup_reply(HEADER *header, size_t qlen,
...
#ifdef CONFIG_WEB_404_REDIRECT
      struct all_addr lan_addr = {0};
      int have_ip=0;

      if (qtype==F_IPV4 && get_v4_addr(W4R_LAN_IF, &lan_addr.addr.addr4)==0)
        have_ip=1;
      else if (qtype==F_IPV6) // refuse ipv6 redirect to lan ip
        have_ip=0;
/*
      else if (qtype==F_IPV6 && get_v6_lladdr(W4R_LAN_IF, &lan_addr.addr.addr6)==0)
        have_ip=1;
*/
      if (!NVRAM_MATCH("nvram_default_value", "1"))
	goto eqzero;

      if (qtype==F_IPV4 && have_ip) {
        header->rcode = NOERROR;
        header->ancount = htons(1);
        header->aa = 1;
        add_resource_record(header, NULL, NULL, sizeof(HEADER), &p, ttl, NULL, T_A, C_IN, "4", &lan_addr);
      } else if (qtype==F_IPV6 && have_ip) {
        header->rcode = NOERROR;
        header->ancount = htons(1);
        header->aa = 1;
        add_resource_record(header, NULL, NULL, sizeof(HEADER), &p, ttl, NULL, T_AAAA, C_IN, "6", &lan_addr);
      } else
#endif
