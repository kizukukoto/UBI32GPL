...
#ident  "ACI $Id: //depot/sw/src/dk/mdk/devlib/ar5416/mEepStruct5416.h#53 $, $Header: //depot/sw/src/dk/mdk/devlib/ar5416/mEepStruct5416.h#53 $"
...
#define AR5416_EEP_MINOR_VER_14      0x14 // added bb_tx_clip, desired scale CCK
#define AR5416_EEP_MINOR_VER_15      0x15 // added pwrTableOffset to non-modal header
#define AR5416_EEP_MINOR_VER_16      0x16 // added fracN5g flag to non-modal header
#define AR5416_EEP_MINOR_VER         0x16 // add new fields needed for merlin
...
typedef struct BaseEepHeader {
    ...
    A_UINT8   dacHiPwrMode_5G; // #bits used: 1, bit0: 1: indicated TB352 5G operation
    A_UINT8   openLoopPwrCntl; //#bits used: 1, bit0: 1: use open loop power control, 0: use closed loop power control
    A_UINT8   dacLpMode;    // bits used: bit0.  
    A_UINT8   txGainType;   // #bits used: all, indicates high power tx gain table support. 0: original 1: high power
    A_UINT8   rcChainMask;  // bits used: bit0, identifies an hb93 1x2
    A_UINT8   desiredScaleCCK; // bits used: bit0-bit4: bb_desired_scale_cck
    A_INT8    pwrTableOffset; // offset in dB to be added to beginning of pdadc table in calibration 
    A_UINT8   fracN5g;        // bits used: bit0, indicates that fracN synth mode applies to all 5G channels 
    A_UINT8   futureBase[21];
...
typedef struct ModalEepHeader {
    ...
    A_UINT8   lna_cntl;                              // 8 // bit0: xlnabufmode 
                                                          ...
                                                          // bit6: force_xpaon
                                                          // bit7: useAnt1
    A_UINT8   miscBits;                              // 5 // bit0, bit1      : bb_tx_dac_scale_cck
                                                          // bit2, bit3, bit4: bb_tx_clip
...
typedef struct calDataPerFreqOpLoop {
    A_UINT8 pwrPdg[2][5]; /* power measurement                  */
    A_UINT8 vpdPdg[2][5]; /* pdadc voltage at power measurement */
    A_UINT8 pcdac[2][5];  /* pcdac used for power measurement   */
    A_UINT8 empty[2][5];  /* future use */
} __ATTRIB_PACK CAL_DATA_PER_FREQ_OP_LOOP;
...
typedef union calDataPerFreq_u {
    struct calDataPerFreqOpLoop calDataOpen;
    struct calDataPerFreq       calDataClose;
} __ATTRIB_PACK CAL_DATA_PER_FREQ_U; 
...
typedef struct ar5416Eeprom {
    ...
    CAL_DATA_PER_FREQ_U calPierData5G[AR5416_MAX_CHAINS][AR5416_NUM_5G_CAL_PIERS];        
    CAL_DATA_PER_FREQ_U calPierData2G[AR5416_MAX_CHAINS][AR5416_NUM_2G_CAL_PIERS];
