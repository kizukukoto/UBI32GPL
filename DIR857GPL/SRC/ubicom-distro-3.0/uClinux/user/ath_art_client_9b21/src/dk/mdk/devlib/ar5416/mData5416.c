...
if(isKiteSW(pLibDev->swDevID))
{
	if(antenna == (USE_DESC_ANT|DESC_ANT_A))
		REGW(devNum, 0x8058, REGR(devNum, 0x8058) & ~(0x1<<24));
	else
		REGW(devNum, 0x8058, REGR(devNum, 0x8058) | (1<<24));
	*antModePtr = antenna;
}
    else {
...
//    assert((numBits(pLibDev->libCfgParams.tx_chain_mask) > 1) || !IS_2STREAM_RATE_INDEX(rateIndex));
    ...
    if (IS_MAC_5416_2_0_UP(pLibDev->macRev)) {
		...
		if(isKiteSW(pLibDev->swDevID))
		{
			if(antMode == (USE_DESC_ANT | DESC_ANT_B))
				local5416v2DescPtr->hwControl[8] = (A_UINT32)0x1<<31;
			else
...
)
{
    ...
    if( pLibDev->libCfgParams.ht40_enable ) {
        ...
        REGW(devNum, F2_D_GBL_IFS_SIFS, 0x200); //sifs, 0x64
    }
...
//  send end packet at rate 1Mbps-Long
