/** \file
 * COPYRIGHT (C) Atheros Communications Inc., 2006, All Rights Reserved
 *
 * - Filename    : mEEPROM_g.C
 * - Description : contains functions for readign eeprom adn getting Vpd power 
 *                  settings for all channels for gen5 chips
 * - $Id : $
 * - $DateTime : 2000/01/01 03:44:44 $
 * 
 * - History :
...
/**
 * - Function Name: setup_raw_dataset_gen5
 * - Description  : 
 * - Arguments
 *     -      
 * - Returns      : 
 ******************************************************************************/
A_BOOL setup_raw_dataset_gen5(A_UINT32              devNum, 
                                         RAW_DATA_STRUCT_GEN5* pRawDataset_gen5, 
                                         A_UINT16              myNumRawChannels, 
                                         A_UINT16*             pMyRawChanList, 
                                         A_UINT32              swDeviceID)
{
	A_UINT16	i, j, kk, channelValue, maxNumPdGainsPerChan;	
	A_UINT32   xpd_mask;
    ...
    /* allocate memory pwer channel. Good for open & close loop power control */
    if(!AllocMemPerChan(devNum, pRawDataset_gen5, myNumRawChannels)) {
	...
	for (i = 0; i < myNumRawChannels; i++) {
        ...
        /* check if open loop power control */
        if(pLibDev->libCfgParams.openLoopPwrCntl) {
            maxNumPdGainsPerChan = MAX_NUM_PDGAINS_PER_CHAN_OP_LOOP;
        }
        else {
            maxNumPdGainsPerChan = MAX_NUM_PDGAINS_PER_CHANNEL;
        }

        for (j = 0; j < maxNumPdGainsPerChan; j++) {
			...
			if ( ((xpd_mask >> j) & 0x1) > 0) {
                ...
                if((kk == 1)) {// lowest pd_gain corresponds to highest power and thus, has one more point 
                    pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[j].numVpd = NUM_POINTS_LAST_PDGAIN; /* SUSP never gets here!! */
				}
			} else {
...
/** 
 * - Function Name: AllocMemPerChan
 * - Description  : 
 * - Arguments
 *     - 
 * - Returns      :                                                                
 *******************************************************************************/
#define POINTS_PER_PDGAIN_OPEN_LOOP (1)
A_BOOL AllocMemPerChan( A_UINT32              devNum, 
                        RAW_DATA_STRUCT_GEN5* pRawDataset_gen5, 
                        A_UINT16              numChannels)
{
	A_UINT16	i, j, k;
    A_UINT16    maxGainsPerChan;
	A_UINT16	numVpds;
	LIB_DEV_INFO* pLibDev = gLibInfo.pLibDevArray[devNum];
    ...
    /* allocate memory for the data per channel: RAW_DATA_PER_CHANNEL_GEN5 */
    ...
    if(pLibDev->libCfgParams.openLoopPwrCntl) {
        maxGainsPerChan = MAX_NUM_PDGAINS_PER_CHAN_OP_LOOP;
    } else {
        maxGainsPerChan = MAX_NUM_PDGAINS_PER_CHANNEL;
    }

    /* now allocate the memory for data per PD gain: DATA_PER_PDGAIN_U */
	for(i = 0; i < numChannels; i ++) {
        ...
        if(pLibDev->libCfgParams.openLoopPwrCntl) { /* open loop power control */
            pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain = 
            (RAW_DATA_PER_PDGAIN_GEN5 *)malloc(sizeof(RAW_DATA_PER_PDGAIN_GEN5) * MAX_NUM_PDGAINS_PER_CHAN_OP_LOOP);
        } else {
            pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain = 
            (RAW_DATA_PER_PDGAIN_GEN5 *)malloc(sizeof(RAW_DATA_PER_PDGAIN_GEN5) * MAX_NUM_PDGAINS_PER_CHANNEL);
        }
        ...
        /* if open loop power control, number of readings is one */
        if(pLibDev->libCfgParams.openLoopPwrCntl) {
            numVpds = POINTS_PER_PDGAIN_OPEN_LOOP;
        } else {
            numVpds = NUM_POINTS_LAST_PDGAIN; 
        }

        /* allocate mem for Vpd, pwr and pdadc */
        for (j=0; j<maxGainsPerChan ; j++) {

            /* first, alloc mem for Vpd */
            ...
            if(pLibDev->libCfgParams.openLoopPwrCntl) { /* open loop power control */
                if(pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[j].Vpd == NULL) {
                    mError(devNum, EIO,"unable to allocate Vpds for a pd_gain (open loop)\n");
                    break;
                }
            } else { /* closed loop power control */
                if(pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[j].Vpd == NULL) {
                    mError(devNum, EIO,"unable to allocate Vpds for a pd_gain (closed loop)\n");
                    break;
                }
            }

            /* next, allocate mem for pwr measurements */
            ...
            if(pLibDev->libCfgParams.openLoopPwrCntl) { /* open loop power control */
                if(pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[j].pwr_t4 == NULL) {
                    mError(devNum, EIO,"unable to allocate pwr_t4 for a pd_gain (open loop)\n");
                    break;
                }
            } else { /* closed loop power control */
                if(pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[j].pwr_t4 == NULL) {
                    mError(devNum, EIO,"unable to allocate pwr_t4 for a pd_gain (closed loop)\n");
                    break;
                }
            }

            /* next, in the open loop case, allocate mem for pcdacs */
            if(pLibDev->libCfgParams.openLoopPwrCntl) { /* open loop power control */
                pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[j].pcdac = (A_UINT16 *)malloc(numVpds*sizeof(A_UINT16));
                if(pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[j].pcdac == NULL) {
                    mError(devNum, EIO,"unable to allocate pcdac for a pcdac (open loop)\n");
                    break;
                }

                if(isKiwi(pLibDev->swDevID)) {
                    pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[j].pwr_t4_pal_on = (A_INT16*)malloc(numVpds*sizeof(A_UINT16));
                    if(pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[j].pwr_t4_pal_on == NULL) {
                        mError(devNum, EIO,"unable to allocate pwr_t4_pal_on for a pcdac (open loop)\n");
                        break;
                    }
                }
            }
        ...
        /* check if malloc failed, if so, break and clean up */
        if (j != maxGainsPerChan) {
			break;
		}
	} /* end numChannels loop */

    /* check for memory allocation errors */
    if (i != numChannels) {
		/* malloc must have failed, cleanup any allocs */
		for (j = 0; j <= i; j++) { /* iterate over the channels */
			for (k=0; k < maxGainsPerChan; k++) { /* iterate over pdgains */
                if( pLibDev->libCfgParams.openLoopPwrCntl ) {
                    ...
                    if( pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[k].pcdac != NULL ) {
                        free(pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[k].pcdac);
                    }
                    if(isKiwi(pLibDev->swDevID)) {
                        if( pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[k].pwr_t4_pal_on != NULL ) {
                            free(pRawDataset_gen5->pDataPerChannel[i].pDataPerPDGain[k].pwr_t4_pal_on);
                        }
                    }
                }
                else {
...
/**
 * - Function Name: eeprom_to_raw_dataset_gen5
 * - Description  : 
 * - Arguments
 *     -      
 * - Returns      : 
		...
		for (jj = 0; jj<numPdGainsUsed; jj++) {
			...
			if (pRawXPD->numVpd < 1) {
				printf("ERROR : numVpd for ch[%d] = %d, pdgain = %d[%d] should not be 0 for xpd_mask = 0x%x\n", 
                       ii, pCalCh->channelValue, ss, jj, xpd_mask);
				exit(0);
			}
...
/**
 * - Function Name: CpFromEepromToRawDataset
 * - Description  : SUSP: nothings needs to be done here since this routine is 
 *                  not relevant to new open power control
 * - Arguments
 *     -      
 * - Returns      : 
 ******************************************************************************/
void CpFromEepromToRawDataset(A_UINT32 devNum, EEPROM_DATA_STRUCT_GEN5 *pCalDataset, RAW_DATA_STRUCT_GEN5 *pRawDataset) {
    ...
    /* if open loop power control, only one pdgain is used */
    if(pLibDev->libCfgParams.openLoopPwrCntl) {
        numPdGainsUsed = 1; /* SUSP should be a global flag */
    } else {
        for (jj = 0; jj < MAX_NUM_PDGAINS_PER_CHANNEL; jj++) {
            if (((xpd_mask >> (MAX_NUM_PDGAINS_PER_CHANNEL-jj-1)) & 1) > 0) {
                if (kk >= MAX_NUM_PDGAINS_PER_CHANNEL) {
...
/**
 * - Function Name: initialize_datasets_gen5
 * - Description  : 
 * - Arguments
 *     -      
 * - Returns      : 
