...
// Output:  None
// Description: This fucntion writes ethernet mac address for PB42.
/************************************************************************/
...
//	printf("0x%04x:0x%04x:0x%04x\n", mac[0], mac[1], mac[2]);
	...
	// Workaround for AP81/AP83 platforms. 
	if(isHowlAP(0)) {
		sysFlashConfigWrite(devNum, FLC_RADIOCFG, 0, &mac0Addr[0], 6);
		sysFlashConfigWrite(devNum, FLC_RADIOCFG, 6, &mac1Addr[0], 6);
    }
}
