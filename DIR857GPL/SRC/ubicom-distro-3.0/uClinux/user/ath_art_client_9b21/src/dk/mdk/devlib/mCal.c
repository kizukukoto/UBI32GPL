...
MERLIN_TXGAIN_TBL merlin_tx_gain_table[MAX_NUM_MODES][MERLIN_TX_GAIN_TBL_SIZE];
        ...
        for (i=0; i<MERLIN_TX_GAIN_TBL_SIZE; i++) {
            if( pLibDev->libCfgParams.openLoopPwrCntl ) { /* open loop power control */
                if( isKiwi(pLibDev->swDevID) ) {
                    if( pLibDev->libCfgParams.PAOffsetCal_enable ) /* pal on */
                    {
                        sprintf(fieldName, "%s%d%s", TXGAIN_TABLE_STRING, i, AR9287_PAL_ON);
                    }
                    else /* pal off */
                    { 
                    ...
                    sprintf(fieldName, "%s%d", TXGAIN_TABLE_STRING_OPEN_LOOP, i);
                }
            }
            else {
                if((isKiteSW(pLibDev->swDevID))  && (pLibDev->libCfgParams.txGainType==1)){ // Kite based High_power design (CUS128)
                    sprintf(fieldName, "%s%d", TXGAIN_TABLE_STRING_HIGH_POWER, i);
                }else{
                    sprintf(fieldName, "%s%d", TXGAIN_TABLE_STRING, i);      
                }
            }
            ...
            merlin_tx_gain_table[currentMode][i].txldBloqgain = 
                (A_UCHAR)((val >> TXGAIN_TABLE_TX1DBLOQGAIN_LSB) & TXGAIN_TABLE_TX1DBLOQGAIN_MASK); 
            merlin_tx_gain_table[currentMode][i].txV2Igain = 
                (A_UCHAR)((val >> TXGAIN_TABLE_TXV2IGAIN_LSB) & TXGAIN_TABLE_TXV2IGAIN_MASK); 
            merlin_tx_gain_table[currentMode][i].pabuf5gn = 
                (A_UCHAR)((val >> TXGAIN_TABLE_PABUF5GN_LSB) & TXGAIN_TABLE_PABUF5GN_MASK); 
            merlin_tx_gain_table[currentMode][i].padrvgn = 
                (A_UCHAR)((val >> TXGAIN_TABLE_PADRVGN_LSB) & TXGAIN_TABLE_PADRVGN_MASK); 
            merlin_tx_gain_table[currentMode][i].paout2gn = 
                (A_UCHAR)((val >> TXGAIN_TABLE_PAOUT2GN_LSB) & TXGAIN_TABLE_PAOUT2GN_MASK); 
            merlin_tx_gain_table[currentMode][i].desired_gain = 
                (A_UCHAR)((val >> TXGAIN_TABLE_GAININHALFDB_LSB) & TXGAIN_TABLE_GAININHALFDB_MASK); 
...
/**
 * - Function Name: GetTxGainTable
 * - Description  : This routine retrieves a pointer to the Tx gain table 
 *                  used.
 * - Arguments
 *     -
 * - Returns      : 
 ******************************************************************************/
void GetTxGainTable(MERLIN_TXGAIN_TBL** pGainTbl, A_UCHAR mode) {
    *pGainTbl = merlin_tx_gain_table[mode];
}
/* GetTxGainTable */

/** 
 * - Function Name: PushTxGainTbl
 * - Description  : Pushes a new Tx gain table immediately based on command 
 *                  form ART menu
 * - Arguments
 *     - 
 * - Returns      :                                                                
 *******************************************************************************/
void PushTxGainTbl( A_UINT32 devNum, A_UINT32 pcdac ) {
	...
	while ((pcdac > pGainTbl[i].desired_gain) && (i < (MERLIN_TX_GAIN_TBL_SIZE -1)) ) {
//        printf("pcdac: %d, pGainTbl[%d].desired_gain: %d\n", pcdac, i, pGainTbl[i].desired_gain);
        i++;
        }  // Find entry closest
...
#if (0)
    printf("\n");
    printf("...i:%d pcdac:%d desiredGain:%d dac_gain:%d paout2gn:0x%x padrvgn:0x%x pabuf5gn:0x%x txV2Igain:0x%x txldBloqgain:0x%x \n",i, pcdac,
        pGainTbl[i].desired_gain, dac_gain, pGainTbl[i].paout2gn, pGainTbl[i].padrvgn, pGainTbl[i].pabuf5gn, pGainTbl[i].txV2Igain, pGainTbl[i].txldBloqgain);
    while(!kbhit()){;};
    //printf("0x%x\n", REGR(devNum, 0xa274)); 
//    getchar(); 
#endif
...
*/
	if(isKiteSW(pLibDev->swDevID))
	{
		...
		readVal = eepromRead(devNum,0x48);
		macAddr[1] = (A_UINT8)(readVal & 0xff);
		macAddr[0] = (A_UINT8)((readVal >> 8) & 0xff);
		readVal = eepromRead(devNum,0x47);
		macAddr[3] = (A_UINT8)(readVal & 0xff);
		macAddr[2] = (A_UINT8)((readVal >> 8) & 0xff);
		readVal = eepromRead(devNum,0x46);
		macAddr[5] = (A_UINT8)(readVal & 0xff);
		macAddr[4] = (A_UINT8)((readVal >> 8) & 0xff);
	}
    else if (isKiwi(pLibDev->swDevID)){
        // Read the mac address from the eeprom
        readVal = eepromRead(devNum,0x88);
        macAddr[1] = (A_UINT8)(readVal & 0xff);
        macAddr[0] = (A_UINT8)((readVal >> 8) & 0xff);
        readVal = eepromRead(devNum,0x87);
        macAddr[3] = (A_UINT8)(readVal & 0xff);
        macAddr[2] = (A_UINT8)((readVal >> 8) & 0xff);
        readVal = eepromRead(devNum,0x86);
        macAddr[5] = (A_UINT8)(readVal & 0xff);
        macAddr[4] = (A_UINT8)((readVal >> 8) & 0xff);
    }
	else
