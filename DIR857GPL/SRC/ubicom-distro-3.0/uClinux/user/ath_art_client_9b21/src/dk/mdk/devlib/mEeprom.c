...
#ident  "ACI $Id: //depot/sw/src/dk/mdk/devlib/mEeprom.c#192 $, $Header: //depot/sw/src/dk/mdk/devlib/mEeprom.c#192 $"
...
extern void getAR9285Struct(A_UINT32 devNum, void **ppReturnStruct, A_UINT32 *pNumBytes);
extern void getAR9287Struct(A_UINT32 devNum, void **ppReturnStruct, A_UINT32 *pNumBytes);
#endif
...
)
{
    ...
    if( isOwl(pLibDev->swDevID) ) {
        if( isKiteSW(pLibDev->swDevID) ) {
            ar9285GetPowerPerRateTable(devNum, channel, pRatesPower);
        }
        else if( isKiwi(pLibDev->swDevID) ) {
            ar9287GetPowerPerRateTable(devNum, channel, pRatesPower);
        }
        else {
...
//			*((A_UINT16 *)ppReturnStruct));
	...
	case EEP_AR5416:
            if( isKiteSW(pLibDev->swDevID) ) {
                getAR9285Struct(devNum, ppReturnStruct, pNumBytes);
            }
            else if( isKiwi(pLibDev->swDevID) ) {
                getAR9287Struct(devNum, ppReturnStruct, pNumBytes);
            }
            else {
...
)
{
	...
	if (isOwl(pLibDev->swDevID)) {
		if(isKiteSW(pLibDev->swDevID))
		{
			ar9285EepromSetTransmitPower(devNum, pLibDev->freqForResetDevice, (A_UINT16 *)pRatesPower);
		}
        else if(isKiwi(pLibDev->swDevID))
        {
            ar9287EepromSetTransmitPower(devNum, pLibDev->freqForResetDevice, (A_UINT16 *)pRatesPower);
        }
		else
		{
 ...
 A_UINT32 rate)
{
    ...
    if( isOwl(pLibDev->swDevID) ) {
        if( isKiteSW(pLibDev->swDevID) ) {
            return(get9285MaxPowerForRate(devNum, freq, rate));
        }
        else if( isKiwi(pLibDev->swDevID) ) {
            return(get9287MaxPowerForRate(devNum, freq, rate));
        }
        else {
