...
#ident  "ACI $Id: //depot/sw/src/dk/mdk/devlib/mData.c#174 $, $Header: //depot/sw/src/dk/mdk/devlib/mData.c#174 $"
...
/**
 * - Function Name: txDataSetup
 * - Description  : create packet and descriptors for transmission
 * - Arguments
 *     - devNum
 *     - rateMask
 *     - dest
...
//printf("TX number of packets: %d\n", pLibDev->tx[queueIndex].numDesc);
    ...
    for (i = 0, j = 0; i < pLibDev->tx[queueIndex].numDesc; i++) {
        ...
        ar5kInitData[pLibDev->ar5kInitIndex].pMacAPI->setDescriptor(devNum, 
                                                                    localDescPtr, 
                                                                    pktSize,
                                                                    antMode, 
                                                                    i, 
    ...
    if (pLibDev->libCfgParams.enableXR) {
        if (isOwl(pLibDev->swDevID)) {
            assert(0);
                ...
                setDescriptorEndPacketAr5212( devNum, &localDesc, pktSize, antMode, 0, 15, 0);
        }
    }
    else if (((pLibDev->swDevID & 0x00ff) >= 0x0013) && (pLibDev->mode == MODE_11G) && (!pLibDev->turbo) && !isKiwi(pLibDev->swDevID)) {
        if (isOwl(pLibDev->swDevID)) {
            setDescriptorEndPacketAr5416( devNum, &localDesc, pktSize, antMode, 0, 8, 0);  // 1Mbps-Long rate
        } else {
                setDescriptorEndPacketAr5212( devNum, &localDesc, pktSize, antMode, 0, 8, 0);
        }
    } else {
        if (isOwl(pLibDev->swDevID)) {
            setDescriptorEndPacketAr5416( devNum, &localDesc, pktSize, antMode, 0, 0, 0);
        } else {
			ar5kInitData[pLibDev->ar5kInitIndex].pMacAPI->setDescriptor(devNum, &localDesc, pktSize, antMode, 0, 0, 0);
		}
    }
...
printf("SNOOP::txDataComplete::enable stats send\n");
    ...
    /* run temp comp for olpc systems */
...
)
{
  ...
  *error=1;
  ...
  printf("Cannot write the MAC ADRRESS into the file %s\n",file_name);
...
printf("reg8000=%x:reg8004=%x:reg8008=%x:reg800c=%x:reg8014=%x\n", REGR(devNum, 0x8000), REGR(devNum, 0x8004), REGR(devNum, 0x8008), REGR(devNum, 0x800c), REGR(devNum, 0x8014));
    ...
    if (waitTime == 1) {
        ...
        mError(devNum, EIO, "Device Number %d:rxDataComplete: nothing received within %d millisecs (waitTime)\n", devNum, waitTime);
...
*
*/
MANLIB_API A_INT32 checkBeaconRSSI
(
 A_UINT32 devNum,
 A_UINT32 timeout,
 A_UINT32 remoteStats,
...
//error codes
#define SSID_MISMATCH 0x00000002
#define RSSI_MISMATCH 0x00000004
#define RDLT_MISMATCH 0x00000008
#define SSID_NOBEACON 0x00000010
#define RSSI_MISMATCH_CH0 0x00000020
#define RSSI_MISMATCH_CH1 0x00000040
					...
					if(statsInfo.status2 & DESC_FRM_RCV_OK) // take only the valid frames
						...
						if(memValue==0x80){ // process only the beacons
							...
							if(ssid_length==0){ // hidden SSID
                                rssi_0=(A_INT8)((statsInfo.status4 ) & SIG_STRENGTH_MASK);
                                rssi_1=(A_INT8)((statsInfo.status4 >> 8) & SIG_STRENGTH_MASK); // for chain 1
                                rssi_d=(A_INT8)((rssi_0 > rssi_1) ? (rssi_0 - rssi_1) : (rssi_1 - rssi_0));
#ifdef __ATH_DJGPPDOS__
                                uiPrintf("Found a beacon with hidden SSID with Chain 0 RSSI= %d , Chain 1 RSSI= %d\n",rssi_0,rssi_1);
                                uiPrintf("RSSI delta % d \n",rssi_d);
#else
                                printf("Found a beacon with hidden SSID with Chain 0 RSSI= %d , Chain 1 RSSI= %d\n",rssi_0,rssi_1);
                                printf("RSSI delta % d \n",rssi_d);
#endif
								if((rssi_0>=rssiThreshold)&&(rssi_1>=rssiThreshold)){
                                    if(rssi_d<=rssi_delta){
#ifdef __ATH_DJGPPDOS__
									    uiPrintf("\nFound one beacon with hidden SSID and per chain RSSI above threshold :%d and RSSI delta below %d\n",rssiThreshold,rssi_delta);
									    uiPrintf("MAC Addr: ");
#else
								        printf("\nFound one beacon with hidden SSID and per chain RSSI above threshold :%d and RSSI delta below %d\n",rssiThreshold,rssi_delta);
									    printf("MAC Addr: ");
									    ...
									    for(i=0;i<5;i++)
#ifdef __ATH_DJGPPDOS__
									    uiPrintf("%02x-",frameMacAddr.octets[i]);
									    uiPrintf("%02x",frameMacAddr.octets[5]);
									    uiPrintf(" Chain 0 RSSI: %d , Chain 1 RSSI: %d\n",rssi_0,rssi_1);
#else
									    printf("%02x-",frameMacAddr.octets[i]);
									    printf("%02x",frameMacAddr.octets[5]);
									    printf(" Chain 0 RSSI: %d , Chain 1 RSSI: %d\n",rssi_0,rssi_1);
#endif
									    *result=1;
									    return rssi_0;
                                    }
                                    else {
                                        *result=*result | RDLT_MISMATCH;     
                                    }
								}
                                else{
                                    if((rssi_0<rssiThreshold)&&(rssi_1<rssiThreshold)){
                                        *result=*result | RSSI_MISMATCH;
                                    } else if ((rssi_0>=rssiThreshold)&&(rssi_1<rssiThreshold)){
                                        *result=*result | RSSI_MISMATCH_CH1; 
                                    } else if ((rssi_0<rssiThreshold)&&(rssi_1>=rssiThreshold)){
                                        *result=*result | RSSI_MISMATCH_CH0; 
                                    } 
                                }
							}
								...
								if( !strcmp(SSID,GIVEN_SSID))
                                {
                                    ...
                                    //else
                                    //    rssi_1=rssi_0; // Kite, Ap91 are single chain..
#ifdef __ATH_DJGPPDOS__
                                     if (!isKite(devNum)){                                     
                                         uiPrintf("Found beacon w/SSID: \"%s\". Ant-0 RSSI=%4d, Ant-1 RSSI=%4d\n",GIVEN_SSID,rssi_0,rssi_1);
                                     }
                                     else { /* kite */
                                         if( antenna == 0) {
                                             currRssi = rssi_0;
                                             uiPrintf("Found a beacon with SSID: \"%s\". Ant %d RSSI= %d\n",GIVEN_SSID,antenna, rssi_0);
                                         }
                                         else {
                                             currRssi = rssi_1;
                                             uiPrintf("Found a beacon with SSID: \"%s\". Ant %d RSSI= %d\n",GIVEN_SSID,antenna, rssi_1);
                                         }
                                     }
#else
                                     printf("Found a beacon with SSID: \"%s\". Chain 0 RSSI= %d , Chain 1 RSSI= %d\n",GIVEN_SSID,rssi_0,rssi_1);
#endif
                                     rssi_d=(A_INT8)((rssi_0 > rssi_1) ? (rssi_0 - rssi_1) : (rssi_1 - rssi_0));
#ifndef __ATH_DJGPPDOS__
                                     printf("RSSI delta % d \n",rssi_d);
#endif
                                     if (isKite(devNum)) {
                                         if( currRssi>=rssiThreshold ) {
#ifdef __ATH_DJGPPDOS__
                                             uiPrintf("\nFound one beacon with SSID: \"%s\" and per chain RSSI above threshold :%d\n",GIVEN_SSID,rssiThreshold);
                                             uiPrintf("MAC Addr: ");
#else
                                             printf("\nFound one beacon with SSID: \"%s\" and and per chain RSSI above threshold :%d and RSSI delta below %d\n",GIVEN_SSID,rssiThreshold,rssi_delta);
                                             printf("MAC Addr: ");
#endif
                                             for(i=0;i<5;i++)
#ifdef __ATH_DJGPPDOS__
                                             uiPrintf("%02x-",frameMacAddr.octets[i]);
                                             uiPrintf("%02x\n",frameMacAddr.octets[5]);
#else
                                             printf("%02x-",frameMacAddr.octets[i]);
                                             printf("%02x\n",frameMacAddr.octets[5]);
#endif
                                     ...
                                     } else { /* non-kite designs */
                                         if( (rssi_0>=rssiThreshold)&&(rssi_1>=rssiThreshold) ) {
                                             if(rssi_d<=rssi_delta){
#ifdef __ATH_DJGPPDOS__
                                                 uiPrintf("\nFound beacon w/SSID: \"%s\", per chain RSSI > %d, and deltaRSSI < %d\n",GIVEN_SSID,rssiThreshold, rssi_delta);
                                                 uiPrintf("MAC Addr: ");
#else
                                                 printf("\nFound one beacon with SSID: \"%s\" and per chain RSSI above: %d and RSSI delta below %d\n",GIVEN_SSID,rssiThreshold,rssi_delta);
                                                 printf("MAC Addr: ");
#endif
                                                 for(i=0;i<5;i++)
#ifdef __ATH_DJGPPDOS__
...
#endif
                                                 *result=1;
                                                 currRssi  = ((A_INT32)rssi_0 & 0xffff);
                                                 currRssi |= (((A_INT32)rssi_1<<16)&0xffff0000);
                                                 return currRssi;
                                             }
                                             else {
                                                 *result=*result | RDLT_MISMATCH;
                                ...
                                else{ // ssid does not match
                                     *result=*result | SSID_MISMATCH;
                                }
                            } // ssid length <= 32
                        } // if memValue==80
...
#ifdef __ATH_DJGPPDOS__
    uiPrintf("Could not find a beacon with SSID: %s and RSSI above threshold: %d\n",GIVEN_SSID,rssiThreshold);
#else                    
    printf("Could not find a beacon with SSID: %s and RSSI above threshold: %d\n",GIVEN_SSID,rssiThreshold);
#endif                    
                    if(*result==0) {
                        *result = SSID_NOBEACON;
                    }
...
//                            printf("\n");
    ...
    /* run temp compensation for olpc systems */
    if( isMerlin(pLibDev->swDevID) && pLibDev->libCfgParams.openLoopPwrCntl ) {
        TempComp(devNum);
    }
...
)
{
    ...
    /*  prior to allocating memory for the packet,
     *  figure out its size. Start with the data body
     */
    *pPktSize = dataBodyLength;
    ...
    /* allocate memory in the driver for the packet */
    *pPktAddr = memAlloc(devNum, *pPktSize);
    ...
    /* allocate local memory to create the packet, 
     * so only do one block write of packet
     */
    pTransmitPkt = pPkt = (A_UCHAR *)malloc(*pPktSize);
    if(!pPkt) {
        mError(devNum, ENOMEM, "Device Number %d:Unable to allocate local memory for packet\n", devNum);
...
//#ifdef ARCH_BIG_ENDIAN
    ...
    /* move ptr past the mac header, unless special Tx100 packet */
    if(!pLibDev->specialTx100Pkt) {
        pPkt += sizeof(WLAN_DATA_MAC_HEADER3);
    }
...
//        *pIV = (pLibDev->wepKey << 30) | 0x123456;  //fixed IV for now
    ...
    /* fill out the mdk packet header with 
     * packet type and number of packets 
     */
    pMdkHeader = (MDK_PACKET_HEADER *)pPkt;
#ifdef ARCH_BIG_ENDIAN
    pMdkHeader->pktType = btol_s(mdkType);
    ...
    /* move ptr past the MDK packet header, unless special
     * Tx100 packet and add the repeating data pattern 
     */
    if(!pLibDev->specialTx100Pkt) {
        pPkt += sizeof(MDK_PACKET_HEADER);
    }
    ...
    while (pktBodySize-4) {
        ...
        if (mismatchBits) {
            ...
            if (pStatsInfo->status2 & DESC_FRM_RCV_OK) {
                ...
                printf(">>> ERROR: at %d/%d : mismatch bits = %d [0x%x==0x%x]\n", 
