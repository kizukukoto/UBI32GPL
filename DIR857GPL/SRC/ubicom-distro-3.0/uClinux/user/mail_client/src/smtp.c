...
//#define MAIL_CLIENT_DEBUG 1 
#if MAIL_CLIENT_DEBUG
    ...
    *msg = NULL;
			...
			|| line[len - 1] != '\n')
		{
		    ...
		    if (counter == 0 && len == 0)
		    {
				*errstr = xasprintf(_("the server sent an empty reply"));
				syslog(LOG_INFO,"[email failed] the server sent an empty reply, %s\n",buf);
		    }
		    else
		    {
				*errstr = xasprintf(_("the server sent an invalid reply"));
				syslog(LOG_INFO,"[email failed] the server sent an invalid reply, %s\n",buf);
		    }
		    return SMTP_EPROTO;
		}
    ...
    if (counter > SMTP_MAXLINES)
    {
		...
		*errstr = xasprintf(_("Rejecting server reply that is longer "
		...
		syslog(LOG_INFO,"[email failed] Rejecting server reply that is longer than %d lines , %s\n",SMTP_MAXLINES,buf);
		return SMTP_EPROTO;
    }
...
int smtp_send_cmd(smtp_server_t *srv, char **errstr, const char *format, ...)
{
	...
	*errstr = xasprintf(_("Cannot send command because it is "
		...
		SMTP_MAXCMDLEN);
	syslog(LOG_INFO,"[email failed] Cannot send command because it is longer than %d characters, %s\n",SMTP_MAXCMDLEN,buf);
	return SMTP_EINVAL;
    }
    line[count++] = '\r';
    line[count++] = '\n';
    ...
    *errmsg = NULL;
    ...
    if (smtp_msg_status(msg) != 220)
    {
		...
		*errstr = xasprintf(_("cannot get initial OK message from server"));
		syslog(LOG_INFO,"[email failed] cannot get initial OK message from server, %s\n",temp_buf);
		return SMTP_EPROTO;
    }
		...
		if (smtp_msg_status(ehlo_response) != 250)
		{
		    ...
		    if (smtp_msg_status(ehlo_response) != 250)
		    {
				...
				*errstr = xasprintf(_("SMTP server does not accept "
				...
				syslog(LOG_INFO,"[email failed] SMTP server does not accept EHLO or HELO commands, %s\n",buf);	
				return SMTP_EPROTO;
		    }
    ...
    else /* protocol is LMTP */
    {
		...
		if (smtp_msg_status(ehlo_response) != 250)
		{
		    ...
		    *errstr = xasprintf(_("command %s failed"), "LHLO");
			syslog(LOG_INFO,"[email failed] command LHLO failed, %s\n",buf);
		    return SMTP_EPROTO;
		}
    }
	...
	*error_msg = msg;
	if (status == 504)
	{
	    *errstr = xasprintf(_("command %s failed"), "AUTH PLAIN");
		syslog(LOG_INFO,"[email failed] command AUTH PLAIN failed, %s\n",buf);
	    return SMTP_EPROTO;
	}
	else
	{
	    *errstr = xasprintf(_("authentication failed (method %s)"), 
		    "PLAIN");
		syslog(LOG_INFO,"[email failed] authentication failed (method PLAIN), %s\n",buf);
    ...
    *error_msg = NULL;
    ...
    if (smtp_msg_status(msg) != 334)
    {
		...
		*errstr = xasprintf(_("command %s failed"), "AUTH LOGIN");
		syslog(LOG_INFO,"[email failed] command AUTH LOGIN failed, %s\n",buf);
		return SMTP_EPROTO;
    }
    ...
    if (smtp_msg_status(msg) != 334)
    {
		...
		*errstr = xasprintf(_("authentication failed (method %s)"), "LOGIN");
		syslog(LOG_INFO,"[email failed] authentication failed (method LOGIN), %s\n",buf);
		return SMTP_EAUTHFAIL;
    }
    ...
    *error_msg = NULL;
    ...
    if (smtp_msg_status(msg) != 334)
    {
		...
		*errstr = xasprintf(_("command %s failed"), "AUTH CRAM-MD5");
		syslog(LOG_INFO,"[email failed]command AUTH CRAM-MD5 failed, %s\n",temp_buf);	
		return SMTP_EPROTO;
    }
    ...
    if (!base64_decode(challenge, challenge_len, b64, &len))
    {
		...
		*errstr = xasprintf(_("authentication method CRAM-MD5: "
		...
		syslog(LOG_INFO,"[email failed] authentication method CRAM-MD5:server sent invalid challenge, %s\n",temp_buf);
		return SMTP_EPROTO;
    }
    ...
    *p++ = ' ';
    ...
    if (smtp_msg_status(msg) != 235)
    {
		...
		*errstr = xasprintf(_("authentication failed (method %s)"), "CRAM-MD5");
		syslog(LOG_INFO,"[email failed] authentication failed (method CRAM-MD5), %s\n",temp_buf);
		return SMTP_EAUTHFAIL;
    }
	...
	list_t **error_msg, char **errstr)
{
	...
	*errstr = xasprintf(_("command %s failed"), "AUTH EXTERNAL");
	syslog(LOG_INFO,"[email failed]command AUTH EXTERNAL failed, %s\n",buf);
	return SMTP_EPROTO;
    }
    list_xfree(msg, free);
    u_len = strlen(user);
	...
	*errstr = xasprintf(_("authentication failed (method %s)"), "EXTERNAL");
	syslog(LOG_INFO,"[email failed] authentication failed (method EXTERNAL), %s\n",buf);
	return SMTP_EAUTHFAIL;
    }
    list_xfree(msg, free);
	...
	char **errstr)
{
	...
	*errstr = xasprintf(
		...
		auth_mech);
	syslog(LOG_INFO,"[email failed]GNU SASL: authentication method %s not supported, %s\n",auth_mech,buf);
	return SMTP_ELIBFAILED;
    }
    if (strcmp(auth_mech, "") == 0)
    {
	...
	if (!tls_is_active(&srv->tls))
	{
	    ...
	    *errstr = xasprintf(_("cannot use a secure authentication method"));
		syslog(LOG_INFO,"[email failed]cannot use a secure authentication method, %s\n",buf);
#ifdef HAVE_TLS
	}
	else
	{
	...
	if (!user)
	{
	    ...
	    *errstr = xasprintf(_("authentication method %s needs a user name"),
		    auth_mech);
		syslog(LOG_INFO,"[email failed]authentication method %s needs a user name, %s\n",auth_mech,buf);
	    return SMTP_EUNAVAIL;
	}
	...
	if (strcmp(auth_mech, "GSSAPI") != 0 && !password)
	{
		...
		*errstr = xasprintf(
			...
			auth_mech);
		syslog(LOG_INFO,"[email failed]authentication method %s needs a password, %s\n",auth_mech,buf);
		return SMTP_EUNAVAIL;
	    }
	...
	*errstr = xasprintf(_("GNU SASL: %s"), gsasl_strerror(error_code));
	...
	if (!input)
	{
		...
		*errstr = xasprintf(_("authentication failed (method %s)"), 
			auth_mech);
		syslog(LOG_INFO,"[email failed]authentication failed (method %s), %s\n",auth_mech,buf);
		return SMTP_EAUTHFAIL;
	    }	    
	...
	*errstr = xasprintf(_("authentication failed: %s (method %s)"), 
	...
	syslog(LOG_INFO,"[email failed]authentication failed %s (method %s), %s\n",gsasl_strerror(error_code),auth_mech,buf);
	return SMTP_EAUTHFAIL;
    }
    gsasl_finish(sctx);
    gsasl_done(ctx);
    ...
    *error_msg = NULL;
    ...
    else
    {
		*errstr = xasprintf(_("authentication method %s not supported"),
			auth_mech);
		syslog(LOG_INFO,"[email failed] authentication method %s not supported, %s\n",auth_mech,buf);
		e = SMTP_ELIBFAILED;
    }
		    ...
		    if (!mailfrom_reply_was_rcvd)
		    {
				...
				if (smtp_msg_status(msg) != 250)
				{
				    ...
				    *errstr = xasprintf(_("envelope from address %s not "
					...
					syslog(LOG_INFO,"[email failed]envelope from address %s not accepted by the server, %s\n",envelope_from,buf);
				    return SMTP_EINVAL;
				}
				list_xfree(msg, free);
		    ...
		    else if (!list_is_empty(rcpt_recv))
		    {
				...
				if (smtp_msg_status(msg) != 250)
				{
				    ...
				    *errstr = xasprintf(_("recipient address %s not "
					    ...
					    (char *)(rcpt_recv->data));
					syslog(LOG_INFO,"[email failed]recipient address %s not accepted by the server, %s\n",(char *)(rcpt_recv->data),buf);
				    return SMTP_EINVAL;
				}
				list_xfree(msg, free);		
			}
		    else
		    {
				...
				if (smtp_msg_status(msg) != 354)
				{
				    ...
				    *errstr = xasprintf(
					...
					syslog(LOG_INFO,"[email failed]the server does not accept mail data, %s\n",buf); 
				    return SMTP_EUNAVAIL;
				}
				list_xfree(msg, free);
...
int edit_log_file(void)
{
	...
	//log full check
	fp_read = fopen(LOG_FILE_FULL, "r");
	if (fp_read != NULL) {
		printf("Open messages_full file : %s\n", LOG_FILE_FULL);
	}
	else {
		fp_read = fopen(LOG_FILE_HTTP, "r");
		if (fp_read == NULL) {
		...
		printf("Open messages file : %s\n", LOG_FILE_HTTP);
	}

	fp_write = fopen(LOG_FILE_MAIL, "w");
	if (fp_write == NULL)
		return 1;

	printf("Write messages_mail file : %s\n", LOG_FILE_MAIL);
	fprintf(fp_write, "Subject: %s Log\n\n", hostname);

	while (fgets(buffer, MAIL_BUFSIZE, fp_read)) {
		fprintf(fp_write, "%s\n", buffer);
	}
...
#define get_ip_addr get_ipaddr
char *get_wan_ip(char *wan_ip_1,long len)
{
        char *tmp_ip_1 = NULL;
        ...
        || strcmp(wan_proto,"usb3g") == 0
			...
			tmp_ip_1 = get_ip_addr("ppp0");
            if(tmp_ip_1)
                    strncpy(wan_ip_1, tmp_ip_1, len);
        }else{
			tmp_ip_1 = get_ip_addr(nvram_safe_get("wan_eth"));
        	if (tmp_ip_1)
                strncpy(wan_ip_1, tmp_ip_1, len);
        }
}
...
#else
void get_wan_ip(char *wan_ip_1,long len)
{
	char *tmp_ip_1 = NULL;
...
int edit_link_file(void)
{
	...
	fprintf(fp_write, "Subject: %s Web File Access Link\n\n", nvram_get("hostname"));
	fputs("You can use this link to connect to the drive remotely after logging with a user account.\n\n", fp_write);

	if (nvram_match("ddns_enable", "1") == 0)
		strcpy(link, nvram_get("ddns_hostname"));
	else
		get_wan_ip(link,128);
...
//printf(">>>[%s]\n",link);
	...
	if (nvram_match("file_access_ssl", "1") == 0) {
		fprintf(fp_write, "https://%s:%s/\n\n", link, nvram_get("file_access_ssl_port"));
	}

	if (nvram_match("file_access_remote", "1") == 0) {
		fprintf(fp_write, "http://%s:%s/\n\n", link, nvram_get("file_access_remote_port"));
	}
	...
	long *mailsize, char **errstr)
{
	...
	FILE *fp_weblink_active=NULL;
	long weblink_active;

	
	weblink_active = 0;
	fp_weblink_active = fopen("/var/tmp/send_weblink_active", "r");
	if( fp_weblink_active ) {
		weblink_active = 1;
		fclose(fp_weblink_active);
		system("rm /var/tmp/send_weblink_active");
	}
	
	DEBUG_MSG("smtp_send_mail: start\n");

	fp_bak = fopen("/var/tmp/send_device_link", "r");
	
	
printf(">>>>>> %d %d\n",weblink_active,fp_bak);	
	
	
	if (weblink_active && fp_bak) {
		if (edit_link_file() != 0) {
			printf("smtp_send_mail: edit_link_file error");
			fclose(fp_bak);
			system("echo -1 > /var/tmp/send_device_link");
			return SMTP_EIO;
		}
		fclose(fp_bak);
		system("echo 1 > /var/tmp/send_device_link");
	...
	fp_bak=fopen(LOG_FILE_MAIL, "r");
	
	if(fp_bak==NULL)
	{
		printf("smtp_send_mail: open LOG_FILE_MAIL failed\n");
		return SMTP_EIO;
	} 
	...
	for (;;)
	{
		...
		if (buffer[0] == 'e')
		{
			printf("smtp_send_mail: reading LOG_FILE_MAIL finished\n");
			break;
		}
	...
	char **errstr)
{
    ...
    *error_msgs = list_new();
	...
	if (smtp_msg_status(msg) != 250)
	{
		...
		syslog(LOG_INFO,"[email failed]the server refuses to send the mail to %s, %s\n",(char *)(lp_recipients->data),buf);
    ...
    *error_msg = NULL;
	...
	case 459: /* Node <x> not allowed: <reason> */
	    ...
	    *errstr = xasprintf(
		...
		syslog(LOG_INFO,"[email failed]the server is unable to fulfill the request,%s\n",buf);
	    return SMTP_EUNAVAIL;
	    break;
	...
	case 501: /* 501 Syntax Error in Parameters */
	    ...
	    *errstr = xasprintf(
		...
		syslog(LOG_INFO,"[email failed]invalid argument for Remote Message Queue Starting,%s\n",buf);
	    return SMTP_EINVAL;
	    break;
	...
	default:
	    ...
	    *errstr = xasprintf(_("command %s failed"), "ETRN");
		syslog(LOG_INFO,"[email failed]command ETRN failed, %s\n",buf);
	    return SMTP_EPROTO;
	    break;
    }
    list_xfree(msg, free);
