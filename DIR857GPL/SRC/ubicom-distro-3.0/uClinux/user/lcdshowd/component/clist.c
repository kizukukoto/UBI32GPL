...
static int pt_in_rect(int x, int y, DFBRectangle *rc);
static int list_hittest(CLiteList *list, int x, int y);
static void get_vertical_scrollbar_rect(CLiteList *list, DFBRectangle *rect);
static int list_needs_scroll(CLiteList *list );
static void list_update_scrollbar(CLiteList *list );

int can_scroll(LiteScrollInfo *info); // scrollbar helper func. defined at scrollbar.c

DFBResult lite_new_clist(
	LiteBox		*parent, 
	DFBRectangle	*rect,
	LiteListTheme	*theme,
	CLiteList	**ret_list)
{
	...
	list = D_CALLOC(1, sizeof(CLiteList));
...
DFBResult lite_clist_set_row_height(CLiteList* list, int row_height)
{
...
DFBResult lite_clist_get_row_height(CLiteList *list, int *row_height)
{
	LITE_NULL_PARAMETER_CHECK(list);
...
DFBResult lite_enable_clist(CLiteList *list, int enable)
{
	LITE_NULL_PARAMETER_CHECK(list);
...
DFBResult lite_clist_insert_item(
	CLiteList	*list, 
	int		index,
	char		*data)
{
	...
	list->arr_data[list->item_count] = strdup(data);
	++list->item_count;
	...
	char		*data)
{
	...
	list->arr_data[index] = strdup(data);
...
DFBResult lite_clist_del_item(CLiteList *list, int index)
{
	LITE_NULL_PARAMETER_CHECK(list);
	if(index < 0 || index >= list->item_count)
		return DFB_INVARG;
...
DFBResult lite_clist_sort_items(
	CLiteList *list,
...
DFBResult lite_clist_get_item_count(CLiteList *list, int *count)
{
	LITE_NULL_PARAMETER_CHECK(list);
...
DFBResult lite_clist_get_selected_item_index(CLiteList *list, int *index)
{
	LITE_NULL_PARAMETER_CHECK(list);
...
DFBResult lite_clist_set_selected_item_index(CLiteList *list, int index)
{
	LITE_NULL_PARAMETER_CHECK(list);
	...
	lite_update_box(&list->box, NULL);
	lite_clist_ensure_visible(list, index);

	return DFB_OK;
}

DFBResult lite_clist_ensure_visible(CLiteList* list, int index)
{
	...
	rcVirtualViewPort.y = (info.track_pos == -1)? info.pos: info.track_pos;
...
DFBResult lite_clist_on_draw_item(
	CLiteList		*list,
...
DFBResult lite_clist_on_sel_change(
	CLiteList		*list,
...
DFBResult lite_clist_recalc_layout(CLiteList *list)
{
...
DFBResult lite_new_clist_theme(
...
DFBResult lite_destroy_clist_theme(LiteListTheme *theme)
{
	LITE_NULL_PARAMETER_CHECK(theme);
...
static DFBResult destroy_list(LiteBox *box)
{
	...
	for (; i < list->item_count; i++)
		free(list->arr_data[i]);

	return lite_destroy_box(box);
}

static DFBResult draw_list(
	LiteBox		*box, 
...
static int on_key_down(LiteBox *box, DFBWindowEvent *evt)
{
	...
	if (new_selected != cur_selected) {
		lite_clist_set_selected_item_index(list, new_selected);
	}     

	return 1;
}

static void on_vertical_scrollbar_update(
	LiteScrollbar	*scrollbar,
	LiteScrollInfo	*info,
	void		*ctx)
{
	CLiteList *list = LITE_LIST(ctx);
...
void get_vertical_scrollbar_rect( CLiteList *list, DFBRectangle *rect)
{
	int scrollbar_thickness;
	DFBRectangle rc;
...
int list_needs_scroll(CLiteList *list)
{
	LiteScrollInfo info;
...
int list_hittest(CLiteList *list, int x, int y)
{
	int scrollpos = 0;
	int sel;
	DFBResult res;
...
void list_update_scrollbar(CLiteList *list)
{
	int total_height;
