...
static DFBResult mouse_callback(DFBWindowEvent *evt, void *data)
{
	...
	if (evt->type == DWET_BUTTONDOWN) {
		...
		md->offset_x = evt->cx - md->x;
		md->offset_y = evt->cy - md->y;
		md->x = evt->cx;
		md->y = evt->cy;
...
DFBWindowEventType mouse_handler(LiteWindow *win, int *x, int *y, int timeout)
{
	...
	*/
	...
	if( (md.offset_y < -20) && (abs(md.offset_x) < abs(md.offset_y))){
		DEBUG_MSG("This is DWET_NONE \n");
		return DWET_NONE;
	}
	if ((abs(md.offset_x) < 8) && (abs(md.offset_y) < 8)){
		DEBUG_MSG("This is DWET_BUTTONUP \n");
