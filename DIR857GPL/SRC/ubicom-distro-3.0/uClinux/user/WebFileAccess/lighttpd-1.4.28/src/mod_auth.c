...
static handler_t mod_auth_uri_handler(server *srv, connection *con, void *p_d) {
	...
	if( !strcmp(con->uri.path->ptr, "/HNAP1/") && con->request.http_method == HTTP_METHOD_GET) 
		auth_satisfied = 1;

	if (NULL != (ds = (data_string *)array_get_element(con->request.headers, "SOAPAction"))){
		soap_string = ds->value->ptr;
	}	
	//	log_error_write(srv, __FILE__, __LINE__, "ss", "soap_string:", soap_string);
			...
			int auth_type_len = auth_realm - http_authorization;
				...
				if ((auth_type_len == 5) &&
					...
					if (0 == strcmp(method->value->ptr, "basic")) {
						if( strstr(con->uri.path->ptr, "/HNAP1") && strstr(soap_string, "GetDeviceSettings") ){
							//log_error_write(srv, __FILE__, __LINE__, "ss", "GetDeviceSettings:", soap_string);
							auth_satisfied = 1;
						}else{	
	...
	if (!auth_satisfied) {
		...
		if( !strcmp(con->uri.path->ptr, "/HNAP1/") && con->request.http_method == HTTP_METHOD_GET){
			...
			buffer_copy_string(con->authed_user,"admin");
			//log_error_write(srv, __FILE__, __LINE__, "s", "0:");
		}else{
			if( strstr(con->uri.path->ptr, "/HNAP1") && con->request.http_method == HTTP_METHOD_POST && strstr(soap_string, "GetDeviceSettings")){
				//log_error_write(srv, __FILE__, __LINE__, "s", "1:");
				buffer_copy_string(con->authed_user,"admin");
			}else{
				//log_error_write(srv, __FILE__, __LINE__, "s", "2:");
