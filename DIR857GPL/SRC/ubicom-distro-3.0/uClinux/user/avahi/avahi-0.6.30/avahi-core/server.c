...
static void update_fqdn(AvahiServer *s) {
    ...
    if (s->host_name_extra) {
        if (!(m = avahi_strdup_printf("%s.%s", s->host_name_extra, s->domain_name)))
            return; /* OOM */
        avahi_free(s->host_name_fqdn_extra);
        s->host_name_fqdn_extra = m;
    }
}
...
AvahiServer *avahi_server_new(const AvahiPoll *poll_api, const AvahiServerConfig *sc, AvahiServerCallback callback, void* userdata, int *error) {
    ...
    s->host_name_extra = s->config.host_name_extra ? avahi_normalize_name_strdup(s->config.host_name_extra) : NULL;
    if (s->host_name_extra)
        s->host_name_extra[strcspn(s->host_name_extra, ".")] = 0;
    s->host_name_fqdn_extra = NULL;
