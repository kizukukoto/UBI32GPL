...
void avahi_interface_address_update_rrs(AvahiInterfaceAddress *a, int remove_rrs, char *name, AvahiSEntryGroup **p_entry_group) {
    ...
    if (m->list_complete &&
        ...
        if (!(*p_entry_group))
            *p_entry_group = avahi_s_entry_group_new(m->server, avahi_host_rr_entry_group_callback, NULL);

        if (!(*p_entry_group)) /* OOM */
            return;

        if (avahi_s_entry_group_is_empty(*p_entry_group)) {
            ...
            if (avahi_server_add_address(m->server, *p_entry_group, a->interface->hardware->index, p, 0, name, &a->address) < 0) {
                avahi_log_warn(__FILE__": avahi_server_add_address() failed: %s", avahi_strerror(m->server->error));
                avahi_s_entry_group_free(*p_entry_group);
                *p_entry_group = NULL;
                return;
            }

            avahi_s_entry_group_commit(*p_entry_group);
        }
    } else {

        /* Clear the entry group */

        if (*p_entry_group && !avahi_s_entry_group_is_empty(*p_entry_group)) {
            char t[AVAHI_ADDRESS_STR_MAX];
            avahi_address_snprint(t, sizeof(t), &a->address);

            avahi_log_info("Withdrawing address record for %s on %s.", t, a->interface->hardware->name);

            if (avahi_s_entry_group_get_state(*p_entry_group) == AVAHI_ENTRY_GROUP_REGISTERING &&
                m->server->state == AVAHI_SERVER_REGISTERING)
                avahi_server_decrease_host_rr_pending(m->server);

            avahi_s_entry_group_reset(*p_entry_group);
        }
    }
}
...
void avahi_interface_update_rrs(AvahiInterface *i, int remove_rrs) {
    ...
    for (a = i->addresses; a; a = a->address_next) {
        // Jery: If address is IPv6, we only want link-local address
        if ( a->address.proto!=AVAHI_PROTO_INET6 || a->global_scope==0) {
            avahi_interface_address_update_rrs(a, remove_rrs, NULL,  &a->entry_group);
            if (a->monitor->server->host_name_fqdn_extra) {
                avahi_interface_address_update_rrs(a, remove_rrs, a->monitor->server->host_name_fqdn_extra, &a->entry_group_extra);
            }
        }
    }
}
...
static int interface_mdns_mcast_join(AvahiInterface *i, int join) {
    ...
    if (join) {
        ...
        // Jery: we want support IPv6 link-local address, don't check global address
...
void avahi_interface_address_free(AvahiInterfaceAddress *a) {
    ...
    avahi_interface_address_update_rrs(a, 1, NULL, &a->entry_group);
    avahi_interface_address_update_rrs(a, 1, a->monitor->server->host_name_fqdn_extra, &a->entry_group_extra);
...
int avahi_interface_address_is_relevant(AvahiInterfaceAddress *a) {
    ...
    for (b = a->interface->addresses; b; b = b->address_next) {
        if (b == a)
            continue;
        // Jery: support IPv6 link-local address
        if (a->address.proto!=AVAHI_PROTO_INET6 && b->global_scope && !b->deprecated)
            return 0;
    }
