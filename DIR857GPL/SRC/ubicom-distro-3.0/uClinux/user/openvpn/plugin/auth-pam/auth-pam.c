...
#define PAM_SERVER_STACK_SIZE (1024 * 8)
...
#ifdef NO_FORK
static int pam_server_wrapper(void *args)
{
  struct auth_pam_context *ctx = (struct auth_pam_context *)args;
  close_fds_except(ctx->background_fd);
  set_signals();
  pam_server(ctx->background_fd, ctx->service, ctx->verb, &ctx->name_value_list);
  close(ctx->background_fd);
  return 0;
}
#endif
...
#ifdef NO_FORK
  {
    void *server_stack = malloc(PAM_SERVER_STACK_SIZE);
    if (!server_stack)  {
      fprintf(stderr, "AUTH-pam: server_stack malloc failed\n");
      close(fd[0]);
      close(fd[1]);
      goto error;
    }

    /* Keep stack variables into a heap one before cloning */
    context->background_fd = fd[1];
    context->service = argv[1];
    context->name_value_list = name_value_list;
    pid = clone(pam_server_wrapper, server_stack + PAM_SERVER_STACK_SIZE, CLONE_VM, context);
    if (pid < 0)
      free(server_stack);
  }
#else
