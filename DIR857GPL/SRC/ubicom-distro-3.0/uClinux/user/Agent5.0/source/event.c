...
// 2010.07.29 filename issue
static const char *event_file = "/etc/tr/tr_events"; //The event file name

static char event_codes[][32] = {
            "0 BOOTSTRAP",
            "1 BOOT",
            "2 PERIODIC",
            "3 SCHEDULED",
...
static void del_oldest_multi_event()
{
	...
	if(o) {
		...
		} else if(ec == M_EVENT_DOWNLOAD || ec == M_EVENT_UPLOAD) {
...
int add_multi_event(event_code_t ec, const char *ck)
{
	...
	if(new) {
		new->event_code = ec;
		war_snprintf(new->command_key, sizeof(new->command_key), "%s", ck==NULL?"":ck);
		add_event(new);
		res = 0;
	} else {
		tr_log(LOG_ERROR, "Out of memory!");
...
/*!
 * \brief Write the event list to file make sure them can across reboot
 *
