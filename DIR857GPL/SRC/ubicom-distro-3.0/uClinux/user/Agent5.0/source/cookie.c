...
/*!
 * \brief Check if or not a cookie match current URL
 *
 * \param ci The cookie
...
/*!
 * \brief Check if or not the current HTTP session's server port in a port list
 *
...
static int match_port(struct cookie_item *ci, unsigned short cur_port)
{
    if(ci->received_port) { //Port attribute in Set-Cookie2
...
void cookie_header(struct cookie *c, int secure, const char *req_host, const char *req_path, unsigned short req_port)
{
    ...
    for(prev = NULL, ci = c->items; ci; ci = next) {
        next = ci->next;
        if(ci->expire && ci->expire < cur) { //The cookie is timeout
