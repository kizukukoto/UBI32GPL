...
/*
 *get modifier value, do not change the pointor buf.
 */
...
/*
 * read the Configuration Management Data
 */
static int read_cm_file(const char* path, struct buffer *buf)
{
	...
	if(fp){
		while(!feof(fp)){
			nread = fread(tmp, 1, sizeof(tmp) - 1, fp);
			tmp[nread] = '\0';
...
/*
 * <fileHeader fileFormatVersion="fileFormatVersion1" senderName="senderName1" vendorName="vendorName1"/>
 */
...
/*
 *<fileFooter dateTime="2006-05-04T18:13:51.0Z"/>
 */
...
/*
 * handle add object
 * cur_node move to the node which is added.
 */
static int handle_add_object(node_t *cur_node, int enable)
{
	...
	if(ao_journal(tmp) ||lib_ao(*cur_node, nin))
		return 1;
 ...
 * the cur_node move to the parent node.
 */
 ...
 * if the children of cur_node is instance objects
 * move cur_node to it's children.
 */
...
/*
 * handle object. add, delete, or update.
 */
...
/*
 * handle parameter, set parameter value.
 */
...
/*
 * skip <Modifier>, the value has already get by get_modifier()
 */
