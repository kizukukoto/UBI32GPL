 ...
 * The TR-069 protocol require each RPC command MUST be executed atomically. That is
...
static int append_journal(int argc, struct item argv[], int jt)
{
	...
	if(journal_header.count < sizeof(journal_header.journals) / sizeof(journal_header.journals[0])) {
		...
		res = fwrite(&j, 1, sizeof(j), journal_header.fp);
		for(i = 0; i < argc; i++) {
			u.unit_type = argv[i].type;
			u.data_len = argv[i].data_len;

			res += fwrite(&u, 1, sizeof(u), journal_header.fp);
			res += fwrite(argv[i].data, 1, argv[i].data_len, journal_header.fp);
		}
...
static int __spa_rollback(const char *name)
{
	...
	fread(&u, 1, sizeof(u), journal_header.fp);
	fread(path, 1, MIN(u.data_len, sizeof(path) - 1), journal_header.fp);
	path[MIN(u.data_len, sizeof(path) - 1)] = '\0';

	fread(&u, 1, sizeof(u), journal_header.fp);

        fread(property, 1, MIN(u.data_len, sizeof(property)), journal_header.fp);
...
static int spv_rollback()
{
	...
	if(value == NULL) {
		...
		fread(value, 1, u.data_len, journal_header.fp);
		value[u.data_len] = '\0';
	}
...
static int ao_rollback()
{
	...
	ret = fread(&u, 1, sizeof(u), journal_header.fp);
	if(ret < 0){
		tr_log(LOG_ERROR, "fread unit error:%s", war_strerror(war_geterror()));
		return -1;
	}
	ret = fread(path, 1, u.data_len, journal_header.fp);
	if(ret < 0){
		tr_log(LOG_ERROR, "fread error:%s", war_strerror(war_geterror()));
		return -1;
	}
...
static int do_rollback()
{
	...
	ret = fread(&u, 1, sizeof(u), journal_header.fp);
	if(ret < 0) {
		tr_log(LOG_ERROR, "fread unit error:%s\n", war_strerror(war_geterror()));
		return -1;
	}
	ret = fread(path, 1, u.data_len, journal_header.fp);
	if(ret < 0) {
		tr_log(LOG_ERROR, "fread error:%s\n", war_strerror(war_geterror()));
		return -1;
	}
	path[u.data_len] = '\0';
...
int rollback_transaction()
{
	...
	for (; journal_header.count > 0;) {
		...
		fread(&j, 1, sizeof(j), journal_header.fp);
...
int clear_journals()
{
	...
	if (journal_header.fp) {
		...
		for (;;) {
			...
			rc = fread(&j, 1, sizeof(j), journal_header.fp);
