...
static void reset_values()
{
	...
	if(lib_start_session() > 0) {
		if(lib_resolve_node(ECHO_BYTES_RECEIVED, &node) == 0) {
			lib_set_value(node, "0");
			bytes_received = 0;
		}
		if(lib_resolve_node(ECHO_BYTES_RESPONDED, &node) == 0) {
			lib_set_value(node, "0");
			bytes_responded = 0;
		}
		if(lib_resolve_node(ECHO_TIME_OF_FIRST_PACKET, &node) == 0) {
			lib_set_value(node, "0001-01-01T00:00:00.0");
			time_of_first_packet = 0;
		}
		if(lib_resolve_node(ECHO_TIME_OF_LAST_PACKET, &node) == 0) {
			lib_set_value(node, "0001-01-01T00:00:00.0");
		}
...
static void enable_changed(const char *new)
{
	int old;
	...
	if(enabled == BOOLEAN_ERROR)
		enabled = old;
	if(enabled != old) {
		if(enabled) { //Reset some parameter
			reset_values();
		}
		launch_echo_server();
	}
...
static void echoplus_enable_changed(const char *new)
{
	int old;
...
static void interface_changed(const char *new)
{
...
static void source_ip_addr_changed(const char *new)
{
...
static void udp_port_changed(const char *new)
{
...
/*!
 * \brief The callback function of echo scheduler. This function read data from the echo
 * socket and echo it to peer if the peer's address is the expected.
 *
...
static void echo_readable(struct sched *sc)
{
	...
	if(res > 0) {
		...
		if(lib_start_session() > 0) {
			...
			packets_received++;
			if(lib_resolve_node(ECHO_PACKETS_RECEIVED, &node) == 0) {
				war_snprintf(num, sizeof(num), "%d", packets_received);
				lib_set_value(node, num);
			}
			...
			if(from.sin_addr.s_addr == (in_addr_t)(sc->pdata)) {
				/*
				 * The peer's address is the expected, then echo back the received data,
				...
				packets_responded++;
				if(lib_resolve_node(ECHO_PACKETS_RESPONDED, &node) == 0) {
...
int launch_echo_server()
{
	...
	if(first) { /*Init related parameters when launch the echo server the first time*/
		char buf[32] = "";
