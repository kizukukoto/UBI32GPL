 ...
 *            C. Scktt Ananian <cananian@alumni.princeton.edu>
 *
...
int main(int argc, char **argv, char **envp)
{
    ...
    if(launchpppd){
        ...
        switch (child_pid = vfork()) {
            case -1:
                fatal("Could not vfork pppd process");
#else
        ...
        char **envp, int pty_fd, int gre_fd)
{
    ...
    for (i = 0; i < NUM_TRIES; i++) {
        if (connect(fd, (struct sockaddr *) &where, sizeof(where)) < 0) {
            ...
            /* fork/vfork and launch call manager process */
#ifdef  __uClinux__
            switch (pid = vfork()) {
                case -1: /* failure */
                    fatal("vfork() to launch call manager failed.");
#else
	    switch (pid = fork()) {
                case -1: /* failure */
