...
// 2010.01.17 support forward
int forward = 0;
uint32 br0_src=0xc0a800ff;
...
// 2010.01.17 support forward
	...
	fprintf(stdout, "******* igmpproxy: starting ********\n");
...
// try find ip 
//							forward = 1;
             if( forward && !strcmp(Dp->Name,"br0") ) {
                 struct SubnetList *currSubnet;
                 for(currSubnet = Dp->allowednets; currSubnet; currSubnet = currSubnet->next) {
                     //log_msg(LOG_DEBUG, 0, "        Network for [%s] : %s",
                     //Dp->Name,
                     //inetFmts(currSubnet->subnet_addr, currSubnet->subnet_mask, s1));
                 
                 // 
                     br0_src = currSubnet->subnet_addr;
                     //int bits = ffs(ntohl(currSubnet->subnet_mask)) - 1;
                     
                     int broadcast = ntohl(currSubnet->subnet_mask);
                     broadcast ^= 0xffffffff;
                     //broadcast = -broadcast;
                     br0_src |= broadcast;
                     //printf(" %x %x %x %x \n", currSubnet->subnet_addr,currSubnet->subnet_mask, bits, broadcast);                   
                     //log_msg(LOG_DEBUG, 0," [%d] \n", 2^bits -1 );
                     printf(" Forward ip = %x\n",br0_src);
                 }                 
              }
...
#ifdef RPPPOE                
		if(strstr(Dp->Name, "ppp") && russia_mode)
			continue;
		else{
#endif		
                addVIF( Dp );
		...
		/*
            Description: Change LOG_ERR to LOG_WARNING, because LOG_ERR will exit igmpproxy and 
                         DUT can't forward the LAN-to-LAN multicast packets
        */
        if(vifcount < 2 || ((upStreamVif[0] < 0) && (upStreamVif[1] < 0))) {
            log_msg(LOG_WARNING, 0, "There must be at least 2 Vif's where one is upstream.");
        }
    }  
...
//
            if( difftime.tv_sec < 0)
                difftime.tv_sec = 0;
//
            if (secs == -1 && difftime.tv_sec > 150) {
                age_callout_queue(0);
            }
            else
            if (secs != -1 && difftime.tv_sec > (secs+1) ) {
                age_callout_queue(0);
