...
void addVIF( struct IfDesc *IfDp )
{
    ...
    if ( VifDp >= VCEP( VifDescVc ) )
        log_msg( LOG_ERR, ENOMEM, "addVIF, out of VIF space" );
    ...
    log_msg( LOG_NOTICE, 0, "adding VIF, Ix %d Fl 0x%x IP 0x%08x %s, Threshold: %d, Ratelimit: %d", 
    ...
    if ( setsockopt( MRouterFD, IPPROTO_IP, MRT_ADD_VIF, 
        ...
        log_msg( LOG_ERR, errno, "MRT_ADD_VIF" );
...
int addMRoute( struct MRouteDesc *Dp )
{
    ...
    if (    sizeof( CtlReq.mfcc_ttls ) != sizeof( Dp->TtlVc ) 
        ...
        log_msg( LOG_ERR, 0, "data types doesn't match in " __FILE__ ", source adaption needed !" );
...

    {
        ...
        log_msg( LOG_NOTICE, 0, "Adding MFC: %s -> %s, InpVIf: %d", 
    ...
    if ( setsockopt( MRouterFD, IPPROTO_IP, MRT_ADD_MFC, (void *)&CtlReq, sizeof( CtlReq ) ) )
        log_msg( LOG_WARNING, errno, "MRT_ADD_MFC" );
}
...
int delMRoute( struct MRouteDesc *Dp )
{
...

    {
        ...
        log_msg( LOG_NOTICE, 0, "Removing MFC: %s -> %s, InpVIf: %d", 
             fmtInAdr( FmtBuO, CtlReq.mfcc_origin ), 
             fmtInAdr( FmtBuM, CtlReq.mfcc_mcastgrp ),
             CtlReq.mfcc_parent == ALL_VIFS ? -1 : CtlReq.mfcc_parent
           );
