			...
			for(i=0;i<2;i++)
			{
				...
				if(upstrIf != NULL) {
					if(strcmp(inetFmt(upstrIf->InAdr.s_addr,s1),inetFmt(src,s2)) == 0){
						log_info(LOG_DEBUG,0,"Route activate request %s to %s is from myself, Ignoring it !",inetFmt(src,s1),inetFmt(dst,s2));
...
extern int forward; // = 0;
extern uint32 br0_src; //=0xc0a800ff;
            if( forward ) {
			           acceptGroupReport(br0_src, dst, IGMP_V2_MEMBERSHIP_REPORT);
            }
    ...
    case IGMP_V2_MEMBERSHIP_REPORT:
    	log_msg(LOG_NOTICE, 0, "MEMBERSHIP_REPORT RECV %s from %-15s to %s",
        igmpPacketKind(igmp->igmp_type, igmp->igmp_code),
        inetFmt(src, s1), inetFmt(dst, s2) );
    ...
    case IGMP_V2_LEAVE_GROUP:
    	log_msg(LOG_NOTICE, 0, "LEAVE_GROUP RECV %s from %-15s to %s",
        igmpPacketKind(igmp->igmp_type, igmp->igmp_code),
        inetFmt(src, s1), inetFmt(dst, s2) );
