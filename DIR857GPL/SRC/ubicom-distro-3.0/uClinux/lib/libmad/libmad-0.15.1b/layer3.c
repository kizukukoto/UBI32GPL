...
#if defined(FPM_IP7K)
#error "you selected the correct option"
  // Array to hold shifted version of X[18], so we don't have to
  // shift before every multiply using X[n] as a multiplicand
  mad_fixed_t X_arg[18];
...
// NOTE: MAD_F_ML0 defined as: ((lo)  = mad_f_mul((x), (y)))
// which is #define mad_f_mul(x, y)	mad_f_mul_ip7k(((x) >> 12) , ((y) >> 16))

// NOTE: MAD_F_MLA defined as: ((lo) += mad_f_mul((x), (y)))
// which is #define mad_f_mul(x, y)	mad_f_mul_ip7k(((x) >> 12) , ((y) >> 16))

// NOTE: MAD_F_MLZ defined as: ((void) (hi), (mad_fixed_t) (lo))
// which is the same as dumping the upper bits.

// MAD_F is defined as: #define MAD_F(x) ((mad_fixed_t) (x##L))
  ...
  asm volatile (
	  " movei  d15,   #3784               \n\t" /* d15 = 0x0ec835e8 >> 16 */
	  " muls.4 acc1,  16(%[X_arg]), d15   \n\t" /* acc1 =(X[4] >> 12) * d15 */
	  " move.4 %[lo], acc1_lo             \n\t" /* lo = (INT32)acc1 */
	  " movei  d15,   #1567               \n\t" /* d15 = 0x061f78aa >> 16 */
	  " muls.4 acc1,  52(%[X_arg]), d15   \n\t" /* acc1 =(X[13] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  : [lo] "=&d" (lo)
	  : [X_arg] "a" (X_arg)
  ...
  asm volatile (
	  ...
	  " add.4  %[lo], acc1_lo, %[lo] \n\t" /* lo += (INT32)acc1 */
	  : [lo] "+d" (lo)
	  : [t8] "d" (t8), [t9] "d" (t9), [t10] "d" (t10), [t11] "d" (t11)
	  : "d15", "d14", "acc1_hi", "acc1_lo", "cc"
  );
#endif
  x[7]  = MAD_F_MLZ(hi, lo);
  ...
  asm volatile (
	  " movei  d15,   #-2494              \n\t" /* d15 = -0x09bd7ca0 >> 16 */
	  " muls.4 acc1,  4(%[X_arg]),  d15   \n\t" /* acc1 = (X[1] >> 12) * d15 */
	  " move.4 %[lo], acc1_lo             \n\t" /* lo = (INT32)acc1 */
	  " movei  d15,   #534                \n\t" /* d15 = 0x0216a2a2 >> 16 */
	  " muls.4 acc1,  28(%[X_arg]), d15   \n\t" /* acc1 = (X[7] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #-4061              \n\t" /* d15 = -0x0fdcf549 >> 16 */
	  " muls.4 acc1,  40(%[X_arg]), d15   \n\t" /* acc1 = (X[10] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #3249               \n\t" /* d15 = 0x0cb19346 >> 16 */
	  " muls.4 acc1,  64(%[X_arg]), d15   \n\t" /* acc1 = (X[16] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  : [lo] "=&d" (lo)
	  : [X_arg] "a" (X_arg)
	  : "d15", "acc1_hi", "acc1_lo", "cc"
  ...
  asm volatile (
	  " movei  d15,   #886                \n\t" /* d15 = 0x03768962 >> 16 */
	  " muls.4 acc1,  0(%[X_arg]),  d15   \n\t" /* acc1 = (X[0] >> 12) * d15 */
	  " move.4 %[lo], acc1_lo             \n\t" /* lo = (INT32)acc1 */
	  " movei  d15,   #3633               \n\t" /* d15 = 0x0e313245 >> 16 */
	  " muls.4 acc1,  8(%[X_arg]),  d15   \n\t" /* acc1 = (X[2] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #-4093              \n\t" /* d15 = -0x0ffc19fd >> 16 */
	  " muls.4 acc1,  12(%[X_arg]), d15   \n\t" /* acc1 = (X[3] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #-2768              \n\t" /* d15 = -0x0acf37ad >> 16 */
	  " muls.4 acc1,  20(%[X_arg]), d15   \n\t" /* acc1 = (X[3] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #1231               \n\t" /* d15 = 0x04cfb0e2 >> 16 */
	  " muls.4 acc1,  24(%[X_arg]), d15   \n\t" /* acc1 = (X[6] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #-2201              \n\t" /* d15 = -0x0898c779 >> 16 */
	  " muls.4 acc1,  32(%[X_arg]), d15   \n\t" /* acc1 = (X[8] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #3454               \n\t" /* d15 = 0x0d7e8807 >> 16 */
	  " muls.4 acc1,  36(%[X_arg]), d15   \n\t" /* acc1 = (X[9] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #3906               \n\t" /* d15 = 0x0f426cb5 >> 16 */
	  " muls.4 acc1,  44(%[X_arg]), d15   \n\t" /* acc1 = (X[11] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #-3020              \n\t" /* d15 = -0x0bcbe352 >> 16 */
	  " muls.4 acc1,  48(%[X_arg]), d15   \n\t" /* acc1 = (X[12] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #178                \n\t" /* d15 = 0x00b2aa3e >> 16 */
	  " muls.4 acc1,  56(%[X_arg]), d15   \n\t" /* acc1 = (X[14] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #-1892              \n\t" /* d15 = -0x07635284 >> 16 */
	  " muls.4 acc1,  60(%[X_arg]), d15   \n\t" /* acc1 = (X[15] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  " movei  d15,   #-3999              \n\t" /* d15 = -0x0f9ee890 >> 16 */
	  " muls.4 acc1,  68(%[X_arg]), d15   \n\t" /* acc1 = (X[17] >> 12) * d15 */
	  " add.4  %[lo], acc1_lo,      %[lo] \n\t" /* lo += (INT32)acc1 */
	  : [lo] "=&d" (lo)
	  : [X_arg] "a" (X_arg)
	  : "d15", "acc1_hi", "acc1_lo", "cc"
