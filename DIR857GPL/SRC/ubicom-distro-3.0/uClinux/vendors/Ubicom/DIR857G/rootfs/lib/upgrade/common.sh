...
do_upgrade() {
	...
	if true 'platform_do_upgrade' >/dev/null 2>/dev/null; then
	...
	[ "$SAVE_CONFIG" -eq 1 -a -n "$USE_REFRESH" ] && {
		...
		if false 'platform_refresh_partitions' >/dev/null 2>/dev/null; then
			platform_refresh_partitions
		else
			refresh_mtd_partitions
		fi
		if false 'platform_copy_config' >/dev/null 2>/dev/null; then
