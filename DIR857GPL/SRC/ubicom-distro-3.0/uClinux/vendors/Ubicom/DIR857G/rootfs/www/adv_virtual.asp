...
<script language="JavaScript">
    ...
    function check_virtual_port(index, chk_port, chk_ip, obj_word, public_flag)
    {
        ...
        for (ii=(index+1); ii<rule_max_num; ii++) {
            if (get_by_id("enable" + ii).checked == true) {
                if (get_by_id("public_portS" + ii).value == chk_port && public_flag) {
					alert(obj_word +" "+TEXT009);
                    return false;
                }
		/*
                if (get_by_id("ip" + ii).value == chk_ip && !public_flag) {   //check IP address is different or not?
                    if (get_by_id("private_portS" + ii).value == chk_port) {
					alert(obj_word +" "+TEXT010);
                        return false;
                    }
                }
		*/
    ...
    function send_request()
    {
            ...
            if (temp_port_name != "") {
                for (var j = i+1; j < rule_max_num; j++) {
                    if (temp_port_name == get_by_id("name" + j).value) {
						alert(sp_name+" "+ get_by_id("name" + j).value + " "+sp_alreadyused);	//av_alert_16
                        return false;
                    }
                }
            }
        ...
        for (var i = 0; i < rule_max_num; i++) {
			...
			var ip_addr_msg = replace_msg(all_ip_addr_msg,help256);
            ...
            if (get_by_id("name" + i).value != "") {
                if (Find_word(check_name,"'") || Find_word(check_name,'"') || Find_word(check_name,"/")) {
					alert(TEXT007a+" "+ i +" "+TEXT007b);
                    return false;
                }

                //check dhcp ip range equal to lan-ip or not?
                if (!check_LAN_ip(temp_lan_ip_obj.addr, temp_ip_obj.addr, help256)){
                    return false;
                }

				if (!is_ipv4_valid(ip)) {
                      alert(KR9+" '"+ get_by_id("name" + i).value + "'.");
                    return false;
                }

                if (get_by_id("name" + i).value =="Wake-On-LAN"){
            		if (!check_wake_address(temp_ip_obj)){
		                return false;
		        	}
		        }
		        else {
		  		...
		  		if (!check_domain(temp_lan_ip_obj, temp_mask_obj, temp_ip_obj)) {
		  			alert("IP address for '" + check_name + "' should be within LAN subnet");
					return false;
				}
                ...
                if (protocol != 3) {
                    if (get_by_id("enable" + i).checked == true) {
                        if (!check_forward_port(protocol, get_by_id("public_portS" + i).value, _vs_port)) {
                            return false;
                        }
                        if (!check_app_port(protocol, get_by_id("public_portS" + i).value, _vs_port)) {
                            return false;
                        }
                        if (!check_virtual_port(i, get_by_id("public_portS" + i).value, get_by_id("ip" + i).value, _vs_port, 1)) {
                            return false;
                        }
                        if (!check_virtual_port(i, get_by_id("private_portS" + i).value, get_by_id("ip" + i).value,_vs_port, 0)) {
                            return false;
                        }
                    }
			...
			if(trim_string(get_by_id("ip" + i).value) == "")
				get_by_id("ip" + i).value = "0.0.0.0";
 ...
 <LI><div><a href="adv_ipv6_firewall.asp" onclick="return jump_if();"><script>show_words(Tag05762)</script></a></div></LI>
                      ...
                      <tbody>
                        ...
                        <script>
                            ...
                            for(var i=0 ; i<rule_max_num ; i++){
								...
								document.write('<option value=6 selected>'+_TCP+'</option>');
                                ...
                                document.write("</td>");
								document.write('<td align=center>'+INBOUND_FILTER+'<br>');
            ...
            <td id=help_text><b><strong>
              <script>show_words(_hints)</script>
                ...
                <script>show_words(hhav_name)</script>
              </p>
