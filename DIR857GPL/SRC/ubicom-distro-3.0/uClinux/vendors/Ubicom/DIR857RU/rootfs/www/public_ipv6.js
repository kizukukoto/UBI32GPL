...
function check_ipv6_address(my_obj,strFind){
	...
	if(strFind == "::"){ 		
		if (my_obj.addr.length == 2){ //Ê™¢Êü•?ØÂê¶???ãÂ?Â°?			
			//check the ip is not multicast IP (FFxx:0:0:0:0:0:0:2 or ffxx:0:0:0:0:0:0:2) ‰∏çÂèØ?âff or FF ?ãÈ†≠
			...
			for(var i = 0; i < 2; i++){	//?©Áî®'::' ?ÜÊ??çÂ?ÊÆ???ÂæåÂ?ÊÆ?			
				ip_temp = ip[i].split(":");
		...
		if (my_obj.addr.length == 8){ //Ê™¢Êü•?ØÂê¶???ãÂ?Â°?			
			//check the ip is not multicast IP (FFxx:0:0:0:0:0:0:2 or ffxx:0:0:0:0:0:0:2) ‰∏çÂèØ?âff or FF ?ãÈ†≠
			if(ip[0].charAt(0) =="f" || ip[0].charAt(0) =="F"){
				if(ip[0].charAt(1) =="f" || ip[0].charAt(1) =="F"){
					alert(my_obj.e_msg[IPv6_MULTICASE_IP_ERROR]);
...
function check_ipv6_chk_address(my_obj,strFind){
	...
	if(strFind == "::"){ 		
		...
		if (my_obj.addr.length == 2){ //			
			...
			//check the ip is not multicast IP (FFxx:0:0:0:0:0:0:2 or ffxx:0:0:0:0:0:0:2) 
			...
			// check the ip cannot be 2002:: , ff00:: , fe80:: 

			if((my_obj.addr[0] == "2002" ||my_obj.addr[0] == "ff00"||my_obj.addr[0] == "fe80")){
			...
			for(var i = 0; i < 2; i++){				
				...
				for(var index =0; index < ip_temp.length; index++){
					...
					if((ip_temp[0] == "2002" ||ip_temp[0] == "ff00"||ip_temp[0] == "fe80")){
