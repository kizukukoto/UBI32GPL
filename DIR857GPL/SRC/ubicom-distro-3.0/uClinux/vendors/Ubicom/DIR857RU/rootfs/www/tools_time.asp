...
<script language="JavaScript">
    ...
    function send_request()
    {
        ...
        if (time_type.checked== false) {
        	...
        	var ntp_s = get_by_id("ntp_server").value;
            if (trim_string(ntp_s) == "") {
				alert(YM185);
                return false;
            }else{
				if(Find_word(ntp_s,'"') || Find_word(ntp_s,"<") || Find_word(ntp_s,">") || Find_word(ntp_s,"\\")){
					alert(TEXT003.replace("+ i +","'" + ntp_s + "'"));
					get_by_id("ntp_server").value = "" + ntp_s + "";
					return false;
				}
			}
        }
