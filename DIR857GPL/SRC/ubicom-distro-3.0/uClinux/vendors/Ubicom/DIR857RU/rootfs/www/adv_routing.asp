			...
			if (get_by_id("Sub_gateway" + i).value == "")
					...
					if(get_checked_value(get_by_id("enable"+i))=="1"){
						if(trim_string(get_by_id("name"+i).value) == ""){
							...
							if(Find_word(check_name,"'") || Find_word(check_name,'"') || Find_word(check_name,"/") || Find_word(check_name,">")){
							alert(TEXT003.replace("+ i +","'" + check_name + "'"));
							get_by_id("name"+i).value = "" + check_name + "";
							return false;
						    }
						}
						for (jj=i+1; jj<rule_max_num; jj++) {
							if (get_by_id("Destination" + jj).value != "0.0.0.0") {
							    if ((get_by_id("Destination" + jj).value == static_ip) && (get_by_id("interface" + jj).value == get_by_id("interface" + i).value)) {
							        alert(_r_alert_new1+", '"+ get_by_id("name" + jj).value + "'" + help264 +" '" + get_by_id("name" + i).value + "'." );
							        return false;
							       }
							    }
							    if(get_by_id("name" +i).value == get_by_id("name" +jj).value){
							        alert(av_alert_16.replace("+ data.virtual_servers[j].entry_name +",get_by_id("name"+i).value));
							        return false;
							    }
							}
							
						if (check_domain(static_ip_obj, static_mask_obj, static_gateway_obj)) {
						alert(GW_ROUTES_GATEWAY_SUBNET_SAME.replace("%v",static_mask));
						return false;
					}

						if ((check_domain(temp_lan_obj, temp_mask_obj, static_ip_obj)) || 
						    (check_domain(temp_lan_obj, temp_mask_obj, static_gateway_obj))) {
							alert(GW_ROUTES_GATEWAY_SUBNET_SAME.replace("%v",mask));
							return false;
						}
					}
