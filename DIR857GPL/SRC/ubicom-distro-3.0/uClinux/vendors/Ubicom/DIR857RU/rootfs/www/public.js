...
function check_ip4(ip4){
	...
	if (ip4.ip == ""){
		alert(ip4.e_msg1);
		return false;
	}else if (!check_positive_integer(ip4.ip) || temp_ip.length > 1 || parseInt(ip4.ip) < ip4.min_range || parseInt(ip4.ip) > ip4.max_range){
		alert(ip4.e_msg2);
		return false;
	}
	return true;
...
function check_key_1(){
	...
	for(var i = 1; i < 2; i++){
			...
			if (wep_def_key[i-1].selectedIndex){
		    	...
		    	if (key.length != wep_key_len && key.length != hex_len){
			    		//alert(TEXT041_1+" " + [i-5] + " "+TEXT041_2+" " + wep_key_len + " "+TEXT041_3+" " + hex_len + " "+TEXT041_4);//TEXT041
			    		alert(TEXT041_1+" "+TEXT041_2+" " + wep_key_len + " "+TEXT041_3+" " + hex_len + " "+TEXT041_4);//TEXT041
			    		return false;
		    	}else if(key.length == hex_len){
			      	for (var j = 0; j < key.length; j++){
			      		if (!check_hex(key.substring(j, j+1))){
			      			alert(TEXT042_1+" " + [i-5] + " "+TEXT042_2);//TEXT042
			      			return false;
			      		}
			      	}
			      	if(i == def_key)
...
function check_integer(which_value, min, max){
	...
	if (temp_obj == "" || temp_obj.length > 1 || !check_positive_integer(which_value)){
		return false;
...
function check_ip_range(order, my_obj, mask){
	...
	if (!check_positive_integer(which_ip) || which_ip == "" || which_ip.length > 1 || (which_ip[0].length > 1 && which_ip[0].substring(0,1) == "0")){	// if the address is invalid
...
function check_port(port){
    ...
    if (!check_positive_integer(port) || port == "" || temp_port.length > 1
...
function check_varible(obj){
	...
	if (temp_obj == "" || temp_obj.length > 1){
		...
		return false;
	}else if (!check_positive_integer(obj.var_value)){
...
function page_cancel(form_name, redirect_url){
	if (is_form_modified(form_name) && confirm (up_fm_dc_1)) {
		if (redirect_url == "sel_wan.asp")
			window.location.href='sel_wan.asp';
		else {
...
function check_reboot(){
	var str;
	if (reboot_needed == "1") {
		str = "<input name=\"reboot_now\" id=\"reboot_now\" type=\"button\" class=button_submit value=\"\" onclick=\"jump_reboot();\">";
...
function reboot_form(){
	var str;
	if (reboot_needed == "1") {
		str = "<form id=\"form_reboot\" name=\"form_reboot\" method=\"post\" action=\"reboot.cgi\">";
		str += "<input type=\"hidden\" name=\"html_response_page\" value=\"reboot.asp\">";
		str += "</form>";
		document.write(str);
	}
}
...
//Light On 0:INTERNET 1:WIRELESS SETTINGS 2:NETWORK_SETTINGS 3:USB SETTING 4:USB STORAGE 5:MEDIA SERVER 6:IPV6 7:PARENTAL CONTROL
function show_side_bar(page_num)
{
	var sidebar_column = 8;
	var sidebar=new Array2DVar(sidebar_column,2);

	sidebar[0][0] = "<li><div id=\"sidenavoff\"><script>show_words(sa_Internet)</script></div></li>";
	sidebar[0][1] = "<li><div id=\"sidenavon\"><a href=\"index.asp\" onclick=\"return jump_if();\"><script>show_words(sa_Internet)</script></a></div></li>";
	sidebar[1][0] = "<li><div id=\"sidenavoff\"><script>show_words(_wirelesst)</script></div></li>";
	sidebar[1][1] = "<li><div id=\"sidenavon\"><a href=\"wizard_wireless.asp\" onclick=\"return jump_if();\"><script>show_words(_wirelesst)</script></a></div></li>";
	sidebar[2][0] = "<li><div id=\"sidenavoff\"><script>show_words(bln_title_NetSt)</script></div></li>"
	sidebar[2][1] = "<li><div id=\"sidenavon\"><a href=\"lan.asp\" onclick=\"return jump_if();\"><script>show_words(bln_title_NetSt)</script></a></div></li>"
	sidebar[3][0] = "<!--<li><div id=\"sidenavoff\"><script>show_words(bln_title_usb)</script></div></li>-->"
	sidebar[3][1] = "<!--<li><div id=\"sidenavon\"><a href=\"usb_setting.asp\" onclick=\"return jump_if();\"><script>show_words(bln_title_usb)</script></a></div></li>-->"
	sidebar[4][0] = "<li><div id=\"sidenavoff\"><script>show_words(_media_server)</script></div></li>"
	sidebar[4][1] = "<li><div id=\"sidenavon\"><a href=\"media_server.asp\" onclick=\"return jump_if();\"><script>show_words(_media_server)</script></a></div></li>"
	sidebar[5][0] = "<li><div id=\"sidenavoff\"><script>show_words(STORAGE_title)</script></div></li>"
	sidebar[5][1] = "<li><div id=\"sidenavon\"><a href=\"http_storage.asp\" onclick=\"return jump_if();\"><script>show_words(STORAGE_title)</script></a></div></li>"
	sidebar[6][0] = "<li><div id=\"sidenavoff\">IPv6</div></li>"
	sidebar[6][1] = "<li><div id=\"sidenavon\"><a href=\"ipv6.asp\" onclick=\"return jump_if();\">IPv6</a></div></li>"
	sidebar[7][0] = "<li><div id=\"sidenavoff\"><script>show_words(_parental_ctl)</script></div></li>"
	sidebar[7][1] = "<li><div id=\"sidenavon\"><a href=\"dns_setting.asp\" onclick=\"return jump_if();\"><script>show_words(_parental_ctl)</script></a></div></li>"
...
//<!--<li><div id="sidenavon"><a href="usb_storage.asp" onclick="return jump_if();"><script>show_words(usb_storage)</script></a></div></li>-->
//<li><div id="sidenavon"><a href="media_server.asp" onclick="return jump_if();"><script>show_words(_media_server)</script></a></div></li>
//<li><div id="sidenavon"><a href="ipv6.asp" onclick="return jump_if();">IPV6</a></div></li>
//<li><div id="sidenavon"><a href="dns_setting.asp" onclick="return jump_if();"><script>show_words(_parental_ctl)</script></a></div></li>

	for(var i=0; i<sidebar_column; i++){
		if(i==page_num){
			document.write(sidebar[i][0]);
		}
		else{
			document.write(sidebar[i][1]);
		}
	}
...
function check_russia_mask(my_mask)
{ 
	var temp_mask = my_mask.addr; 
	if (temp_mask.length == 4){ 
		for (var i = 0; i < temp_mask.length; i++){ 
