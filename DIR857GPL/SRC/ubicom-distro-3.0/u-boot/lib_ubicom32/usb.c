...
static void board_usb_reset(void)
{
	 ...
	 */
	struct ubicom32_io_port *io_port = UBICOM32_IO_PORT(RJ);
	io_port->function =
			IO_FUNC_FUNCTION_RESET(1) | IO_FUNC_FUNCTION_RESET(2) |
			IO_FUNC_FUNCTION_RESET(3) | IO_FUNC_FUNCTION_RESET(4);
	io_port->ctl0 = 0;
	ubicom32_disable_interrupt(PORT_OTHER_INT(RJ));
	ubicom32_clear_interrupt(PORT_OTHER_INT(RJ));
...
int board_usb_init(void)
{
	...
	ubicom32_enable_interrupt(PORT_OTHER_INT(RJ));
	return 0;
}

#else

#error No USB Initialization code present in lib_ubicom32/usb.c for this architecture
