...
#define MIN_SAMPLE_CLOCKS 500000000
...
bool profile::init_all_data()
{
	pid_pending = -1;
	d_access_per_sec = 0;
	bad_mapped_sample_count = 0;
	bad_unmapped_sample_count = 0;
	d_miss_per_sec = 0;
	d_buffer_per_sec = 0;
	i_miss_per_sec = 0;
	...
	for (i = 0; i < NUMTHREADS; ++i) {
		...
		last_instruction_count[i] = 0;
		current_inst_count[i] = 0;
		hazard_count[i] = 0.0;
		inst_counted[i] = 0.0;
		recent_mips[i] = 0;
		last_recent_mips[i] = 0;
...
void profile::add_call(unsigned int p_adr, unsigned int s_adr, unsigned int s_pid, double freq, bool reliable)
{
	for (unsigned int i = 0; i < next_cg; ++i) {
		if (cg[i].parent == p_adr && cg[i].self == s_adr && cg[i].pid == s_pid) {
			cg[i].count += freq;
			return;
		}
	}
	...
	cg[next_cg].pid = s_pid;
	cg[next_cg].count = freq;
	cg[next_cg].reliable = reliable;
...
int profile::call_valid(unsigned int ret_addr, unsigned int pc, unsigned int pid, unsigned int func_start, int type) 
{
	...
	int parent_type;
	struct symbol *sym = mem.find_symbol_by_address(parent, pid, &parent_func_start, &parent_type);
	if (sym == NULL) {
		return CALL_INVALID;
	}
	...
	if (opcode == 0x1b) {		/* call */
		...
		unsigned int target = parent + offset24(inst_parent);
		if (!mem.address_valid_text(target, pid)) {
			return CALL_INVALID;
		}
		unsigned int inst_target = mem.pmem(target, pid);
		if ((inst_target & 0xf8000000) == 0 && (inst_target & 0xf800) == 0xf000) {
			return CALL_VALID;	/* call target is a pdec, so it's in the PLT TODO: long PLTs and validate call address somehow */
		}
		return target == func_start ? CALL_VALID : CALL_POSSIBLE;	/* possible when there is a tail call optimization */
	}
...
void profile::call_graph(struct profile_sample sample, double freq)
{
	...
	struct symbol * sym = mem.find_symbol_by_address(sample.pc, sample.pid, &func_start, &type);
	if (sym == NULL) {	
		return;		// can't find the address - KLM is not reloacated in elf file so no way to validate the address
	}

	valid = call_valid(sample.a5, sample.pc, sample.pid, func_start, type);
	if (valid == CALL_VALID) {
		add_call(sample.a5 - 4, func_start, sample.pid, freq, true);
		return;
	} else if (!have_possible && valid == CALL_POSSIBLE) {
		from = sample.a5 - 4;
		have_possible = true;
	}
	for (int i = 0; i < PROFILE_STACK_WORDS; ++i) {
		valid = call_valid(sample.parent[i], sample.pc, sample.pid, func_start, type);
		if (valid == CALL_VALID) {
			add_call(sample.parent[i] - 4, func_start, sample.pid, freq, true);
			return;
		} else if (!have_possible && valid == CALL_POSSIBLE) {
			from = sample.parent[i] - 4;
			have_possible = true;
		}
	}
	if (have_possible) {
		add_call(from, func_start, sample.pid, freq, false);
	} else {
		add_call(0, func_start, sample.pid, freq, false);
	}
}
...
bool profile::symbol_address_to_name(unsigned int pc, unsigned int pid, char *buf)
{
	unsigned int func_start;
	int type;
	struct symbol *sym = mem.find_symbol_by_address(pc, pid, &func_start, &type);
	if (sym == NULL) {
...
// index is index of this instruciton in seg
// not: jmp (not jmpt), calli as return, or ret
// and update next value for unconditional next index or conditional jmp target
// next can point to index

bool profile::next_pc(memblock *seg, unsigned int index, int type, unsigned int *next) 
{
	unsigned int inst = seg->pmem[index];
	if (type == INST_JMP || type == INST_RET) {	// conditional jmp or ret
		*next = index + offset21(inst) / 4;
		return false;
	}
	if (type == INST_CALLI) {		// is it a call?
		unsigned int prev = seg->pmem[index - 1];
...
void profile::on_sample(struct profile_sample &sample) 
{
	...
	if (!mem.address_valid(sample.pc, sample.pid)) {
		...
		if (sample.active & (1 << thread)) {
			if (sample.pid == 0)
				bad_unmapped_sample_count++;		// unknown addresses in active threads
			else
				bad_mapped_sample_count++;		// unknown addresses in active threads
		}
	}
	...
	if (thread_active) { 
		...
		unsigned int instruction = mem.pmem(sample.pc, pid);
		int inst_type = decode_instruction(instruction);
		mem.inst_type(sample.pc, sample.pid) = inst_type;
		int src = idata[inst_type].src && ((instruction & 0x400) || (instruction & 0x200));
		int dst = idata[inst_type].dst && ((instruction & 0x04000000) || (instruction & 0x2000000));
		...
		if (idata[inst_type].illegal) {
			...
			illegal_value = instruction;
			illegal_pid = pid;
		}
		icount[inst_type] += freq;
		if (inst_type == INST_JMP && type[thread] != T_HRT) {	//no condition code for HRT threads
			int taken = decode_condition(instruction, sample.cond_codes);
			int pbit = (instruction >> 22) & 1;
			jmpcounts[taken * 2 + pbit] += freq;
			int dist = instruction & 0x1fffff;
			if (dist <= 3) {
		...
		if (inst_type == INST_MULU4 || inst_type == INST_MULS4) {
			...
			int accumulator = (instruction >> 16) & 1;
		...
		int srcan = (instruction >> 5) & 0x7;
		int dstan = (instruction >> 21) & 0x7;
		if (src) {
			if (instruction & 0x400) {
				am_offset_count[thread] += freq;
			} else if ((instruction & 0x300) == 0x300) {
				am_index_count[thread] += freq;
			} else if ((instruction & 0x210) == 0x210) {
				am_pre_count[thread] += freq;
			} else if ((instruction & 0x210) == 0x200) {
				am_post_count[thread] += freq;
			}
		}
		if (dst) {
			if (instruction & 0x4000000) {
				am_offset_count[thread] += freq;
			} else if ((instruction & 0x3000000) == 0x3000000) {
				am_index_count[thread] += freq;
			} else if ((instruction & 0x2100000) == 0x2100000) {
				am_pre_count[thread] += freq;
			} else if ((instruction & 0x2100000) == 0x2000000) {
				am_post_count[thread] += freq;
			}
		}
		if (src && dst) {
...
void profile::on_receive(const char *buf, const char *ip_address)
{
	...
	if (console_state == CONSOLE_MAPPING) {
		 ...
		 */
		if (!mem.parse_map((char *)buf, &pid_pending, current_time, total_rate)) {
			/*
 ...
 */
bool profile::on_packet(char *buf, unsigned int size)
{
	if (start_time == 0) {
		return false;		// ignore packets before first interval starts
	}
	if (elf_file_count == 0) {
		return false;		// ignore if we don't have an elf file
	}
	last_packet_time = time(NULL);
	...
	if (receive_version != PROFILE_VERSION) {
		...
		if (receive_version < 17) {
			return false;		// version 4-12 are forward compatible with each other and 13 and above are forward compatible, ver 16 breaks compatibility due to thread count change.  ver 17 breaks compatibility on sample format.
		}
	} 
	...
	if (current_clock == 0) {
		...
		return false;		// wait until next sample for consistent data
	}
	...
	stack_words = ntohl(header.sample_stack_words);
	if (stack_words != PROFILE_STACK_WORDS) {
		error = ERROR_BAD_VERSION;
		return false;
	}
	...
	for (int i = 0; i < header.sample_count; ++i) {
		...
		for (int j = 0; j < stack_words; ++j) {
			sample.parent[j] = ntohl(sample.parent[j]);
		}
...
int profile::read_posix_elf_file(const char *lpszPathName)
{
	...
	for (int snum = 0; snum < count && snum < MAX_SECTIONS; ++snum) {
		...
		if (elf_header.e_type == ELF_TYPE_REL && !strstr(name, "ocm_text")) {
			m = mem.add_memblock(section.sh_addr + section_offset, section.sh_size, data, seg_name, type);
			section_offset += length;
		} else {
			m = mem.add_memblock(section.sh_addr, section.sh_size, data, seg_name, type);
		}
		if (type == MEM_TYPE_LINUX) {
			mem.add_memory_seg(0, m, section.sh_addr, section.sh_addr + section.sh_size, seg_name);	// add an unmapped segment
		}
...
int profile::read_elf_file(const char *lpszPathName)
{
	...
	mem.add_memory_map(0, "Unmapped memory (Ultra, Kernel, etc)", current_time, 0);
	...
	for (int snum = 0; snum < count; ++snum) {
		...
		memblock *m = mem.add_memblock(section.sh_addr, section.sh_size, data, seg_name, MEM_TYPE_ULTRA);
		mem.add_memory_seg(0, m, section.sh_addr, section.sh_addr + section.sh_size, seg_name);
		if (strcmp(name, ".text") == 0 || strcmp(name, ".ocm_text") == 0) {
...
bool profile::save_hazards(int count, const char *filename, int page)
{
	FILE *f;
	if (filename != NULL) {
		f = fopen(filename, "w");
	...
	for (memblock *p = mem.get_first_memblock(); p; p = p->next) {
...
struct funct {
	...
	double called_count[FUNCT_CALLERS];	// number of callers for each recorded caller	
	int cgi[FUNCT_CALLERS];		// cg index of recorded caller 
	int pram;			// is function in OCM?
	int type;
...
struct oneloop {	// one per loop for BTB analysis
	double hazmips;
	memblock *seg;
	unsigned int index;	// index in segment to the instruction closing the loop
...
// find simple loops, within a text segment
...
bool profile::findloop(int lp, int type) 
{
	...
	next_pc(seg, loop[lp].index, type, &index);	// get target
	int count = 1;
	while (index != loop[lp].index) {
		loop[lp].numinst++;
		...
		if (t == INST_CALL) {
			...
			loop[lp].callhazmips += (double)clock_rate * seg->hazclocks[index] / itotal_nrt * instcnt / total_clocks;
			unsigned int next;
			bool valid = next_pc(seg, index, t, &next);
			stack[sp++] = index + 1;
			index = next;
			loop[lp].calls++;
			...
			if (!valid) {	// it's really a return
				...
				loop[lp].rethazmips += (double)clock_rate * seg->hazclocks[index] / itotal_nrt * instcnt / total_clocks;
				index = stack[--sp];
			} else {
				if (sp == MAXDEPTH){
					return false;
			...
			if (t == INST_JMP) {		// don't follow conditional branches, nested loops
				...
				if (seg->gprof[index] != 0) {
					taken = seg->taken[index] / seg->gprof[index];
				}
				if (sp == 0 && next >= loop[lp].index + 1) {		// early exit
					fall *= (1 - taken);
					index++;
				} else if (!backward && taken > 0.5) {
					loop[lp].fwdtakenmips += (double)clock_rate * seg->hazclocks[index] / itotal_nrt * instcnt / total_clocks;
	...
	if (seg->gprof[loop[lp].index] > 0.1) {
		double untaken = 1.0 - (seg->taken[loop[lp].index] / seg->gprof[loop[lp].index]) * fall;
		loop[lp].untaken = seg->gprof[loop[lp].index] - seg->taken[loop[lp].index];
...
double profile::get_callers(unsigned int address, int *cgi, double *called_count)
{
	double called = 0.0;
	for (int i = 0; i < FUNCT_CALLERS; ++i) {
		cgi[i] = -1;
		called_count[i] = 0.;
	}
	for (int i = 0; i < next_cg; ++i) {
		if (mem.translate(cg[i].pid, cg[i].self) == address) {
			called += cg[i].count;
			if (cg[i].count > called_count[FUNCT_CALLERS - 1]) {
				called_count[FUNCT_CALLERS - 1] = cg[i].count;
				cgi[FUNCT_CALLERS - 1] = i;
			}
			for (int j = FUNCT_CALLERS - 2; j >= 0; j--) {
				if (called_count[j + 1] > called_count[j]) {
					double td = called_count[j + 1];
					called_count[j + 1] = called_count[j];
					called_count[j] = td;
					int ti = cgi[j + 1];
					cgi[j + 1] = cgi[j];
					cgi[j] = ti;
...
void profile::caller_format(char *buf, int *cgi)
{
	char buf2[1000];
	char buf3[200];
	buf[0] = 0;
	if (cg[cgi[0]].parent == 0 && cgi[0] != -1) {
		sprintf(buf2, " (%1.0f %s)", cg[cgi[0]].count, "???");
		strcat(buf, buf2);
	}
	else if (cgi[0] != -1) {
		symbol_address_to_name(cg[cgi[0]].parent, cg[cgi[0]].pid, buf3);
		sprintf(buf2, " (%1.0f%s%s)", cg[cgi[0]].count,  cg[cgi[0]].reliable? " " : "*", 
			buf3);
		strcat(buf, buf2);
	} else { 
		strcat(buf, "                                    ");
	...
	if (cg[cgi[1]].parent == 0 && cgi[1] != -1) {
		sprintf(buf2, " (%1.0f %s)", cg[cgi[1]].count, "???");
		strcat(buf, buf2);
	}
	else if (cgi[1] != -1) {
		symbol_address_to_name(cg[cgi[1]].parent, cg[cgi[1]].pid, buf3);
		sprintf(buf2, " (%1.0f%s%s)", cg[cgi[1]].count, cg[cgi[1]].reliable? " " : "*",
			buf3);
		strcat(buf, buf2);
	} else { 
		strcat(buf, "                                    ");
	...
	if (cg[cgi[2]].parent == 0 && cgi[2] != -1) {
		sprintf(buf2, " (%1.0f %s)", cg[cgi[2]].count, "???");
		strcat(buf, buf2);
	}
	else if (cgi[2] != -1) {
		symbol_address_to_name(cg[cgi[2]].parent, cg[cgi[2]].pid, buf3);
		sprintf(buf2, " (%1.0f%s%s)", cg[cgi[2]].count, cg[cgi[2]].reliable? " " : "*",
			buf3);
		strcat(buf, buf2);
	} else { 
		strcat(buf, "                                    ");
...
void profile::display_functions(int sortby, double hazard_total, __int64 total_instruction_count)
{
	...
	funct *fdata = (funct *)malloc(sizeof(funct) * MAX_DISPLAY);
	if (!fdata) {
		addline("Out of memory, malloc failed", P_FUNC);
		return;
	}
	for (memblock *m = mem.get_first_memblock(); m != NULL; m = m->next) {
		for (int sym = 0; sym < m->symbol_count && sym_cnt < MAX_DISPLAY; ++sym) {
			f[sym_cnt] = &fdata[sym_cnt];
			...
			f[sym_cnt]->pram = inpram(f[sym_cnt]->address);	// OCM addresses are not mapped, so elf file address works
			...
			f[sym_cnt]->called = get_callers(f[sym_cnt]->address, f[sym_cnt]->cgi, f[sym_cnt]->called_count);
	...
	addline("Unknown functions are caused by missing files on the command line or stripped files.", P_FUNC);
	addline("Unknown functions have unknown hazard MIPS", P_FUNC);
	addline("Unknown applications typically run for less than a second, and exit before the profiler gets their emmory map.", P_FUNC);
	addline("* indicates an uncertain caller.", P_FUNC);
	addline(" ", P_FUNC);
	...
	strcat(buf, "  Size    DRAM   OCM ");
	strcat(buf, " Sample  Function name (sampled called-from) ");
	addline(buf, P_FUNC);
	...
	for (int sym = 0; sym < sym_cnt; ++sym) {
		...
		buf[0] = 0;
		if (f[sym]->count != 0) {
			if (f[sym]->type == MEM_TYPE_UNKNOWN || f[sym]->type == MEM_TYPE_DUMMY) {
			...
			strcat(buf, buf2);
			sprintf(buf2, "%6.0f  ", f[sym]->called);
			strcat(buf, buf2);
		} else {
			unused_size += size;
		...
		if (f[sym]->called != 0 && f[sym]->type != MEM_TYPE_UNKNOWN && f[sym]->type != MEM_TYPE_DUMMY) {
			caller_format(buf2, f[sym]->cgi);
			strcat(buf, buf2);
		}
		addline(buf, P_FUNC);
	}
...
void profile::display_instructions(bool btb, double total_mips)
{
	...
	addline(buf, P_INST);
	addline(" ", P_INST);
	double mem_total = src_count + dst_count + 2. * both_count;
	double mem_ref[NUMTHREADS];
	double am_offset_total = 0;
	double am_index_total = 0;
	double am_post_total = 0;
	double am_pre_total = 0;
	sprintf(buf, "    Per Instruction  %5.2f ",
		mem_total / itotal_all);
	for (int i = 0; i < NUMTHREADS; ++i) {
		if (type[i] == T_DISABLED) {
			continue;
		}
		if (inst_counted[i] == 0.0) {
			...
			mem_ref[i] = src_count_thread[i] + dst_count_thread[i] + 2.0 * both_count_thread[i];
			sprintf(tbuf, " %6.2f% ", mem_ref[i] / inst_counted[i]);
			strcat(buf, tbuf);
			am_offset_total += am_offset_count[i];
			am_index_total += am_index_count[i];
			am_post_total += am_post_count[i];
			am_pre_total += am_pre_count[i];
		}
	}
	addline(buf, P_INST);
	addline(" ", P_INST);
	addline("Memory addressing modes (percentage of memory operands)", P_INST);
	sprintf(buf, "    Offset imm(An)  %5.1f%% ",
		100. * am_offset_total / mem_total);
	for (int i = 0; i < NUMTHREADS; ++i) {
		if (type[i] == T_DISABLED) {
			continue;
		}
		if (inst_counted[i] == 0.0) {
			...
			sprintf(tbuf, " %5.1f%%", 100. * am_offset_count[i] / mem_ref[i]);
			strcat(buf, tbuf);
		}
	}
	addline(buf, P_INST);

	sprintf(buf, "    Indexed (An,Dn) %5.1f%% ",
		100. * am_index_total / mem_total);
	for (int i = 0; i < NUMTHREADS; ++i) {
		if (type[i] == T_DISABLED) {
			continue;
		}
		if (inst_counted[i] == 0.0) {
			...
			sprintf(tbuf, " %5.1f%%", 100. * am_index_count[i] / mem_ref[i]);
			strcat(buf, tbuf);
		}
	}
	addline(buf, P_INST);

	sprintf(buf, "    Post (An)imm++  %5.1f%% ",
		100. * am_post_total / mem_total);
	for (int i = 0; i < NUMTHREADS; ++i) {
		if (type[i] == T_DISABLED) {
			continue;
		}
		if (inst_counted[i] == 0.0) {
			...
			sprintf(tbuf, " %5.1f%%", 100. * am_post_count[i] / mem_ref[i]);
			strcat(buf, tbuf);
		}
	}
	addline(buf, P_INST);

	sprintf(buf, "    Pre imm(An)++   %5.1f%% ",
		100. * am_pre_total / mem_total);
	for (int i = 0; i < NUMTHREADS; ++i) {
		if (type[i] == T_DISABLED) {
			continue;
		}
		if (inst_counted[i] == 0.0) {
			...
			sprintf(tbuf, " %5.1f%%", 100. * am_pre_count[i] / mem_ref[i]);
			strcat(buf, tbuf);
		}
	}
	addline(buf, P_INST);
	...
	for (int lp = 0; lp < 50 && lp < nextloop; ++lp) {
		...
		if (symbol_address_to_name(loop[lp].seg, loop[lp].index, tbuf)) {
			char trips[20];
...
{
	char tmp[500];
	addline("Linux application and library code segment memory maps.  Bars show relative time spent in that text segment.", P_APPS);
	addline(" ", P_APPS);

	addline("Loaded text segments from files specified on the command line.", P_APPS);
	addline(" ", P_APPS);
	...
	addline(" ", P_APPS);
	addline(" ", P_APPS);
	addline("Unmapped text segments.  Unknown functions indicates that this segment was not found in a command line elf file.", P_APPS);
	addline(" ", P_APPS);
	addline("Load address range  Size   Time    Type     Segment name", P_APPS);
	...
	for (map_segment *ms = mm0->first_seg; ms; ms = ms->next) {
		sprintf(tmp, "%08x-%08x %4d KB %5.1f%%   %s %s",
			...
			ms->seg? ms->seg->seg_name : ms->path);
	...
	addline(" ", P_APPS);
	addline(" ", P_APPS);
	addline("User space memory mapped text segments.  * indicates that the segment was not found in a command line elf file.", P_APPS);
	...
	for (memory_map *mm = mem.first_memory_map(); mm; mm = mm->next) {
		...
		for (map_segment *ms = mm->first_seg; ms; ms = ms->next) {
			...
			sprintf(tmp, "%08x-%08x%c %4d KB %5.1f%%    %s",
				ms->start_address, ms->end_address, 
				ms->seg == 0 || ms->seg->type == MEM_TYPE_DUMMY? '*' : ' ',
				(ms->end_address - ms->start_address + 511) / 1024,
				total_rate == 0.? 0.: 100. * ms->gprof / total_rate,
				ms->path);
			addbar(total_rate == 0.? 0.: 100. * ms->gprof / total_rate, P_APPS);
	...
	addline(" ", P_APPS);
	sprintf(tmp, "Tracking %d Applications", app_count);
	addline(tmp, P_APPS);
}
...
#define STAT_VIRTUAL_COUNTER 0
#define STAT_UTIL 1
#define STAT_MIPS 2
#define STAT_DMISS 3
#define STAT_IMISS 4
#define STAT_DBUFFER 5
//#define STAT_CLOCKS 6
...
//{	STAT_CLOCKS,				1,	1000,	"M Clocks last second",	},
};
...
void profile::display_stats(void)
{
	...
	for (int i = 0; i < stats_count; ++i) {
		...
		if (stats_last_val[i] != 0 && stats_time[i] != stats_last_time[i]) {
			...
			gvals[j + 1] = last_gvals[j + 1];	// if no data was received, display the same rate as last time
		}
	...
	for (int j = 0; j < FIXED_STAT_DISPLAY + stats_count * 2; ++j) {
		...
		if (stat_data[j].name == STAT_DBUFFER) {
			gvals[j] = d_buffer_per_sec;
		}
		if (stat_data[j].name == STAT_IMISS) {
			gvals[j] = i_miss_per_sec;
		}
		if (stat_data[j].name == STAT_UTIL && total_clocks >= MIN_SAMPLE_CLOCKS) {
			gvals[j] = 100. - recent_idle;
		}
//		if (stat_data[j].name == STAT_CLOCKS) {
//			gvals[j] = curr_clocks / 1000000.;
//		}
...
void profile::display_locks(void)
{
	...
	for (memblock *p = mem.get_first_memblock(); p; p = p->next) {
		if (p->type != MEM_TYPE_LINUX && p->type != MEM_TYPE_KLM) {
			continue;
		}
		for (unsigned int i = 0; i < p->size / 4; i++) {
			double cnt = p->gprof[i];
			...
			// look for bset or tbset followed by jmpne backwards one instruction
			unsigned int inst = p->pmem[i];
			...
			if (mips > MIN_LOCK_MIPS && cnt > 5 && next_lock < NUM_LOCK_DESC) {
				...
				lock_desc[next_lock].bit = bit;
				lock_desc[next_lock].seg = p;
				lock_desc[next_lock].index = i;
	...
	addline(" ", P_LOCKS);
	if (next_lock == 0) {
		sprintf(buf, "No single lock uses more than %3.2f MIPS. (or too few samples)", MIN_LOCK_MIPS);
		addline(buf, P_LOCKS);
		return;
	}

	addline("Top locks by time used (with enough samples seen).  Shows samples in this function from each caller.", P_LOCKS);
	qsort(lock_desc, next_lock, sizeof(lock_desc[0]), lock_compare);

	for (int i = 0; i < next_lock && i < 20; ++i) {
		unsigned int func_start;
		int type;
		lock_desc[i].seg->find_symbol_by_address(lock_desc[i].seg->start_address + lock_desc[i].index * 4, &func_start, &type);
		int cgi[FUNCT_CALLERS];
		double called_count[FUNCT_CALLERS];
		double called = get_callers(func_start, cgi, called_count);
		char buf2[1000];
		caller_format(buf2, cgi);
		symbol_address_to_name(lock_desc[i].seg, lock_desc[i].index, name);
		sprintf(buf, "%s: %s Bit %2d.  %5.3f MIPS  %4.0f samples %s.  %s", locknames[lock_desc[i].type], lock_desc[i].try_lock ? "TRY " : "LOCK", 
			lock_desc[i].bit, lock_desc[i].mips, lock_desc[i].samples + 0.99, name, buf2);
		addline(buf, P_LOCKS);
	}
	for (int i = 0; i < 20; ++i) {
		addline("                                                                                  ", P_LOCKS);
...
void profile::display_threads(double total_mips)
{
	...
	// cache statistics
	curr_clocks = total_clocks - last_total_clocks;

	if (chip_type == CHIP_7K || chip_type == CHIP_8K) {
		numthreads = 12;
	}

	// do all the rates, but only if we have reveived enough data since the last display update
	if (curr_clocks > MIN_SAMPLE_CLOCKS) {
		recent_mips[0] = clock_rate * (instruction_count[0] - last_instruction_count[0] - (profile_count - last_profile_count)) / curr_clocks;
		last_instruction_count[0] = instruction_count[0];
		last_profile_count = profile_count;
		for (int i = 1; i < NUMTHREADS; ++i) {
			recent_mips[i] = clock_rate * (instruction_count[i] - last_instruction_count[i]) / curr_clocks;
		...
		last_recent_idle = recent_idle;
		for (int i = 0; i < NUMTHREADS; ++i) {
			last_recent_mips[i] = recent_mips[i];
		}
	}
	...
	tbuf[24] = 0;
	current_time = (int)(total_clocks / clock_rate / 1000000.);
	sprintf(buf, "%5d second profile through %s.  IP%xxxx rev %x running at %3.0f MHz, with %3d MHz DDR clock.", 
		current_time, tbuf, cpu_id >> 16, cpu_id & 0xffff, clock_rate, ddr_freq/1000000);
	...
	sprintf(buf, "Received %d v%d packets and %d samples (%.0f/s). %4.1f%% (%d) dropped packets.  Received %d stat packets", 
		packet_count, 
		receive_version,
		sample_count[0], 
	...
	double bad_unmapped_rate = (double)bad_unmapped_sample_count / total_active_count;
	double bad_mapped_rate = (double)bad_mapped_sample_count / total_active_count;
	if (bad_unmapped_rate + bad_mapped_rate > 0.05) {
		sprintf(buf, "Warning: (%4.1f%% kernel/ultra, %4.1f%% user) of samples are not in a known text segment.  Check the elf file names on the command line.  Example: 0x%x",
			bad_unmapped_rate * 100., bad_mapped_rate * 100., bad_pc_value);
		addline(buf, P_SUMMARY);
	} else  if (bad_unmapped_rate + bad_mapped_rate > 0.001) {
		sprintf(buf, "(%4.1f%% kernel/ultra, %4.1f%% user) of samples are not in a known text segment.  They are lumped together as unknown_functions on the functions tab.",
			bad_unmapped_rate * 100., bad_mapped_rate * 100.);
		addline(buf, P_SUMMARY);
	} else {
		addline(" ", P_SUMMARY);
	}
	...
	for (int i = 0; i < numthreads; ++i) {
		if (type[i] == T_DISABLED) {
			continue;
		} else if(type[i] == T_GDB || sample_count[i] == 0) {
			strcat(buf, "        ");
		} else {
			sprintf(tbuf, "%6.1f%% ",
	...
	for (int i = 0; i < numthreads; ++i) {
		if (type[i] == T_DISABLED) {
			...
			strcat(buf, tbuf);
			current_mips += recent_mips[i];
		}
	}
	sprintf(tbuf, "   %7.1f", current_mips);
	strcat(buf, tbuf);
	addline(buf, P_THREAD);
	...
	strcat(buf, tbuf);
	strcat(buf," (Includes trap delivery and ultra debugger messages)");
	addline(buf, P_THREAD);
	...
	if (curr_clocks > MIN_SAMPLE_CLOCKS) {	// only take data is enough time has passed, otherwise accumulate data next sample
		d_access_per_sec = clock_rate * (perf_counters[2] - last_perf_counters[2]) / curr_clocks;
		d_miss_per_sec = clock_rate * (perf_counters[3] - last_perf_counters[3]) / curr_clocks;
		i_miss_per_sec = clock_rate * (perf_counters[1] - last_perf_counters[1]) / curr_clocks;
		d_tlb_miss_per_sec = clock_rate * (perf_counters[5] - last_perf_counters[5]) / curr_clocks;	// total misses
		i_tlb_miss_per_sec = clock_rate * (perf_counters[4] - last_perf_counters[4]) / curr_clocks;
		d_tlb_miss_per_sec -= i_tlb_miss_per_sec;
		ptec_miss_per_sec = clock_rate * (perf_counters[6] - last_perf_counters[6]) / curr_clocks;
		d_buffer_per_sec = clock_rate * (perf_counters[0] - last_perf_counters[0]) / curr_clocks;
		for (int i = 0; i < PROFILE_COUNTERS; ++i) {
			last_perf_counters[i] = perf_counters[i];
		}
	}
	...
	sprintf(buf, "Instruction cache:  %5.2f M miss/s, %5.1f any blocked per miss, %5.1f all blocked/miss, %5.1f%% utilized",
		imisspersec,
		perf_counters[1]? bps[0] * total_clocks / perf_counters[1] : 0,
		imisspersec? i_blocked_total_count / imisspersec : 0,
	...
	sprintf(buf, "D cache cancels instruction (D miss or lose arbitration) %5.2f M / second.  Cost is approximately %5.2f MIPS", 
		dnoackpersec, (nrt_active_count || nrt_active_total < nrt_active_count)? dnoackpersec * 7. / (nrt_active_total / nrt_active_count) : 0);
	addline(buf, P_THREAD);
	...
	sprintf(buf, "Data cache:         %5.2f access/inst, %6.2f%% miss rate, %5.2f M access/s, %5.2f M miss/s, %5.1f any blocked per miss, %5.1f all blocked/miss, %5.1f%% utilized",
		...
		dmisspersec,
		perf_counters[3]? bps[1] * total_clocks / perf_counters[3] : 0,
		dmisspersec? d_blocked_total_count / dmisspersec: 0,
...
#define MAX_FUNCS 100
	funct *f[MAX_FUNCS];
	funct *fdata = (funct *)malloc(sizeof(funct) * MAX_FUNCS);
	int next_hit = 0;
...
void profile::create_display(bool btb, int page, int sortby)
{
	...
	if (!update || sample_count[0] == 0) {	// TODO - broken when page == -1
		return;
	}
	...
	/*
	 * always display threads for summary information, and rate updates
...
// pgr binary file format:
// int version
// int size - number of statistics data values
// int gnow
// stat_data
// lastline[MAXPAGES]
// character data for pages
