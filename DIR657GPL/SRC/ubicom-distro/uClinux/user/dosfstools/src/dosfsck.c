...
#ifdef USE_SILEX
#define MSG_ID 1115 //talk to silex mount_thread
#define TEXT_SIZE 16
struct msgbuffer
{
	long mtype;
...
int main(int argc,char **argv)
{
...
#ifdef USE_SILEX
	if(wait_msg != 1){if (optind != argc-1) usage(argv[0]);}
#else
    ...
    if(wait_msg == 1){
			...
			printf("dosfsck: test receive path: (%s)", msgp.mtext);
			fs_open(msgp.mtext, rw);
		}
		else
#endif
