...
unsigned char host_name[MAX_BUFLEN];
unsigned char if_name[MAX_BUFLEN];
...
int main(int argc, char *argv[ ] )
{
    ...
    if (verbose) printf("argv[1] = %s, argv[2] = %s\n", argv[1], argv[2]);

    if (argc != 3) USAGE;

    strncpy(if_name, argv[1], MAX_BUFLEN);
    strncpy(host_name, argv[2], MAX_BUFLEN);
...
static int get_local_v6addr(unsigned char *ifname, unsigned char *addr6)
{
              ...
              devname) != EOF)
    {
        ...
        {
	    bind_if6_idx = if_idx;		//Jery Lin added 2010/02/01
            memcpy(addr6   , str_addr+ 0, 4);
            *(addr6+ 4) = ':';
            memcpy(addr6+ 5, str_addr+ 4, 4);
            *(addr6+ 9) = ':';
            memcpy(addr6+10, str_addr+ 8, 4);
            *(addr6+14) = ':';
            memcpy(addr6+15, str_addr+12, 4);
            *(addr6+19) = ':';
            memcpy(addr6+20, str_addr+16, 4);
            *(addr6+24) = ':';
            memcpy(addr6+25, str_addr+20, 4);
            *(addr6+29) = ':';
            memcpy(addr6+30, str_addr+24, 4);
            *(addr6+34) = ':';
            memcpy(addr6+35, str_addr+28, 4);
            *(addr6+39) = 0;

            if ( verbose )
                fprintf(stderr, "v6 addr = %s\n", addr6);
                     ...
                     size_t     *descSize )
{
        ...
        ai = ai->ai_next )
    {
        ...
        if (ai->ai_family == PF_INET6) /* IPv6 */
        {
            ...
            getaddrinfo( LLMNR_MCAST_V6GROUP, service, &hints, &addr6);
    
            get_local_v6addr(if_name, v6_addr);
    
            memcpy(&mreq6.ipv6mr_multiaddr, 
                &((struct sockaddr_in6 *)(addr6->ai_addr))->sin6_addr, 
                    ...
                    *(pbuf+1)   = 0;
                    ...
                    for (i=0; i<htons(llmnr_hdr->qdcount); i++)
                    {
                        ...
                        if ( verbose )
                        {
                            ...
                            fprintf(stderr, "qd_name + 1 = %s\n", qd_name + 1);
                        }
                        
                        if(strcmp(host_name, qd_name + 1) == 0)
                        {                
                            llmnr_hdr->ancount++;
                            if ( verbose ) fprintf(stderr, "matched hostname counter = %d\n", llmnr_hdr->ancount);
                        }
                        else
                        {
