...
int Option6RD(struct dhcpMessage *packet)
{
	...
	int i = 0;
	FILE *fp;
	char nvram_value[64] = {0};

	if ((fp = popen("nvram get ipv6_wan_proto", "r")) != NULL){
		fgets(nvram_value, sizeof(nvram_value), fp);
		pclose(fp);
		if(!strstr(nvram_value, "ipv6_6rd"))
			return 0;
	}
	...
	if (!(temp = get_option(packet, OPTION_6RD))){
		...
		if((option_len >= 22)){
			...
			sprintf(cmd, "cli ipv6 6rd dhcp %d %d %02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x %d.%d.%d.%d",
