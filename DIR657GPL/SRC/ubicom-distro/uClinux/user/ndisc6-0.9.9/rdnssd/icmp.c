...
static int icmp_recv (int fd, char* interface)
{
		...
		/*Joe Huang:Parses RA prefix flag, check m & o bit are set or not (IPv6 spec 1.12)*/
		...
		if( (icmp6.nd_ra_flags_reserved & ND_RA_FLAG_MANAGED) && last_flag != icmp6.nd_ra_flags_reserved){
			...
			if(last_flag != 1)
				system("cli ipv6 autoconfig stop");
//			system("cli ipv6 autoconfig mset");
			sprintf(cmd, "cli ipv6 autoconfig mset %s", interface);
			system(cmd);
			...
			if(last_flag != 1)
				system("cli ipv6 autoconfig stop");
//			system("cli ipv6 autoconfig oset");
			sprintf(cmd, "cli ipv6 autoconfig oset %s", interface);
			system(cmd);
			last_flag = icmp6.nd_ra_flags_reserved;
		}

		else if (!(icmp6.nd_ra_flags_reserved & ND_RA_FLAG_MANAGED || icmp6.nd_ra_flags_reserved & ND_RA_FLAG_OTHER)){
			char tmp[50];
			FILE *fp;
			...
			if(last_flag != icmp6.nd_ra_flags_reserved){
				if(last_flag != 1)
					system("cli ipv6 autoconfig stop");
//				system("cli ipv6 autoconfig noneset");
				sprintf(cmd, "cli ipv6 autoconfig noneset %s", interface);
				system(cmd);
				last_flag = icmp6.nd_ra_flags_reserved;
			}
			/* Parses RA options */
			if ((fp = popen("nvram get ipv6_wan_specify_dns","r")) != NULL){
