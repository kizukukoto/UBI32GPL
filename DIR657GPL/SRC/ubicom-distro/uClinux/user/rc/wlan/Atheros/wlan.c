...
//#define  RC_DEBUG_WLAN  1
//#define ATH_AR938X_WPS20 1
#ifdef ATH_AR938X_WPS20 // remark UUID-E code from new Atheros wpa2 for Win7 logo
#define UUID 1
#endif
#ifdef  RC_DEBUG_WLAN
...
#ifdef ATH_AR938X
	system("rmmod wlan_scan_ap");
   	system("rmmod wlan_scan_sta");
	system("rmmod ath_pktlog");
	sleep(1);
	system("rmmod wlan_me");
	sleep(1);
	//system("rmmod umac");
	//sleep(1);
	system("rmmod ath_dev");
	system("rmmod ath_dfs");
	system("rmmod ath_spectral");
	system("rmmod ath_rate_atheros");
	system("rmmod ath_hal");
	system("rmmod asf");
	system("rmmod adf");
#else
...
void wlan0_ifnum_setup(char *op_mode)
{
		  ...
		  strcmp(wlan0_dot11_mode, "11n")==0) )
	{
		...
		{
			if (strcmp(wlan0_11n_protection, "20") == 0){
				Ap_Chmode="11NGHT20";
		    } else/*wlan0_11n_protection=auto*/
		    {
			    if (atoi(wlan0_channel) <= 6) {
			        if (atoi(wlan0_channel) == 5) {
		        		/* NickChou, 802.11n System Interoperability Test Plan Version 2.0.5 
		        		 4.2.41 AP 20/40 MHz Coexistence Test 
		        		 Step3. If the ¡§Secondary Channel Offset¡¨ field of HT Operation Element in the Beacon 
		        		 is not set to 3 then fail
		        		 Channel 5~7 Support 11NGHT40MINUS and 11NGHT40PLUS
		        		*/
		...
		else /*auto channel*/
		{
			if(strcmp(wlan0_11n_protection, "auto") == 0)
				Ap_Chmode="11NGHT40";
			else if(strcmp(wlan0_11n_protection, "40") == 0)
				Ap_Chmode="11NGHT40";
			else
				Ap_Chmode="11NGHT20";
		}
...
void wlan1_ifnum_setup(void)
{
	...
	char *Ap_Chmode_2 = "11NAHT20";
	char *Ap_Primary_Ch_2=NULL;
	char *wlan0_domain = "0x3A";
	int wlan1_channel_int=0;

	memset(ap_ifnum_2, 0, sizeof(ap_ifnum_2));
	memset(ap_ifnum_4, 0, sizeof(ap_ifnum_4));
	wlan1_channel_int = atoi(wlan1_channel);
	...
	else if( (wlan1_channel_int>35) && (wlan1_channel_int<166) )
	{
		/* HT40MHz*/
	...
	if ( (strcmp(wlan1_dot11_mode, "11na")==0 || strcmp(wlan1_dot11_mode, "11n")==0) )
	{
		...
		if(wlan0_domain == NULL)
			wlan0_domain = nvram_safe_get("wlan0_domain");
#else
		wlan0_domain = nvram_safe_get("wlan0_domain");
#endif		
	 	...
	 	if(wlan1_auto_channel_enable==0)
	 	{
     		...
     		else if( ((wlan1_channel_int-28)%8==0)  || //36, 44, 52, 60
     				 ((wlan1_channel_int-92)%8==0)  || //100, 108, 116, 124, 132
		    		 ((wlan1_channel_int-141)%8==0)	   //149, 157
		    		)
		    {
		    	/*soem wlan domain (ex.Russia) do not support 11a-HT40*/
		    	if( strtol(wlan0_domain, NULL, 16) != 0x3C )
		    	{
		    ...
		    else if(wlan1_channel_int==165 || wlan1_channel_int==140)
		    	Ap_Chmode_2="11NAHT20";
		    else
		    {
		       	if( strtol(wlan0_domain, NULL, 16) != 0x3C )
		...
		{
			if (strcmp(wlan1_11n_protection, "auto") == 0 &&
				strtol(wlan0_domain, NULL, 16) != 0x3C
			)
...
int rg2Country(int regdomain)
{
	...
	*/
	...
	switch(regdomain)
	{
	   ...
	   case	0x3A://new
#ifdef ATH_AR938X
		/*The country code 840 is a STA only country code*/
		countryCode = 841; //CTRY_UNITED_STATES for AP
#else
...
#ifdef ATH_AR938X
/* save mac to nvram if the nvram key exits.*/
static void set_mac_to_nvram(const char *str_key, const char *mac, int band)
{
	int i = 0;
	char tmp_mac[24] = {0};
	...
	if (strcmp(nvram_safe_get(str_key), "") != 0) 
	{
		for(i = 0; i < strlen(mac); i++) {
			sprintf(tmp_mac, "%s%c", tmp_mac, toupper(mac[i]));
		}
		nvram_set(str_key, tmp_mac);
	}
}
#else
...
void set_wlan_mac(void)
{
	...
	int  lan_mac[8]={0};
	int  wan_mac[8]={0};
	char wlan0_mac[20]={0}; // Don't forget space for NULL terminator
	char wlan1_mac[20]={0}; // Don't forget space for NULL terminator
        int i=0;
#ifdef ATH_AR938X
	char tmp_wlan_mac[20]={0};
#endif
...
#ifdef ATH_AR938X
	set_mac_to_nvram("wlan0_mac", p_lan_mac, 0);
#endif
...
#ifdef ATH_AR938X
	sprintf(tmp_wlan_mac, "%02x:%02x:%02x:%02x:%02x:%02x",
		lan_mac[0],lan_mac[1],lan_mac[2],lan_mac[3],lan_mac[4],lan_mac[5]);

	_system("iwpriv wifi0 setHwaddr %s", tmp_wlan_mac);
#else
	...
	if(nvram_match("wps_enable", "1") == 0)
		set_wps_uuid(wlan0_mac);	//Add for 2.4GHz and 5GHz with the same UUID
#endif
	...
	//sscanf(p_wan_mac,"%02x:%02x:%02x:%02x:%02x:%02x",&wan_mac[0],&wan_mac[1],&wan_mac[2],&wan_mac[3],&wan_mac[4],&wan_mac[5]);
        ...
        memset(tmp_wlan_mac, 0, 20);
        
        if(lan_mac[5]== 0xff) i=1;
	if((lan_mac[5]!= 0xff)&&(lan_mac[5]!= 0xfe))
	{
		sprintf(wlan1_mac, "%02x%02x%02x%02x%02x%02x",lan_mac[0],lan_mac[1],lan_mac[2],lan_mac[3],lan_mac[4],lan_mac[5]+2);
		sprintf(tmp_wlan_mac, "%02x:%02x:%02x:%02x:%02x:%02x",lan_mac[0],lan_mac[1],lan_mac[2],lan_mac[3],lan_mac[4],lan_mac[5]+2);
	}
	else if(lan_mac[4]!= 0xff)
	{
		sprintf(wlan1_mac, "%02x%02x%02x%02x%02x%02x",lan_mac[0], lan_mac[1], lan_mac[2], lan_mac[3], lan_mac[4]+1, i);
		sprintf(tmp_wlan_mac, "%02x:%02x:%02x:%02x:%02x:%02x",lan_mac[0], lan_mac[1], lan_mac[2], lan_mac[3], lan_mac[4]+1, i);
	}
	else if(lan_mac[3]!= 0xff)
	{
		sprintf(wlan1_mac, "%02x%02x%02x%02x%02x%02x",lan_mac[0],lan_mac[1],lan_mac[2],lan_mac[3]+1, 0, i);
		sprintf(tmp_wlan_mac, "%02x:%02x:%02x:%02x:%02x:%02x",lan_mac[0],lan_mac[1],lan_mac[2],lan_mac[3]+1, 0, i);
	}
	else if(lan_mac[2]!= 0xff)
	{
		sprintf(wlan1_mac, "%02x%02x%02x%02x%02x%02x",lan_mac[0],lan_mac[1],lan_mac[2]+1, 0, 0, i);
		sprintf(tmp_wlan_mac, "%02x:%02x:%02x:%02x:%02x:%02x",lan_mac[0],lan_mac[1],lan_mac[2]+1, 0, 0, i);
	}
	else if(lan_mac[1]!= 0xff)
	{
		sprintf(wlan1_mac, "%02x%02x%02x%02x%02x%02x",lan_mac[0],lan_mac[1]+1, 0, 0, 0, i);
		sprintf(tmp_wlan_mac, "%02x:%02x:%02x:%02x:%02x:%02x",lan_mac[0],lan_mac[1]+1, 0, 0, 0, i);
	}
	else
	{
		sprintf(wlan1_mac, "%02x%02x%02x%02x%02x%02x",lan_mac[0]+1, 0, 0, 0, 0, i);
		sprintf(tmp_wlan_mac, "%02x:%02x:%02x:%02x:%02x:%02x",lan_mac[0]+1, 0, 0, 0, 0, i);
	}
	set_mac_to_nvram("wlan1_mac", wlan1_mac, 1);

	_system("iwpriv wifi1 setHwaddr %s", tmp_wlan_mac);
#endif
...
#ifdef ATH_AR938X
        system("insmod /lib/modules/adf.ko");
        system("insmod /lib/modules/asf.ko");
        system("insmod /lib/modules/ath_hal.ko");
        system("insmod /lib/modules/ath_rate_atheros.ko");
        system("insmod /lib/modules/ath_spectral.ko maxholdintvl=5000 nfrssi=1 nobeacons=0");
        system("insmod /lib/modules/ath_dfs.ko");
        system("insmod /lib/modules/ath_dev.ko");
        system("insmod /lib/modules/umac.ko");
...
void start_makeVAP(int ath_count, int wds_enable, char *apifnum, char *op_mode)
{
	...
	if(strcmp(set_RF, "RF")==0)
	{
		...
		if(strcmp(wlan0_11n_protection, "40") == 0){
			_system("iwpriv ath%d disablecoext 1", ath_count);
#ifndef ATH_AR938X
			_system("iwpriv ath%d cwmmode 2", ath_count);
#endif
		}else {
#ifndef ATH_AR938X
...
void create_topo_file(int wps_enable)
{
    ...
    for(bss_index=0; bss_index<4; bss_index++)
    {
    	 ...
    	 {
#ifdef ATH_AR938X
    	 	_system("iwpriv ath%d wps 1",  use_hostapd[bss_index]);
#endif
...
#ifdef UUID
static char wps_uuid[37]="\0";
void set_wps_uuid(char *wlan_mac)
{
#if 1
        sprintf(wps_uuid, "%s-%s", "b2e122d0-1dd1-11b2-b149", wlan_mac);
#else
	FILE *uuid_fp;
	
	uuid_fp = fopen(UUID_FILE, "r");	
	
	if( fscanf(uuid_fp, "uuid:%36s", wps_uuid) != 1 )
	{
		printf("set_wps_uuid: error\n");
		fclose(uuid_fp);
		return;
	}
	fclose(uuid_fp);
...
/*
 * Date : 07/06/2010 Robert
 * Action : Modify lan_bridge, lan_eth, and wlan0_op_mode from pointer to the array.
 * Reasion : Those contents of pointers could be change after calling other function to do something.
 * Solve : To replace those pointers with arrays, and to make sure that 
 *         those value will not be change if we don't change.
 */
...
int start_wlan(void)
{
	...
	int ath_count=0;
	char wlan0_op_mode[12];

	if(!( action_flags.all  || action_flags.wlan ) )
		return 1;

	sprintf(wlan0_op_mode, "%s", nvram_safe_get("wlan0_op_mode"));
	...
	sprintf(lan_bridge, "%s", nvram_safe_get("lan_bridge"));
	sprintf(lan_eth, "%s", nvram_safe_get("lan_eth"));
	...
	/* Run 2.4G or 5G */
	if ((nvram_match("wlan0_enable", "1") == 0) && (nvram_match("wlan0_schedule", "Never"))) {
		printf("##################### wlan0_enable#########################\n");
		wlan0_enable = 1;
		if ((nvram_match("wlan0_vap1_enable", "1") == 0) && (nvram_match("wlan0_vap1_schedule", "Never"))) {
	...
	if ((nvram_match("wlan1_enable", "1") == 0) && (nvram_match("wlan1_schedule", "Never"))) {
		printf("##################### wlan1_enable#########################\n");
		wlan1_enable = 1;
		if ((nvram_match("wlan1_vap1_enable", "1") == 0) && (nvram_match("wlan1_vap1_schedule", "Never"))) {
...
#ifdef ATH_AR938X
        system("echo 1 > /proc/irq/27/smp_affinity");
        system("echo 4 > /proc/irq/93/smp_affinity");
#endif
...
void start_activateVAP(int ath_count, int guest_zone, int netusb_guest_zone)
{
	...
	//guest_zone=1 => Disable Routing between LAN
...
static void set_security(int bss_index, int ath_count)
{
	...
	else if((strcmp(wlan_security,"wep_auto_64") == 0))
	{
		set_wep_key(64, 4, bss_index, ath_count);
		if(strcmp(wlan0_op_mode, "AP")==0)
		{
	...
	else if((strcmp(wlan_security,"wep_auto_128") == 0))
	{
		set_wep_key(128, 4, bss_index, ath_count);
		if(strcmp(wlan0_op_mode, "AP")==0)
		{
...
void start_wps(void)
{
	...
	)
	{
		...
		/* "hostapd -d" for more debug messages */
		if( use_hostapd[0]!=4 )
			_system("hostapd -d /var/tmp/WSC_ath%d.conf &", use_hostapd[0]);
		if( use_hostapd[1]!=4 )
			_system("hostapd -d /var/tmp/WSC_ath%d.conf &", use_hostapd[1]);
		if( use_hostapd[2]!=4 )
			_system("hostapd -d /var/tmp/WSC_ath%d.conf &", use_hostapd[2]);
		if( use_hostapd[3]!=4 )
			_system("hostapd -d /var/tmp/WSC_ath%d.conf &", use_hostapd[3]);			
#else
...
void create_wsc_conf(int bss_index, int ath_count, int wps_enable, int wep_len)
{
    ...
    char radius_server_0[256];
    char radius_server_1[256];
	...
	*/
    ...
    /* only usable if not forking to background
		0 = verbose debugging, 1 = debugging
		2 = informational messages, 3 = notification, 4 = warning
    */	
#else
...
#else
	/*
	HOSTAPD_DEBUG_NO = 0, HOSTAPD_DEBUG_MINIMAL = 1,
	HOSTAPD_DEBUG_VERBOSE = 2, HOSTAPD_DEBUG_MSGDUMPS = 3,
	...
	*/
	...
	/*
		WPS only support WEP-Open or WEP-Auto
		Because hostapd state machine do not support Shared Key Auth
	*/
	if(strstr(wlan_security, "wep_auto")) /* Open or SharedKey */		
	...
	/*
		In order to make hostapd interoperate with old clients, 
		the version number can be set to the older version (1)
	To fix DCS-2120(DCS-5220) IOT issue and follow UbiCom (Win Vista) setting.
	*/
	...
	else if (strstr(wlan_security, "_eap") && (wps_enable==0 || bss_index==2 || bss_index==3))
	{
		...
		switch(bss_index)
		{
			...
			case 1:
				...
				break;
			case 2:
				wlan_eap_mac_auth = nvram_safe_get("wlan0_vap1_eap_mac_auth");
				...
				break;
			case 3:
				wlan_eap_mac_auth = nvram_safe_get("wlan1_vap1_eap_mac_auth");
		...
		{
			sprintf(radius_server_0, "%s", wlan_wpa_eap_radius_server_0);
			radius1_ip = strtok(radius_server_0, "/");
		...
		{
			sprintf(radius_server_1, "%s", wlan_wpa_eap_radius_server_1);
			radius2_ip = strtok(radius_server_1, "/");
	...
	if(wps_enable==0 || bss_index==2 || bss_index==3) //guest zone do not support WPS
	{
		/* 
		1. WPS disabled
		2. Guest Zone do not support WPS 
		*/
		
		if (atoi(rekey_time) < 120)
           rekey_time = "120";
        
        /* rekeying GTK (broadcast/multicast encryption keys) */   
	...
	else /* WSC configuration section => Wi-Fi Protected Setup (WPS) */
	{
#ifdef ATH_AR938X_WPS20
		fprintf(fp, "upnp_iface=%s\n", nvram_safe_get("lan_bridge"));
		
		if( nvram_match("wps_lock", "") != 0 ) 
			fprintf(fp, "ap_setup_locked=%s\n", nvram_safe_get("wps_lock"));
		
		//fprintf(fp, "config_methods=label display push_button keypad virtual_push_button physical_push_button\n");
		...
		fprintf(fp, "manufacturer_url=%s\n", nvram_safe_get("manufacturer_url"));
		fprintf(fp, "model_description=%s\n", nvram_safe_get("model_name"));
		fprintf(fp, "model_url=%s\n", nvram_safe_get("model_url"));
		
		/*
		the length of dev_name must be less than 32 bytes
		this name appears in the Vista "networks" window icon,
		when the device reports itself as unconfigured
		*/
		fprintf(fp, "device_name=%s\n", nvram_safe_get("hostname"));
		
		/* Universal Product Code (optional for UPnP)
		fprintf(fp, "upc=123456789012\n");
		*/
		
		/*
			If not set, hostapd will not allow external WPS Registrars to control the access point.
			The AP PIN can also be set at runtime with hostapd_cli wps_ap_pin command.
		*/
		fprintf(fp, "ap_pin=%s\n", nvram_safe_get("wps_pin"));
				
		/* 
		Used format: <categ>-<OUI>-<subcateg>
		categ = 6 = Network Infrastructure
		OUI = 0050F204
		subcateg = 1 = AP
		*/
...
#ifdef UUID		
		/* 
		Default is based on mac address of interface, 
		which is usually a betteridea than putting a uuid here!
		using uuidgen to generate UUID => uuid:bc5041d4-1dd1-11b2-a986-a27c302edc0e
		Add uuid= for 2.4GHz and 5GHz with the same UUID
		...
		*/
		...
		if(atoi(nvram_safe_get("wps_configured_mode"))==1){
			...
			if(atoi(nvram_safe_get("disable_wps_pin"))==1){
			fprintf(fp, "ap_setup_locked=1\n");
				fprintf(fp, "wps_config_methods=0x0080\n");
			}
			else{
			fprintf(fp, "ap_setup_locked=0\n");
				fprintf(fp, "wps_config_methods=0x0084\n");
