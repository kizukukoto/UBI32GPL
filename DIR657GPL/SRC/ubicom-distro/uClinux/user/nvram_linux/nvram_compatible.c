...
/* 2010/12/08 Robert fix multi-lang issue. 
 * If user do restore_default, it should not be clear for multi-language nvram key
 */
#ifdef DIR652V
	if (!strcmp(n, "lang_date") || !strcmp(n, "lang_version") 
	|| !strcmp(n, "lang_size") || !strcmp(n , "lang_region")) {
		//printf("XXX %s(%d),%s=%s XXX\n", __func__, __LINE__, n, v);
		return ;
	}
#endif
	nvram_set(n, v);
