/* $Id: options.c,v 1.2 2009/07/08 11:18:33 jimmy_huang Exp $ */
/* MiniUPnP project
...
static const struct {
	...
	*/
	{ UPNPINT_IFNAME, "int_ifname" }, // int_ifname, string used in miniupnpd.conf
	{ UPNPEXT_IP,	"ext_ip" },
...
readoptionsfile(const char * fname)
{
	...
	while(fgets(buffer, sizeof(buffer), hfile))
	{
		...
		{
			struct upnpperm * new_upnppermlist;
			new_upnppermlist = realloc(upnppermlist,
			                       sizeof(struct upnpperm) * (num_upnpperm+1));
			if (!new_upnppermlist) {
				free(upnppermlist);
				return -1;
			}
			upnppermlist = new_upnppermlist;
			/* parse the rule */
		...
		*equals = '\0';
		...
		{
			struct option *new_ary_options;
			num_options += 1;
			new_ary_options = (struct option *)realloc(ary_options, num_options * sizeof(struct option));
			if (!new_ary_options) {
				free(ary_options);
				return -1;
			}
			ary_options = new_ary_options;
