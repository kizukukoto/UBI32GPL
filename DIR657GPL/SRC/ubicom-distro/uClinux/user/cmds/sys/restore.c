...
/*XXX: if we will never use ath base nvram, delete that! */
struct nvram_tuple {
	char *name;
	char *value;
...
#define OS_BUILD_VER	OS_VERSION "b18"
struct nvram_tuple router_defaults[] = {
/* system settings */
	{ "os_version", OS_VERSION, 1},
	{ "__os_version", OS_BUILD_VER, 1},
	{ "os_date", __DATE__, 1},
	{ "ips_version", "", 1},
	{ "ips_date", "", 1},
	{ "ips_size", "0", 1},
	{ "model_name", "DIR-730", 1},
	{ "model_number", "D-Link Router", 1},
	{ KEY_NVRAM_RELEASE_TIME, VALUE_NVRAM_RELEASE_TIME, 1},	/* force restore if check dismatch at boot time */
...
int restore_main(int argc, char *argv[])
{
	...
	if (nvram_init(NULL) != 0)
		goto out;
	for (t = router_defaults; t->name != NULL; t++) {
		if (t->next != 1)
			continue;
		DEBUG_MSG("restore: %s:%s\n", t->name, t->value);
...
extern int ddns_restore_main(int argc, char *argv[]);
/*
 * cat nvram.txt|awk -F, '{ print $1"=^"$2"^;"$3}'|sed 's/"//g'|sed 's/[\{ |\}]//g'|sed  's/\s@star//'|tr -d ' '|sed 's/\^/"/g' 
 * */

int nvram_restore_stream_main(int argc, char *argv[])
{
	FILE *fd;
	char *nvram_file = "/etc/nvram.txt";
	int rev = -1;
	char buf[4096], *k, *v, *p;
	
	if (argc > 1)
		nvram_file = argv[1];
	if ((fd = fopen(nvram_file, "r")) == NULL)
		goto out;
