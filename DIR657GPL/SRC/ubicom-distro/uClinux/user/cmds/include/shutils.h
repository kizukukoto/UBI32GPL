 ...
 *
 * $Id: shutils.h,v 1.2 2009/03/26 15:06:29 peter_pan Exp $
 */
#include <string.h>
...
extern char *get_if_val(const char *zone_alias, const char *if_key);

extern char *get_proto_val(const char *zone_alias, const char *proto_key);
...
#define eval_nowait(pid, cmd, args...) ({ \
	char *argv[] = { cmd, ## args, NULL }; \
...
extern char g1[], g2[];
static inline char *strcat_i(const char *s, int i, char *buf)
{
	return strcat_r(s, itoa(i), buf);
}
#define setenv1(env, val)	setenv(env, val, 1)
#define nvram_get_i(s, i, buf)	nvram_get(strcat_i(s, i, buf))
#define nvram_safe_get_i(s, i, buf)	nvram_safe_get(strcat_i(s, i, buf))
#define nvram_set_i(s, i, buf, v)	nvram_set(strcat_i(s, i, buf), v)
#define nvram_unset_i(s, i, buf)	nvram_unset(strcat_i(s, i, buf))
/********************************************************************/
#ifdef __cplusplus
}
#endif
