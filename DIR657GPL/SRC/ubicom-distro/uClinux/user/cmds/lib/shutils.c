 ...
 *
 * $Id: shutils.c,v 1.1 2009/03/11 04:06:28 peter_pan Exp $
 */
...
#ifdef linux
char g1[4096], g2[4096]; /* for tmp buffer, a *very* short scope life */
/*
...
*/
#define NVRAM_MAX_PARAM_LEN	1024
int 
...
char *itoa(int i)
{
	...
	return gcvt(i, sizeof(str) - 1, str);
}
#if 0
char *get_proto_val(const char *zone_alias, const char *proto_key)
{
	int i, j;
	char tmp[32], alias[32];

	if ((i = nvram_find_index("zone_alias", zone_alias)) == -1)
		goto out;
	if ((i = nvram_find_index("if_alias", nvram_safe_get(strcat_r("zone_if_list", itoa(i), tmp)))) == -1)
		goto out;
	sprintf(alias, "%s_alias", nvram_safe_get(strcat_r("if_proto", itoa(i), tmp)));
	if ((j = nvram_find_index(alias, nvram_safe_get(strcat_r("if_proto_alias", itoa(i), tmp)))) == -1)
		goto out;
	sprintf(alias, "%s_%s", nvram_safe_get(strcat_r("if_proto", itoa(i), tmp)), proto_key);
		return nvram_safe_get(strcat_r(alias, itoa(j), tmp));
out:
	return NULL;
}
#endif
char *get_if_val(const char *zone_alias, const char *if_key)
{
	int i;
	char tmp[32];
	...
	if ((i = nvram_find_index("if_alias", nvram_safe_get(strcat_r("zone_if_list", itoa(i), tmp)))) == -1)
		goto out;
	return nvram_safe_get(strcat_r(if_key, itoa(i), tmp));
out:
	return NULL;
}
