...
struct in_addr localbind_mgr = { INADDR_NONE };
int call_ID;
...
int open_inetsock(struct in_addr inetaddr)
{
    ...
    if (localbind_mgr.s_addr != INADDR_NONE) {
        bzero(&src, sizeof(src));
        src.sin_family = AF_INET;
        src.sin_addr   = localbind_mgr;
...
int open_unixsock(struct in_addr inetaddr)
{
    ...
    callmgr_name_unixsock( &where, inetaddr, localbind_mgr, call_ID); /*Ubicom: Modified 20091113 */
...
void close_unixsock(int fd, struct in_addr inetaddr)
{
    struct sockaddr_un where;
    close(fd);
    callmgr_name_unixsock(&where, inetaddr, localbind_mgr, call_ID);
...
void callmgr_name_unixsock(struct sockaddr_un *where,
			   struct in_addr inetaddr,
			   struct in_addr localbind_mgr, int callid) /*Ubicom: Modified 20091113 */
{
    char localaddr[16], remoteaddr[16];
    where->sun_family = AF_UNIX;
    strncpy(localaddr,  inet_ntoa(localbind_mgr), 16);
    strncpy(remoteaddr, inet_ntoa(inetaddr),  16);
    snprintf(where->sun_path, sizeof(where->sun_path),
            PPTP_SOCKET_PREFIX "%s:%i", remoteaddr,callid); /*Ubicom: Modified 20091113 */
}
