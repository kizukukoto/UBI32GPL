...
/*!
 * \file sched.c
 * \brief A scheduling to simplement an event drived application in a single thread program
 *
 ...
 * to easy the schedulers management. Waiting writable scheduler means it is waiting write IO is
 ...
 * in the future. When the time expired, the driver will call the on_timeout callback function
 * to process the event. If one scheduler want to destroy it or any other schedulers, it just
 * need to set the need_destroy member to non-zero of that scheduler. When the driver find a
 * scheduler's need_destroy member is non-zero, it call the on_destroy callback function to
 ...
 * To make sure the scheduler list nerver be empty that will simplify the list operation
 */
...
void start_sched()
{
	...
	for(;;) {
		...
		if(old_time > cur_time) { //The system clock was adjust backward
			time_t diff;
		...
		for(prev = sched_header.first, cur = prev->next; cur; ) {
			if(cur->need_destroy) {
				tr_log(LOG_DEBUG, "Destroy scheduler");
				prev->next = cur->next;
