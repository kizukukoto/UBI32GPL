...
char crs_username[80] = ""; //used by udp.c
char crs_password[80] = ""; //used by udp.c
static char path[80] = "";
...
// 2011.04.22
// default use ipv4, tryt use ipv6 socket
#if 0
	tcp = tr_listen("::", port, SOCK_STREAM, 2);
	if(tcp >= 0) {
		sched = calloc(1, sizeof(*sched));
		if(sched == NULL) {
			tr_log(LOG_ERROR, "Out of memory!");
