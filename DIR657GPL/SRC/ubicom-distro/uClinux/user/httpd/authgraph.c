...
int AuthGraph_ValidateIdCodeByStr(unsigned char *auth_id, unsigned char *auth_code)
{
	...
	/* ---------------------------------------------- */
		...
		if(strcasecmp(auth_id_str, auth_id) == 0 && strcasecmp(auth_code_str, auth_code) == 0){
			...
			*	Date:	2009-05-18
			*   Name:	Jimmy Huang
			*   Reason:	Prevent hackers get id and code via capturing ehtereal packes
			*			we let the auth_id and auth_code gone
			*			once we have validated the code successfully
			*   Notice :
			*/
			auth_graph_map[i].auth_id   = (unsigned long) rand();
        	auth_graph_map[i].auth_code = (unsigned long) rand() % 0xffff;
			//-----------
			return 0;
		}
    }
