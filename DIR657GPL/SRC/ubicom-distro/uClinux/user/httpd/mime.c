...
//#define SYSLOG(fmt, a...) printf(fmt, ##a)
...
static int each_boundary(FILE *stream, const char *boundary)
{
...
static inline char *get_boundary_from_env()
{
	...
	if ((p = getenv("CONTENT_TYPE")) == NULL) {
		if ((p = getenv("BOUNDARY")) != NULL)
			return p;	/* return boundary from env */
		return NULL;
	} else
		goto out;
	printf("GET ENV CONTENT_TYPE:%s\n", p);
	/*
	 * FIXME:
