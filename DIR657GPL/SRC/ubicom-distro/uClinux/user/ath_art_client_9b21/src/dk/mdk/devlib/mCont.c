...
#ident  "ACI $Id: //depot/sw/src/dk/mdk/devlib/mCont.c#80 $, $Header: //depot/sw/src/dk/mdk/devlib/mCont.c#80 $"
...
void ar928xSetSingleCarrier( A_UINT32 devNum ) {
    ...
    if(pLibDev->mode == MODE_11G) {
		...
		{
            REGW(devNum, 0x7868, (0x1 << 29) | (0x1 << 27) | (0x1 << 17) | (0x1 << 16) |
        ...
        else if(isKiwi(pLibDev->swDevID))
        {
            ...
            {
                REGW(devNum, 0x78ac, (0x1 << 29) | (0x1 << 20) | 
                     (0x1 << 18) | (0x1 << 16) | 
                     (0x1 << 12) | (0x1 << 10) | 
                     (0x1 << 7)  | (0x1 << 5)  | 
                     (0x1 << 3)); 
    ...
    if( pLibDev->mode == MODE_11A ) {
        ...
        if( pLibDev->hwDevID == MERLIN_DEV_AR5416_PCI ) {
            REGW(devNum, COMMON_SW_TABLE_REG, 0x00000dd0);
        }
...
//printf("SNOOP:txContBegin:tx%d:descAddr=%x\n", queueIndex, pLibDev->tx[queueIndex].descAddress);
	//memDisplay(devNum, pLibDev->tx[queueIndex].descAddress, 8);
        ...
        if( isKiteSW(pLibDev->swDevID) ) {
            createTransmitPacket(devNum, MDK_NORMAL_PKT, NULL, 1, 2366, pData, 
