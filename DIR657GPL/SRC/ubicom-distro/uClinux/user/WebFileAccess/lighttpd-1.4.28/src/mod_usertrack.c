...
URIHANDLER_FUNC(mod_usertrack_uri_handler) {
	...
	builtin_MD5_Init(&Md5Ctx);
	builtin_MD5_Update(&Md5Ctx, (unsigned char *)con->uri.path->ptr, con->uri.path->used - 1);
	builtin_MD5_Update(&Md5Ctx, (unsigned char *)"+", 1);

	/* we assume sizeof(time_t) == 4 here, but if not it ain't a problem at all */
	LI_ltostr(hh, srv->cur_ts);
	builtin_MD5_Update(&Md5Ctx, (unsigned char *)hh, strlen(hh));
	builtin_MD5_Update(&Md5Ctx, (unsigned char *)srv->entropy, sizeof(srv->entropy));
	LI_ltostr(hh, rand());
	builtin_MD5_Update(&Md5Ctx, (unsigned char *)hh, strlen(hh));

	builtin_MD5_Final(h, &Md5Ctx);
