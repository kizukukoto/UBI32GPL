...
static int check_lan_match(void *p_d, char *authority) {
	...
	DEBUG_MSG(" #### count:%d\n", count);
	if (count == 0 && !nvram_match("lan_ipaddr", authority)) { // maybe is 192.168.0.1 or dlinkrouter or dlinkrouterXXXX
		...
		if (!nvram_match("lan_ipaddr", addr))
			ret = 1;
...
URIHANDLER_FUNC(mod_graph_auth_uri_handler)
{
	...
	if (get_path_match(p->conf.except_paths, con->uri.path) || !buffer_is_empty(con->authed_user))
		goto out;
