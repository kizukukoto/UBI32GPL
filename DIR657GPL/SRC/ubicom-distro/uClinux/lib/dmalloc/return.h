...
/* 
 * somehow configure keep saying RETURN_MACROS_WORK fails, but it actually works.
 * this hack forces it to Ubicom platform specific and bypass configure checking
 */
#if 0
...
#endif	/* #if 0 -- for Ubicom*/

/* This is Ubicom ABI, return address is always in register a5 */
#define GET_RET_ADDR(file)		\
	asm volatile ("move.4 %0, a5 \n\t" 	\
	: "=g" (file)			\
	:				\
	)
