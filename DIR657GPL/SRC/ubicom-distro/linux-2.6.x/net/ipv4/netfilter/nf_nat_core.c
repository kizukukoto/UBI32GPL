 ...
 * Date: 2009.11.18
 */
static int modified_src_occupied(struct net *net, const struct nf_conntrack_tuple *tuple,
                    const struct nf_conn *conntrack);

static inline int same_modified_src(const struct nf_conn *ct,
 ...
 */
    ...
    if((ignored_conntrack->status & IPS_SNATP2P_SRC) 
        ...
        if (modified_src_occupied(net, tuple, ignored_conntrack))
            return 1;
	}
#endif
...
static int
modified_src_occupied(struct net *net, const struct nf_conntrack_tuple *tuple,
                    const struct nf_conn *conntrack)
{
	unsigned int h = hash_by_src(tuple);
	const struct nf_conn *ct;
...
int
find_appropriate_p2p_dst(struct net *net, const struct nf_conntrack_tuple *tuple,
                    struct nf_conntrack_tuple *result)
{
	unsigned int h = hash_by_src(tuple);
	const struct nf_conn *ct;
	...
	hlist_for_each_entry_rcu(nat, n, &net->ipv4.nat_by_modified_source[h], by_modified_source) {
		...
		if (same_modified_src(ct, tuple)) {
			if (ct->status & IPS_SNATP2P_DST)
				...
				nf_ct_invert_tuplepr(result, &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple);
...
EXPORT_SYMBOL_GPL(find_appropriate_p2p_dst);
#endif
...
hash_by_dst(const struct nf_conntrack_tuple *tuple)
{
	...
	/* Original dst, to ensure we map it consistently if poss. */
 ...
 */
#if defined(CONFIG_IP_NF_TARGET_SNATP2P)
	int have_to_hash_modified_src = ((ct->status & IPS_SNATP2P_MASK)
		&& !(ct->status & IPS_SNATP2P_DONE_MASK));
#endif
 ...
 */
	...
	if (have_to_hash_modified_src){
		unsigned int srchash = (ct->status & IPS_SNATP2P_SRC) ?
			hash_by_dst(&ct->tuplehash[IP_CT_DIR_REPLY].tuple) : hash_by_dst(&ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple);
		spin_lock_bh(&nf_nat_lock);
		nat = nfct_nat(ct);
		nat->ct = ct;
		hlist_add_head_rcu(&nat->by_modified_source, &net->ipv4.nat_by_modified_source[srchash]);
		spin_unlock_bh(&nf_nat_lock);
	}
#endif
 ...
 */
	...
	net->ipv4.nat_by_modified_source = nf_ct_alloc_hashtable(&nf_nat_htable_size,
						      &net->ipv4.nat_bysource_vmalloced);
 ...
 */
#if defined(CONFIG_IP_NF_TARGET_SNATP2P)
	nf_ct_free_hashtable(net->ipv4.nat_by_modified_source, 
					net->ipv4.nat_bysource_vmalloced,
					nf_nat_htable_size);
#endif
	nf_ct_free_hashtable(net->ipv4.nat_bysource, net->ipv4.nat_vmalloced,
