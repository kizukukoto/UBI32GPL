...
/*
 * Reason: don't change ttl issue
 * Modified: aaron
 * Date: 2009.12.28
 */
#define ROUTE_TTL_ISSUE 1
...
static void ipv4_link_failure(struct sk_buff *skb)
{
	...
	/*
	 * Reason: Thomas's patch. This will help make BT more stable.
	 * Modified: Yufa Huang
	 * Date: 2010.07.08
	 */
	/*
...
#define PROCFS_NAME             "ttl_ipv4"
/**
 * This structure hold information about the /proc file
 *
 */
static struct proc_dir_entry *ip_rt_Our_Proc_File;
/**
 * The buffer used to store character for this module
 *
 */
static char ip_rt_procfs_buffer[PROCFS_MAX_SIZE];
/**
 * The size of the buffer
 *
 */
static unsigned long ip_rt_procfs_buffer_size = 0;
/**
...
{
        /* get buffer size */
        ip_rt_procfs_buffer_size = count;
        if (ip_rt_procfs_buffer_size > PROCFS_MAX_SIZE ) {
                ip_rt_procfs_buffer_size = PROCFS_MAX_SIZE;
        }
        /* write data to the buffer */
        if ( copy_from_user(ip_rt_procfs_buffer, buffer, ip_rt_procfs_buffer_size) ) {
                return -EFAULT;
        }
        
        ip_rt_procfs_buffer[ip_rt_procfs_buffer_size] = 0;
        sscanf(ip_rt_procfs_buffer,"%d",&ip_rt_ttl);
        printk(KERN_INFO "procfile_write (/proc/%s) called ttl=%d\n", PROCFS_NAME,ip_rt_ttl);
        
        return ip_rt_procfs_buffer_size;
}
/**
...
static int myproc_init_module(void)
{
        ...
        ip_rt_Our_Proc_File = create_proc_entry(PROCFS_NAME, 0644, NULL);
        if (ip_rt_Our_Proc_File == NULL) {
                printk(KERN_ALERT "Error: Could not initialize /proc/%s\n",
                         PROCFS_NAME);
                return -ENOMEM;
        }
        ip_rt_Our_Proc_File->read_proc   = procfile_read;
        ip_rt_Our_Proc_File->write_proc  = procfile_write;
        ip_rt_Our_Proc_File->owner       = THIS_MODULE;
        ip_rt_Our_Proc_File->mode        = S_IFREG | S_IRUGO;
        ip_rt_Our_Proc_File->uid         = 0;
        ip_rt_Our_Proc_File->gid         = 0;
        ip_rt_Our_Proc_File->size        = 37;
