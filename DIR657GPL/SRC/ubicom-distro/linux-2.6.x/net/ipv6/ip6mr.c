...
// 2011.06.02
struct in6_addr ipv6_Zero; //={{0,0,0,0}};

static struct timer_list ipmr_udp_timer;
unsigned long ipmr_udp_timer_count=0;

static void ipmr_udp__expire_process(unsigned long dummy)
{
//	printk("time---\n");
	ipmr_udp_timer_count++;

	mod_timer(&ipmr_udp_timer, jiffies + HZ);
...
// 2011.06.02
static void *ip6mr_udp_seq_start(struct seq_file *seq, loff_t *pos)
	__acquires(mrt_lock)
{
	read_lock(&mrt_lock);
	return (*pos ? ip6mr_vif_seq_idx(seq->private, *pos - 1)
...
static void *ip6mr_udp_seq_next(struct seq_file *seq, void *v, loff_t *pos)
{
	struct ipmr_vif_iter *iter = seq->private;
...
static void ip6mr_udp_seq_stop(struct seq_file *seq, void *v)
	__releases(mrt_lock)
{
	read_unlock(&mrt_lock);
}

static int ip6mr_udp_seq_show(struct seq_file *seq, void *v)
{
	if (v == SEQ_START_TOKEN) {
		seq_puts(seq,
			 "Interface      BytesIn  PktsIn  BytesOut PktsOut Flags\n");
...
static struct seq_operations ip6mr_udp_seq_ops = {
	.start = ip6mr_udp_seq_start,
	.next  = ip6mr_udp_seq_next,
	.stop  = ip6mr_udp_seq_stop,
	.show  = ip6mr_udp_seq_show,
};


// 2011.06.02
static int ip6mr_udp_open(struct inode *inode, struct file *file)
{
	return seq_open_private(file, &ip6mr_udp_seq_ops,
				sizeof(struct ipmr_vif_iter));
}

static struct file_operations ip6mr_udp_fops = {
	.owner	 = THIS_MODULE,
	.open    = ip6mr_udp_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
...
static void print_ipv6_c(const struct in6_addr *ipv6)
{
	int i;
	if( ipv6 == NULL) {
		printk("null");
	...
	for(i=0;i<16;i++) {
		printk("%02X",ipv6->in6_u.u6_addr8[i]);
	}
}

static void print_ipv6(struct in6_addr *ipv6)
{
	print_ipv6_c((const struct in6_addr *)ipv6);
}
...
// 2011.06.02
	setup_timer(&ipmr_udp_timer, ipmr_udp__expire_process, 0);
...
// 2011.06.02
	if (!proc_net_fops_create(&init_net, "ip6_mr_udp",
				     0, &ip6mr_udp_fops))
		goto proc_cache_fail;
#endif

// 2011.06.02
	ipmr_udp__expire_process(0);
...
// 2011.06.02
	del_timer(&ipmr_udp_timer);
	proc_net_remove(&init_net, "ip6_mr_udp");
#ifdef CONFIG_PROC_FS
...
// 2011.06.02
void ip6_mr_udp_update(int vif,struct sk_buff *skb)
{
	struct mif_device *v = &vif6_table[vif];
	struct mfc6_udp *udp;
	struct mfc6_udp *newudp;

	if( (v->flags & MIFF_UDPINFO) == 0) {
		// non support check udp
		return; 
	}
	...
	while( udp) {
		if( udp->active ) 
		if (ipv6_addr_equal(&udp->mf6c_mcastgrp, &ipv6_hdr(skb)->daddr) &&
			ipv6_addr_equal(&udp->mf6c_origin, &ipv6_hdr(skb)->saddr)) {
			udp->time = ipmr_udp_timer_count;
			// find 
			return;
		}
		udp = udp->next;
	...
	newudp =  kmalloc(sizeof(*newudp), GFP_KERNEL);
	if( !newudp)
		return;
	...
	newudp->active = 1;
	newudp->next = NULL;
	newudp->time = ipmr_udp_timer_count;
	ipv6_addr_copy(&newudp->mf6c_mcastgrp,&ipv6_hdr(skb)->daddr);
	ipv6_addr_copy(&newudp->mf6c_origin,&ipv6_hdr(skb)->saddr);


	newudp->next = v->udp_info;
	v->udp_info = newudp;


	print_ipv6(&newudp->mf6c_mcastgrp);
	printk(" ");
	print_ipv6(&newudp->mf6c_origin);
	printk("new add\n");

	DEBUG_PRINTHEX(skb->data,skb->len);	
}
...
static int ip6mr_send_icmp_info(struct sk_buff *pkt, mifi_t mifi, int assert)
{
...
// quick return
	...
	if (!pskb_may_pull(pkt, sizeof(struct in6_addr)))
		return -EINVAL;

	hdr = icmp6_hdr(pkt);

//	len = pkt->len - sizeof(struct ipv6hdr);

//printk(">>%d %d %d>>",pkt->len,sizeof(struct ipv6hdr),len);
//DEBUG_PRINTHEX((char *) hdr,sizeof(*hdr));
//printk(">>>");

//DEBUG_PRINTHEX(pkt->data,pkt->len);
...
//	msg = (struct mrt6msg *)skb_transport_header(skb);
	...
	skb_put(skb, len - sizeof(struct ipv6hdr));
...
//DEBUG_PRINTHEX(skb->data,skb->len);
	...
	report = (struct igmp_report *) skb->data;
	...
	s = skb->data;
	s += sizeof(*report);
	ipv6_addr_copy((struct in6_addr*)s, &ipv6_hdr(pkt)->saddr);
	s += sizeof(struct in6_addr);
	ipv6_addr_copy((struct in6_addr*)s, &ipv6_hdr(pkt)->daddr);
	s += sizeof(struct in6_addr);
...
//DEBUG_PRINTHEX(skb->data,skb->len);
//printk(">>%d %d %d\n",skb->len,sizeof(struct ipv6hdr),len);
...
int ip6_mr_process_icmp(struct sk_buff *skb)
{
	int vif;
	vif = ip6mr_find_vif(skb->dev);

	if( vif >= 0 ) 
		ip6mr_send_icmp_info(skb,vif,199);
...
// 2011.06.03
	vif = ip6mr_find_vif(skb->dev);
	if (vif >= 0 && vif6_table[vif].flags & MIFF_NO_ORG ) {
		cache = ip6mr_cache_find(&ipv6_Zero, &ipv6_hdr(skb)->daddr);
//		ip6mr_send_ip_info(skb,vif,199);
