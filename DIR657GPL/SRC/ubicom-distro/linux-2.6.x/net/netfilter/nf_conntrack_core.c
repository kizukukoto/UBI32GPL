				   ...
				   gfp_t gfp, int is_tcp)
{
	...
	/*
		BT will cause soft lockup.We let httpd pass nat.
		And setup ip_contract_max to 2048. 2048 is a value that does not cause CPU busy.
		It depends on HW and CPU.
		BUG: wireless packets might drop while ip_contract_count == ip_contract_max.
			 This might be sloved if we can confirm a packet as wireless packets. 
	*/
#if 1
	if (orig->dst.u.tcp.port == 80 || orig->src.u.tcp.port == 80 || repl->dst.u.tcp.port == 80 || repl->src.u.tcp.port == 80 \
		|| orig->dst.u.tcp.port == 53 || orig->src.u.tcp.port == 53 || repl->dst.u.tcp.port == 53 || repl->src.u.tcp.port == 53 \
		|| orig->dst.u.icmp.type == 0 || orig->dst.u.icmp.type == 8 || repl->dst.u.icmp.type == 0 || repl->dst.u.icmp.type == 8) {
		goto keep_going;
	}
#endif
        ...
        {
	        if (nf_conntrack_max &&
	                unlikely(atomic_read(&net->ct.count) > 2048)) {	    	
	       ...
	       unsigned int dataoff)
{
	...
	ct = nf_conntrack_alloc(net, tuple, &repl_tuple, GFP_ATOMIC, (strcmp(l4proto->name, "tcp") == 0)? 1:0);
