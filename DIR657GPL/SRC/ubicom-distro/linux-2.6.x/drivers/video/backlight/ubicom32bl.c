...
static volatile u32_t *ubicom32bl_channel_ctl[] = {
	&(UBICOM32_IO_PORT(RC)->ctl1),
	&(UBICOM32_IO_PORT(RC)->ctl2),
	&(UBICOM32_IO_PORT(RH)->ctl1),
};
#elif IP8000
#define UBICOM32BL_NUM_PWM_CHANNELS 4
static volatile u32_t *ubicom32bl_channel_ctl[] = {
	&(UBICOM32_IO_PORT(RQ)->ctl1),
	&(UBICOM32_IO_PORT(RR)->ctl1),
	&(UBICOM32_IO_PORT(RS)->ctl1),
	&(UBICOM32_IO_PORT(RT)->ctl1),
};
#else
...
# elif IP8000
	{
		struct ubicom32_io_port *port = UBICOM32_IO_PORT(RQ + (ud->pdata->pwm_channel * 0x4000));
		...
		if (fn_sel & (1 << gpio_bit(ud->pdata->gpio))) {
