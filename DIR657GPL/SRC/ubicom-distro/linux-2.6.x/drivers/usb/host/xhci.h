...
#define XHCI_UNALIGN_WORKAROUND
#endif

#ifdef XHCI_UNALIGN_WORKAROUND
#define XHCI_4BYTES_POOL_SIZE PAGE_SIZE				/* x000 unaligned packets per HCD seems more than enough */
#define XHCI_4BYTES_MAP_CNT (XHCI_4BYTES_POOL_SIZE/4/32)	/* each bit represents 4 bytes in the pool */
struct xhci_4bytes_pool {
	void *buf;
	dma_addr_t dma;
	u32 bitmap[XHCI_4BYTES_MAP_CNT];
};
#endif
...
#ifdef XHCI_UNALIGN_WORKAROUND
	struct xhci_4bytes_pool *align_pool;
#endif
};
...
#ifdef CONFIG_UBICOM32
static unsigned int xhci_readl(const struct xhci_hcd *xhci,
		__u32 __iomem *regs);
static void xhci_writel(struct xhci_hcd *xhci,
