...
static void ubi32_serdes_hw_init(struct uart_port *port, int baud)
{
	 ...
	 */
	*(unsigned int *)(GENERAL_CFG_BASE + GEN_CLK_IO_DISABLE) &= ((unsigned int)port->membase == RO) ? ~GEN_CLK_DISABLE_SERDES0 : ~GEN_CLK_DISABLE_SERDES1;
	switch (port->uartclk) {
	case 250000000:
		*(unsigned int *)(GENERAL_CFG_BASE + GEN_CLK_IO_SELECT) |= (1 << 29);
		*(unsigned int *)(GENERAL_CFG_BASE + GEN_CLK_IO_SELECT) |= ((unsigned int)port->membase == RO) ? (1 << 16) : (1 << 17);
		break;
	...
	case RP:
		gpio_port = UBICOM32_GPIO_PORT(PG3);
		break;

	case RO:
		gpio_port = UBICOM32_GPIO_PORT(PG4);
		break;
