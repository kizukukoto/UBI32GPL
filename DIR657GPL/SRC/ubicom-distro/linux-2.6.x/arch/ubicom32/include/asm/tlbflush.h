...
{
	tlb_purge_range(vma->vm_mm->context.asid, start, end);
}
...
{
	tlb_purge_page(vma->vm_mm->context.asid, addr);
}
...
{
	tlb_purge_range(init_mm.context.asid, start, end);
}
...
{
	tlb_purge_page(init_mm.context.asid, addr);
}
...
{
	int target = (vma->vm_flags & VM_EXEC) ? 0 : 1;
