 ...
 */
static __ocm_text void sha2_init_512(struct crypto_tfm *tfm)
{
	struct ubicom32_sha512_ctx *ctx = crypto_tfm_ctx(tfm);
 ...
 */
static __ocm_text void sha2_init_384(struct crypto_tfm *tfm)
{
	struct ubicom32_sha512_ctx *ctx = crypto_tfm_ctx(tfm);
 ...
 */
static __ocm_text void sha2_init_256(struct crypto_tfm *tfm)
{
	struct ubicom32_sha256_ctx *sctx = crypto_tfm_ctx(tfm);
 ...
 */
static __ocm_text void sha2_init_224(struct crypto_tfm *tfm)
{
	struct ubicom32_sha256_ctx *sctx = crypto_tfm_ctx(tfm);
 ...
 */
static void __ocm_text sha2_update_1024(struct crypto_tfm *tfm, const u8 *data,
			unsigned int len)
{
	struct ubicom32_sha512_ctx *sctx = crypto_tfm_ctx(tfm);
	int index, clen;
 ...
 */
static void __ocm_text sha2_final_1024(struct crypto_tfm *tfm, u8 *out)
{
	struct ubicom32_sha512_ctx *sctx = crypto_tfm_ctx(tfm);
	u64 bits[2];
	...
	sha2_update_1024(tfm, sctx->buf, end);
 ...
 * by the external caller to indicate termination of the digest calculation.
 */
static void __ocm_text sha2_update_512(struct crypto_tfm *tfm, const u8 *data,
			unsigned int len)
{
	struct ubicom32_sha256_ctx *sctx = crypto_tfm_ctx(tfm);
	int index, clen;
 ...
 * message digest. 
 */
static void __ocm_text sha2_final_512(struct crypto_tfm *tfm, u8 *out)
{
	struct ubicom32_sha256_ctx *sctx = crypto_tfm_ctx(tfm);
	u64 bits;
	unsigned int index, end;
	...
	sha2_update_512(tfm, sctx->buf, end);
...
static struct crypto_alg alg_512 = {
	.cra_name	=	"sha512",
	...
	.cra_list	=	LIST_HEAD_INIT(alg_512.cra_list),
	.cra_u	  = {
		.digest = {
			.dia_digestsize =       SHA512_DIGEST_SIZE,
			.dia_init       =       sha2_init_512,
			.dia_update     =       sha2_update_1024,
			.dia_final      =       sha2_final_1024,
		}
	}
};

static struct crypto_alg alg_384 = {
	.cra_name	=	"sha384",
	...
	.cra_ctxsize	=	sizeof(struct ubicom32_sha512_ctx),
	.cra_module	=	THIS_MODULE,
	.cra_list	=	LIST_HEAD_INIT(alg_384.cra_list),
	.cra_u	  = {
		.digest = {
			.dia_digestsize =       SHA384_DIGEST_SIZE,
			.dia_init       =       sha2_init_384,
			.dia_update     =       sha2_update_1024,
			.dia_final      =       sha2_final_1024,
		}
	}
...
static struct crypto_alg alg_256 = {
	.cra_name	=	"sha256",
	...
	.cra_module	=	THIS_MODULE,
	.cra_list	=	LIST_HEAD_INIT(alg_256.cra_list),
	.cra_u	  = {
		.digest = {
			.dia_digestsize =       SHA256_DIGEST_SIZE,
			.dia_init       =       sha2_init_256,
			.dia_update     =       sha2_update_512,
			.dia_final      =       sha2_final_512,
		}
	}
};

static struct crypto_alg alg_224 = {
	.cra_name	=	"sha224",
	...
	.cra_ctxsize	=	sizeof(struct ubicom32_sha256_ctx),
	.cra_module	=	THIS_MODULE,
	.cra_list	=	LIST_HEAD_INIT(alg_224.cra_list),
	.cra_u	  = {
		.digest = {
			.dia_digestsize =       SHA224_DIGEST_SIZE,
			.dia_init       =       sha2_init_224,
			.dia_update     =       sha2_update_512,
			.dia_final      =       sha2_final_512,
		}
	}
...
#endif
	hw_crypto_init();
 	crypto_register_alg(&alg_224);
 	crypto_register_alg(&alg_256);
 	crypto_register_alg(&alg_384);
	return crypto_register_alg(&alg_512);
}
...
{
	crypto_unregister_alg(&alg_224);
	crypto_unregister_alg(&alg_256);
	crypto_unregister_alg(&alg_384);
	crypto_unregister_alg(&alg_512);
}
