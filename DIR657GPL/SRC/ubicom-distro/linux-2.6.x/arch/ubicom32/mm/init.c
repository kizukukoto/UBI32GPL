...
/*
 * PMDs for the vmalloc region.
 */
pte_t ubicom32_vmalloc_ptes[(VMALLOC_END - VMALLOC_START) / PAGE_SIZE]
...
{
	int i;
	pte_t *ppte __attribute__ ((unused));
...
#if defined(CONFIG_MMU)
	/*
	 * Initialize the default PGD and the vmalloc PMDs.
	 */
	memset(swapper_pg_dir, 0, sizeof(swapper_pg_dir));
	memset(ubicom32_vmalloc_ptes, 0, sizeof(ubicom32_vmalloc_ptes));

	/*
 	 * Load the vmalloc PMDs into the default PGD.
	 */
	ppte = ubicom32_vmalloc_ptes;
	for (i = VMALLOC_START / (PAGE_SIZE * PTRS_PER_PTE);
			i < VMALLOC_END / (PAGE_SIZE * PTRS_PER_PTE); i++) {
		unsigned int pfn = ((unsigned int)ppte) >> PAGE_SHIFT;
		pte_t pte = (pte_t)pfn_pte(pfn, PAGE_KERNEL);
		pmd_populate_kernel(&init_mm, (pmd_t *)(swapper_pg_dir + i), &pte);
		ppte += (PAGE_SIZE / sizeof(pte_t));
	}

	/*
	 * Set the kernel break address to the start of UNMAPPED space.
	 */
...
/*
 * mem_init()
 *	Figure out memory sizing and put free memory into proper zones.
 */
