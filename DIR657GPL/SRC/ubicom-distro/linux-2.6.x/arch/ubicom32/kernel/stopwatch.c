 ...
 */
static inline unsigned int stopwatch_cycles_convert(unsigned int cycles, unsigned int scale)
{
	unsigned int micro = ((cycles * scale) / (stopwatch_freq / 1000000));
	return micro;
}
...
static int __init stopwatch_init(void)
{
	...
	stopwatch_freq = processor_frequency();
	spin_unlock_irqrestore(&stopwatch_lock, flags);
	return 0;
}
