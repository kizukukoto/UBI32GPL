...
int unaligned_emulate(struct pt_regs *regs)
{
	...
	if (unlikely(save_csr & 0x04100)) {
		 ...
		 */
		printk(KERN_CRIT "CSR has alternate thread select bits on.\n");
		return -1;
	}
	...
	if (src_operand) {
		...
		if (user_mode(regs)) {
			...
			if (src_address >= (unsigned char *)UBICOM32_FDPIC_UNMAPPED_START) {
							/*
	...
	if (dest_operand) {
		...
		if (user_mode(regs)) {
			...
			if (dest_address >= (unsigned char *)UBICOM32_FDPIC_UNMAPPED_START) {
							/*
				 * Set  TRAP_CAUSE_PRIV_VIOLATION bit in regs->trap_cause
				 */
				regs->trap_cause = (1 << TRAP_CAUSE_PRIV_VIOLATION);
...
int unaligned_only(struct pt_regs *regs)
{
	...
	if (regs->csr & UBICOM32_CSR_FPTRAP_MASK) {
		/*
		 * Floating point traps detected. Go service it.
		 */
		return 0;
	}
