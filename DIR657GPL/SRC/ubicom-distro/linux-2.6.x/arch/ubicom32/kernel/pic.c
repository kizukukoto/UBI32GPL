 ...
 */
STATIC void pic_lock_release(void)
{
 ...
 */
STATIC void pic_lock_acquire(void)
{
...
#if defined(CONFIG_UBICOM32_STALL_ON_TRAPS_IN_PIC)
	/*
...
STATIC int pic_snapshot_rotate(unsigned int snapshot[])
{
	...
	DEBUG_ASSERT(nextword < PIC_IRQ_WORDS);
	DEBUG_ASSERT(nextbit < (sizeof(unsigned int) * 8));
	...
	for (i = 0; i < PIC_IRQ_WORDS; i++) {
		 ...
		 */
		if (!interrupts) {
			nextword = ((++nextword) == PIC_IRQ_WORDS) ? 0 : nextword;
			continue;
		}
		...
		nextbit += pic_bits[interrupts & 0x7];
		thisbit = (nextbit & PIC_IRQ_MASK);
		
		/*
		 * Now update the choice for next time.
		 */
		nextbit = (thisbit + 1) & PIC_IRQ_MASK;
 ...
 */
static int pic_show(struct seq_file *m, void *v)
{
	int n = (int)v - 1;

	if (n == 0) {
		seq_printf(m, "Delivery attempts: %10d\n", pic_bank.attempts);
		seq_printf(m, "Deliveries:\n");
		seq_printf(m, "%10u - Interrupts\n", pic_bank.interrupts);
		seq_printf(m, "%10u - Traps\n", pic_bank.traps);
		seq_printf(m, "%10u - Page faults\n", pic_bank.faults);
		seq_printf(m, "Retries:\n");
		seq_printf(m, "%10u - CPUs affinity failures (should always be 0)\n", 
			   pic_bank.retry_nocpus);
		seq_printf(m, "%10u - Global Locks Held\n", 
			   pic_bank.retry_global);
		seq_printf(m, "%10u - Desired CPU has interrupts disabled\n", 
			   pic_bank.retry_local);
		seq_printf(m, "%10u - Blocked by MMU or Blocking I/O Region\n",
			   pic_bank.retry_mmuio);
		seq_printf(m, "%10u - Number of backout/retries\n\n", 
			   pic_bank.backout);
	}
 ...
 */
static void *pic_start(struct seq_file *m, loff_t *pos)
{
	unsigned long i = *pos;
 ...
 */
static void *pic_next(struct seq_file *m, void *v, loff_t *pos)
{
	++*pos;
	return pic_start(m, pos);
}

/*
 * pic_stop()
 */
static void pic_stop(struct seq_file *m, void *v)
{
}
