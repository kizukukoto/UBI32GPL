 ...
 */
static inline void pcie_br_writeb(volatile u8 * addr, u8 val)
{
	asm volatile (
		" move.1	0(%[addr]), %[val]	\n"
		" jmpt.f	.+4			\n"
...
void __init pcie_phy_config(struct ubicom32_io_port *phy)
{
	...
	phy_writeb(phy, 0x002, 0x10);//01-10VV
	...
	phy_writeb(phy, 0x065, 0x42);//46
	phy_writeb(phy, 0x066, 0xFB);//8B
	...
	phy_writeb(phy, 0x206, 0x19);//15//
	phy_writeb(phy, 0x207, 0x7A);//5A//
	phy_writeb(phy, 0x208, 0x87);
	phy_writeb(phy, 0xA82, 0x8E);//80//
	...
	phy_writeb(phy, 0x215, 0x40);//40//00
