...
{
	u32 data = 0;
	u32 reset_time = UBICOM32_IO_TIMER->sysval;
...
s32 synopGMAC_init_tx_rx_desc_queue(synopGMACdevice *gmacdev)
{
	...
	//dma_map_single(NULL,gmacdev->TxDesc,sizeof(DmaDesc) * gmacdev->TxDescCount,DMA_TO_DEVICE);

	for (i =0; i < gmacdev -> RxDescCount; i++) {
		synopGMAC_rx_desc_init_ring(gmacdev->RxDesc + i, i == gmacdev->RxDescCount-1);
	}
	//dma_map_single(NULL,gmacdev->RxDesc,sizeof(DmaDesc) * gmacdev->RxDescCount,DMA_TO_DEVICE);
...
void synopGMAC_get_desc_data(DmaDesc * desc, u32 * Status, u32 * Buffer1, u32 * Length1, u32 * Data1)
{
	 ...
	 */
	//dma_unmap_single(NULL,virt_to_phys(desc),DESC_FLUSH_SIZE,DMA_FROM_DEVICE);
