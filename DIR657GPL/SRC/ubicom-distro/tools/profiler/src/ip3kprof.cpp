...
BOOL CIp3kprofApp::InitInstance()
{
	...
	if (cmdInfo.m_nShellCommand != CCommandLineInfo::FileOpen) {
		if (!cmdInfo.autoload) {
			AfxMessageBox("  Usage: ip3kprof [-t:seconds] [-c:filename] [-M] [-S[:seconds]] [-log[:seconds]] [-ip:board_ip_address] [-D:distro_path] [-a] [ultra_elf-file linux_elf-file] [[-L:nnnnnnnn] app_elf-file] ...");
			return FALSE;
		}
