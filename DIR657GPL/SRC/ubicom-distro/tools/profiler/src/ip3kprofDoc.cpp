...
BOOL CIp3kprofDoc::OnOpenDocument(LPCTSTR lpszPath)
{
	...
	if (strstr(lpszPath, ".pgr") != NULL) {
		pr.init_profiler(false, linux_console_send);
		...
		if (linux_console == NULL) {
			...
			if (linux_console == NULL) {
				AfxMessageBox("Can't create network console socket to conenct to the board");
				return FALSE;
			}
		}
...
BOOL CIp3kprofDoc::OnSaveDocument(LPCTSTR lpszPathName) 
{
	...
	if (!pr.save_hazards(1000, "hazards.txt", 0)) {
		AfxMessageBox("Can't open hazards.txt");
	}

	name.Replace(".txt", ".pgr");
	if (!pr.save_graph(name)) {
		AfxMessageBox("Can't save graph file");
	}
