...
#define MEM_UNKOWN_LABEL "Unknown functions"
...
// all of text memory, a list of memory blocks
class memory {
public:
	memory();
	~memory();
	...
	memory_map *add_memory_map(int pid, char *command);

	/* parse input for a memory map for a pid */
	bool parse_map(char *input, int *pid);

	/* connect the memory map to segments wit the same name */
	...
	struct symbol *find_symbol_by_address(unsigned int addr, unsigned int *sym_addr, int *type);

	// add a new code segment of seg_size bytes.  data must be from malloc and will be freed in memory destructor
	memblock *add_seg(unsigned int start, unsigned int seg_size, unsigned int *data, char *name, int type) 
	{
	...
	{
		for (memblock *seg = first_seg; seg != 0; seg = seg->next) {
	...
	{	// return total size of all code segments in bytes
		unsigned int size = 0;
		for (memblock *p = first_seg; p != 0; p = p->next) {
			size += p->size;
		}
		return size;
	}
#if 0
	// get the relocation value for the segment containing address
	unsigned int relocation(unsigned int pc)
	{
		for ( memblock *seg = first_seg; seg != 0; seg = seg->next) {
			if (pc >= seg->start_address && pc < seg->start_address + seg->size) {
				return seg->relocation;
			}
		}
		return 0;
	}
...
private:
	memblock *first_seg;		// all loaded text segments
	memblock *dummy_seg;		// segment for unrecognized addresses
	...
	bool target_error;
	int haztype_error;
	unsigned int address_n;
	memblock *address_mem;
