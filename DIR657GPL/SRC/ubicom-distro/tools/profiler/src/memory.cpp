...
memory_map::memory_map(int c_pid, char *com)
{
	next = NULL;
	first_seg = NULL;
	pid = c_pid;
...
void memory::init()
{
	...
	// create dummy segment for unknown addresses
	unsigned int *data = (unsigned int *)malloc(8);
	if (data) {
		*data = *(data + 1) = 0xc8000000;	// NOP instruction
	}
	dummy_seg = add_seg(0, 8, data, MEM_UNKOWN_LABEL, MEM_TYPE_UNKNOWN);
	address_mem = 0;
	touched_error = 0; 
	target_error = false;
	uint_error = 0;
	double_error = 0.0;
	haztype_error = 0;
	address_n = 0xffffffff;
	line[0] = 0;
	end = line;
}


memory_map *memory::add_memory_map(int pid, char *command) {
	for (memory_map *pmap = first_map; pmap; pmap = pmap->next) {
		if (pmap->pid == pid) {
			return NULL;
		}
	}
	memory_map *pmap = new memory_map(pid, command);
	pmap->next = first_map;
	first_map = pmap;
	// every map needs a dummy segment
	map_segment *ms = new map_segment(0, 8, 0, 0, MEM_UNKOWN_LABEL);
	ms->seg = dummy_seg;
...
#if 0
bool memory::parse_map(char *input, int *pid) 
{
	for (memory_map *pmap = first_map; pmap; pmap = pmap->next) {
		if (pmap->pid == *pid) {
			return pmap->parse_map(input, pid);
		}
	}
	memory_map *pmap = add_memory_map(*pid, "Unknown");
	return pmap->parse_map(input, pid);
}
#endif
...
void memory::update_memory_map(memory_map *mm)
{
	...
	for (map_segment *ms = mm->first_seg; ms; ms = ms->next) {
		...
		for (memblock *m = first_segment(); m != NULL; m = m->next) {
			char segname[500];
 ...
 */
bool memory::parse_map(char *in, int *new_pid)
{
	...
	while (*in) {
		if (!pmap && *new_pid != -1) {
			...
			if (!pmap) {
				pmap = add_memory_map(*new_pid, "Unknown");
			}
		}
		// copy a line
		while (*in && *in != '\n') {
		...
		*end = 0;
		...
		if (!isdigit(*p)) { 
			...
			*n = 0;
			...
			if (*p != 0) {
				assert(pmap);
				pmap->add_segment(start, end_adr, ocm_start, ocm_end, name);
			}
			continue;
		}
		// regular map
 ...
 * NULL for error
 */
struct symbol *memory::find_symbol_by_address(unsigned int addr, unsigned int *sym_addr, int *type_out) 
{
	struct symbol *ret;
	for ( memblock *seg = first_seg; seg != 0; seg = seg->next) {
		ret = seg->find_symbol_by_address(addr, sym_addr, type_out);
		if (ret) {
			return ret;
		}
	}
