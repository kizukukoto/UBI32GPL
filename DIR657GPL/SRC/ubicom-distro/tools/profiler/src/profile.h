...
#define MAX_STAT_DISPLAY (FIXED_STAT_DISPLAY + PROFILE_MAX_COUNTERS)
...
public:
	...
	void on_packet(char *buf, unsigned int size);					// a packet has arrived from the board on the data connection
	...
	bool save_hazards(int count, char *filename, int page);		// NULL filename for sending to display
...
//	maps linux_memory_map;		// map of unmapped memory segments
	...
	bool next_pc(unsigned int inst, unsigned int pc, memblock *seg, int type, unsigned int *next);
	void write_log();
	...
	bool symbol_address_to_name(unsigned int pc, char *buf);
	...
	int mtcount[NUMTHREADS];		// how many threads simultaneously active and unblocked
	int mtactivecount[NUMTHREADS];		// how many threads simultaneously active
	double recent_idle;
	...
	unsigned int min_sp_val[NUMTHREADS];	// minimum stack pointer value
	...
	void add_call(unsigned int p_adr, unsigned int s_adr, double freq);

	unsigned int current_clock;			// last received clock value
	unsigned int current_inst_count[NUMTHREADS];
	double current_mips;				// moving average of current MIPS
