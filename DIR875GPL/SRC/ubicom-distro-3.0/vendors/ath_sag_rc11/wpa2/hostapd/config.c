        ...
        )
{
	...
	if (bss->auth_algs == 0) {
		...
		bss->ieee80211w = atoi(value);
	...
	} else if (strcmp(tag, "ap_setup_locked") == 0) {
                bss->wps->ap_setup_locked = atol(value); 
	        ...
	        if (uuid_str2bin(value, wps->uuid)) {
...
//	            value[SIZE_16_BYTES * 2] != '\0') {
        ...
        #endif /* WPS_OPT_NFC */
        #endif /* EAP_WPS */
	} else if ((strcmp(tag, "mac_auth") == 0)) {
                        bss->mac_auth = atoi(value);
			wpa_printf(MSG_ERROR, "%s(%d), mac_auth :: %d\n", __func__, __LINE__, bss->mac_auth);
	} else {
		wpa_printf(MSG_ERROR,
