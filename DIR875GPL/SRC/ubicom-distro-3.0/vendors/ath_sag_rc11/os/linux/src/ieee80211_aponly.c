...
ieee80211_decap_aponly(struct ieee80211vap *vap, wbuf_t wbuf, size_t hdrspace)
{
    ...
    struct llc *llc;
    u_int16_t ether_type = 0;
    u_int8_t a1[IEEE80211_ADDR_LEN], a2[IEEE80211_ADDR_LEN], a3[IEEE80211_ADDR_LEN];
    u_int8_t fc1;
    ...
    switch (fc1 & IEEE80211_FC1_DIR_MASK) {
    case IEEE80211_FC1_DIR_NODS:
        IEEE80211_ADDR_COPY(eh->ether_dhost, a1);
        IEEE80211_ADDR_COPY(eh->ether_shost, a2);
        break;
    case IEEE80211_FC1_DIR_TODS:
        IEEE80211_ADDR_COPY(eh->ether_dhost, a3);
        IEEE80211_ADDR_COPY(eh->ether_shost, a2);
        break;
    case IEEE80211_FC1_DIR_FROMDS:
        IEEE80211_ADDR_COPY(eh->ether_dhost, a1);
        IEEE80211_ADDR_COPY(eh->ether_shost, a3);
        break;
    } 
...
#ifndef __ubicom32__     /* memory flush will be done in HAL */
...
ath_rx_buf_link_aponly(struct ath_softc *sc, struct ath_buf *bf, bool rxenable)
{
    ...
    if (sc->sc_rxlink == NULL)
        ...
        *sc->sc_rxlink = bf->bf_daddr;
        OS_FLUSH_CACHE_SINGLE(sc->sc_rxlink);
    }
...
ath_rx_intr_aponly(ath_dev_t dev, HAL_RX_QUEUE qtype)
{
    ...
    do {
        ...
        OS_SYNC_SINGLE(sc->sc_osdev,
                       bf->bf_buf_addr[0], sc->sc_rxstatuslen, BUS_DMA_UBICOM32_FROMDEVICE,
                       OS_GET_DMA_MEM_CONTEXT(bf, bf_dmacontext));
         ...
         */
        OS_SYNC_SINGLE(sc->sc_osdev,
                       bf->bf_buf_addr[0], sc->sc_rxbufsize, BUS_DMA_UBICOM32_FROMDEVICE,
                       OS_GET_DMA_MEM_CONTEXT(bf, bf_dmacontext));
        bf->bf_status |= ATH_BUFSTATUS_SYNCED;
#endif
...
#ifndef __ubicom32__ // Already done immediately after RX
    if (unlikely(!(bf->bf_status & ATH_BUFSTATUS_SYNCED))) {
        OS_SYNC_SINGLE(sc->sc_osdev,
                   bf->bf_buf_addr[0], wbuf_get_pktlen(wbuf), BUS_DMA_UBICOM32_FROMDEVICE,
                   OS_GET_DMA_MEM_CONTEXT(bf, bf_dmacontext));
        bf->bf_status |= ATH_BUFSTATUS_SYNCED;
    }
#endif
...
#endif
#ifdef __ubicom32__
                       OS_FLUSH_CACHE_SINGLE(txq->axq_link);
#else
...
ieee80211_encap_8023_aponly(struct ieee80211_node *ni, wbuf_t wbuf)
{
    ...
    if (likely(!is_amsdu && ether_type >= IEEE8023_MAX_LEN)) {
    ...
    *(u_int16_t *)wh->i_dur = 0;
     ...
     */
    if (unlikely(ether_type == htons(ETHERTYPE_PAE))) {
        /* 
...
#if ATH_SUPPORT_WAPI
    else if (ether_type == htons(ETHERTYPE_WAI)) {
            wbuf_set_exemption_type(wbuf, WBUF_EXEMPT_ALWAYS);
    }
#endif
...
ieee80211_classify_aponly(struct ieee80211_node *ni, wbuf_t wbuf)
{
     ...
     *
       KASSERT(((priority >> 4) == 0) || ((priority >> 16) == 0xffef));
     */
...
osif_vap_hardstart_aponly(struct sk_buff *skb, struct net_device *dev)
{
    ...
    daddr = eh->ether_dhost;
    prof_flag = (IEEE80211_ADDR_EQ(daddr,my_peer_addr));   
