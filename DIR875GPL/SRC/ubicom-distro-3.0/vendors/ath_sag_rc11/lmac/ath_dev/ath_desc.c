...
#define ATH_DESC_4KB_BOUND_NUM_SKIPPED(_len) ((_len) / 4096)
    ...
    desc_len += (L1_CACHE_BYTES - 16) + (L1_CACHE_BYTES - 1);
    desc_len &= ~(L1_CACHE_BYTES - 1);
#endif
...
#if defined(__ubicom32__)
    /* Check that descriptor address must be cache line aligned */
    ASSERT(((int)dd->dd_desc & (L1_CACHE_BYTES - 1)) == 0);

    dd->dd_desc += (L1_CACHE_BYTES - 16);
    dd->dd_desc_paddr += (L1_CACHE_BYTES - 16);
#endif
    ds = dd->dd_desc;
        ...
        else
        {
            ...
            for (i = 0; i < nbuf_alloc; i++, bf++, ds += (desc_len * ndesc)) {
                ...
                if (!ath_hal_has4kbsplittrans(sc->sc_ah)) {
                    ...
                    */
#if defined(__ubicom32__)
                    if (ATH_DESC_4KB_BOUND_CHECK((bf->bf_daddr - (L1_CACHE_BYTES - 16)), desc_len * ndesc)) {
                        ds += 0x1000 + (L1_CACHE_BYTES - 16) - (bf->bf_daddr & 0xFFF);
#else
...
#if defined(__ubicom32__)
    /* Check that total descriptor size must be multiple of cache line */
    ASSERT((dd->dd_desc_len & (L1_CACHE_BYTES - 1)) == 0);
