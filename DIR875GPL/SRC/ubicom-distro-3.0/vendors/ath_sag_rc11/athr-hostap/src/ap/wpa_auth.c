				    ...
				    struct wpa_auth_callbacks *cb)
{
	...
	wpa_printf(MSG_CAMEO, "CAMEO: wpa_init");
	...
	wpa_printf(MSG_CAMEO, "CAMEO: wpa_init: wpa_auth_gen_wpa_ie");
	...
	wpa_printf(MSG_CAMEO, "CAMEO: wpa_init: wpa_group_init");
		 ...
		 u8 *data, size_t data_len)
{
	...
	if (sm->wpa == WPA_VERSION_WPA2) {
		/* Robert Chen, 2011,11,30
 		*  Reason : To fix DCS2121(IP-CAM) wireless issue that can not connect with wpa2.
 		*  We find DCS with EAPOL_KEY_TYPE_WPA to connect our dut.
 		* */
		if (key->type != EAPOL_KEY_TYPE_RSN && key->type != EAPOL_KEY_TYPE_WPA) {
