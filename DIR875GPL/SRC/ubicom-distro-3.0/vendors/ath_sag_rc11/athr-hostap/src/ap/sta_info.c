...
#ifdef CONFIG_MYDLINK /* Mydlink */
#include "sutil.h"
#include "nvram.h"
#define LOG_DNSQUERY 		"/tmp/log/dnsquery"
#define LOG_TRIGGEREVENT 	"/tmp/log/triggerevent"
#endif
...
#define NEW_STA 1
#define UNWELCOME_STA 2
#define DEBUG(fmt, args...) cprintf("%s(%d):"fmt, __FILE__, __LINE__, ## args)

#ifdef CONFIG_MYDLINK /* Mydlink */
int mydlink_trigevent_log(const u8 *sta, int flag)
{
	char buf[50] = {0};
	if(flag == NEW_STA) {
		sprintf(buf, MACSTR ";new user add the network\n", MAC2STR(sta));
	} else if (flag == UNWELCOME_STA) {
		sprintf(buf, MACSTR ";unwelcome sta try to connect\n", MAC2STR(sta));
	}

	mydlink_log(LOG_TRIGGEREVENT, buf);
...
struct sta_info * ap_sta_add(struct hostapd_data *hapd, const u8 *addr)
{
	...
	if( !nvram_match("trigger_event_enable", "1") && !nvram_match("wlan0_security", "disable")) {
		DEBUG(MACSTR, MAC2STR(sta->addr));
		mydlink_trigevent_log(sta->addr, NEW_STA);
	}
#endif
	return sta;
}
...
#ifdef CONFIG_MYDLINK		
		if( !nvram_match("trigger_event_enable", "1")) {
			DEBUG(MACSTR, MAC2STR(sta->addr));
			mydlink_trigevent_log(sta->addr, NEW_STA);
		}
#endif		
		       ...
		       const u8 *addr, u16 reason)
{
	...
	sta->timeout_next = STA_REMOVE;
#ifdef CONFIG_MYDLINK
	if(reason == WLAN_REASON_PREV_AUTH_NOT_VALID) {
		if( !nvram_match("trigger_event_enable", "1")) {
			DEBUG(MACSTR, MAC2STR(sta->addr));
			mydlink_trigevent_log(sta->addr, UNWELCOME_STA);
		}
	}
#endif
}
