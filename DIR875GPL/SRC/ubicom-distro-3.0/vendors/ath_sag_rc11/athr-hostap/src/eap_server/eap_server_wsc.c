...
static void * eap_wsc_init(struct eap_sm *sm)
{
	...
	if (sm->identity && sm->identity_len == WSC_ID_REGISTRAR_LEN &&
	    ...
	    0)
	{
		wpa_printf(MSG_CAMEO, "CAMEO: eap_wsc_init: Supplicant is Registrar");
		registrar = 0; /* Supplicant is Registrar */
		 ...
		 == 0)
	{
		wpa_printf(MSG_CAMEO, "CAMEO: eap_wsc_init:  Supplicant is Enrollee");
...
static struct wpabuf * eap_wsc_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	case MESG:
		wpa_printf(MSG_CAMEO, "CAMEO: eap_wsc_buildReq: wps_get_msg\n");
