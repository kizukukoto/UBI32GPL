...
/*------------------------------------------------------------------*/
/*
 * Convert our internal representation of frequencies to a int channel.
 * NickChou 07.4.24 add for printf float(double) causing segmentation fault
 */
int
iw_freq2channel(iwfreq *	in)
{
	int	freq = in->m;
	int channel;
  ...
  if(iw_get_ext(skfd, ifname, SIOCGIWFREQ, &wrq) >= 0)
    {
      info->has_freq = 1;
      /* NickChou  20100510
      For 5GHz => ex. Frequency:5.58 GHz
      */
      info->freq = iw_freq2float(&(wrq.u.freq));
      info->freq_flags = wrq.u.freq.flags;

      /*NickChou 07.4.24
      For 2.4GHz => ex. Channel:6
      */
      info->channel = iw_freq2channel(&(wrq.u.freq));      
    }
...
void
iw_print_channel(char *	buffer,
	      int	channel)
{
	if(channel > 0)
		sprintf(buffer, "Channel:%d", channel);
	else
		sprintf(buffer, "Channel:Unknow");	
}
