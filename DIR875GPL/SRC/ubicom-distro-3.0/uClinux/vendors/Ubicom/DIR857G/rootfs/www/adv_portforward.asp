...
<script language="JavaScript">
    ...
    function onPageLoad()
    {
         for (var i = 0; i < rule_max_num; i++) {
            var temp_pf;
            if (i > 9)
                temp_pf = get_by_id("port_forward_both_" + i);
            else
                temp_pf = get_by_id("port_forward_both_0" + i);

            if (temp_pf.value == "") {
                temp_pf.value = "0//0.0.0.0/0/0/Always/Allow_All";
            }
        }
	...
	if((!check_port(port1)) && (port1!=0)){
		if(potocol == 6){
			alert(MSG008.replace("%s","TCP"));
			return false;
		}else {
			alert(MSG008.replace("%s","UDP"));
			return false;
				}
			}
	...
	function check_portforward_port(proto_type, check_index, check_ip, chk_port, obj_word){
		...
		for (var i = 0; i < checkTimes; i++){  // get chk_port_arry data
			...
			if (port.length > 1){
				...
				//GW_NAT_PORT_FORWARD_PORT_RANGE_INVALID
    ...
    function send_request()
    {
        ...
        for (var i = 0; i < rule_max_num; i++) {
            ...
            if(get_by_id("enable" + i).checked == true){
	            if (trim_string(temp_port_name) != ""){
                for (var j = i+1; j < rule_max_num; j++) {
	                    if (temp_port_name == get_by_id("name" + j).value && (get_by_id("enable" + j).checked == true)) {
						alert(sp_name+" " + temp_port_name + " "+sp_alreadyused);
                        return false;
                    }
                }
            }
	            else{
	            	alert(av_alert_empty_name);
	            	return false;
	            }
	        }
        }
        ...
        for (var i = 0; i < rule_max_num; i++) {
            ...
            if (get_by_id("name" + i).value != "" || get_by_id("enable" + i).checked == true) {
                check_name = get_by_id("name" + i).value;
                if (Find_word(check_name,'"') || Find_word(check_name,"/")) {
					alert(TEXT007a+" " + i +" "+TEXT007b);//TEXT007
                    return false;
                }
                    ...
                    if (get_by_id("enable" + i).checked == true) {
                        if (remote_port_enable == "1") {
                            if (!check_multi_port(remote_port, get_by_id("tcp_ports" + i).value)) {
								alert(_tcpports+" [" + get_by_id("tcp_ports" + i).value+ "] "+ag_conflict4a);	//ag_conflict4
                                return false;
                            }
                        }
                    }
                ...
                while (get_by_id("tcp_ports" + i).value.indexOf(" ", 0) != -1) {
                get_by_id("tcp_ports" + i).value = get_by_id("tcp_ports" + i).value.replace(/ /,"");
                }

                //if port value have blank, replace to ""
                while (get_by_id("udp_ports" + i).value.indexOf(" ", 0) != -1) {
                      ...
                      <tbody>
                        ...
                        <script>
                            ...
                            for(var i=0 ; i<rule_max_num ; i++){
								...
								document.write('<td align=center valign=bottom>'+_TCP+'<br>');
                                ...
                                document.write("</select></td>");
								document.write('<td align=center valign=bottom>'+_UDP+'<br>');
