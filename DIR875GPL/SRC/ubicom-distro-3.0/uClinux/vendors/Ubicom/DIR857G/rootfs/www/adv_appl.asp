...
<script language="JavaScript">
    ...
    function onPageLoad()
    {
        for (var i = 0; i < rule_max_num; i++) {
            ...
            if (i > 9)
                temp_appl = get_by_id("application_" + i);
            else
                temp_appl = get_by_id("application_0" + i);

            if (temp_appl.value == "") {
                temp_appl.value = "0//TCP/0/TCP/0/Always";
            }
        }
    ...
    function send_request()
    {
        ...
        for (var i = 0; i < rule_max_num; i++) {
            ...
            if (trim_string(name.value) != "") {
				if(Find_word(name.value,'"') || Find_word(name.value,"/")){
					alert(TEXT003.replace("+ i +","'" + name.value + "'"));
					get_by_id("name"+i).value = "" + name.value + "";
					return false;
			    }
                    ...
                    <table borderColor=#ffffff cellSpacing=1 cellPadding=2 width=525 bgColor=#dfdfdf border=1>
                      <tbody>
                        ...
                        <script>
							...
							document.write('<option value=\"TCP\">'+_TCP+'</option>');
							document.write('<option value=\"UDP\">'+_UDP+'</option>');
