...
<script>
    ...
    function opendns_enable_selector(value)
    {
		...
		get_by_id("wan_pppoe_russia_primary_dns").value ="204.194.232.200";
		get_by_id("wan_pppoe_russia_secondary_dns").value="204.194.234.200";
		get_by_id("wan_pppoe_russia_primary_dns").disabled = true;
		get_by_id("wan_pppoe_russia_secondary_dns").disabled = true;		
	}else{
            get_by_id("wan_specify_dns").value ="0";
		get_by_id("wan_pppoe_russia_primary_dns").disabled = false;
		get_by_id("wan_pppoe_russia_secondary_dns").disabled = false;
		get_by_id("wan_pppoe_russia_primary_dns").value = "0.0.0.0";
		get_by_id("wan_pppoe_russia_secondary_dns").value =	"0.0.0.0";
        }
    }
function onPageLoad()
{
...
function send_request()
{
	...
	var fixIP = get_by_name("wan_pppoe_dynamic_00");
	var fixIP_russia = get_by_name("wan_pppoe_russia_dynamic");
	var ip = get_by_id("wan_pppoe_ipaddr_00").value;
	var clonemac = get_by_id("wan_mac").value;
	var ip_russia = get_by_id("wan_pppoe_russia_ipaddr").value;
	var netmask_russia = get_by_id("wan_pppoe_russia_netmask").value;
	var gateway_russia = get_by_id("wan_pppoe_russia_gateway").value;
	...
	var temp_ip_russia = new addr_obj(ip_russia.split("."), ip_addr_msg, false, false);
	var temp_netmask_russia = new addr_obj(netmask_russia.split("."), ip_addr_msg, false, false);
	var temp_gateway_russia = new addr_obj(gateway_russia.split("."), ip_addr_msg, false, false);
	...
	if(get_by_name("wan_pppoe_connect_mode_00")[1].checked)
	{
	...
	if (fixIP_russia[1].checked)
	{
		if (!check_address(temp_ip_russia)){
			return false;
		}
		if (!check_russia_mask(temp_netmask_russia)){
			return false;
		}
		if (!check_address(temp_gateway_russia)){
			return false;
		}
...
<p><script>show_words(bwn_intro_ICS)</script></p>
	...
	<option value="l2tp">Russia L2TP (Dual Access)</option>
              ...
              <div class=box id=show_poe >
                <h2>RUSSIA PPPOE (Dual Access) <script>show_words(bwn_ict)</script> </h2>
     ...
     <script>show_words(_optional)</script></td>
                ...
                <td align=right class="duple"><script>show_words(bwn_MIT)</script> :</td>
                      <td>&nbsp;<input type=text id="wan_pppoe_max_idle_time_00" name="wan_pppoe_max_idle_time_00" size="10" maxlength="5" value="<% CmoGetCfg("wan_pppoe_max_idle_time_00","none"); %>">
...
<div class=box id=show_poe >
                ...
                <table cellSpacing=1 cellPadding=1 width=525 border=0>
                  <tbody>
                    <tr>
                      <td  width=185 class="duple"><script>show_words(bwn_AM)</script></td>
                      <td width="331">&nbsp;
                    ...
                    <tr>
                      <td width=150 valign=top class="duple"><script>show_words(_ipaddr)</script> :</td>
                      <td valign=top>&nbsp;
