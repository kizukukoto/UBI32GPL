...
#define MINIUPNPD_SERVER_STRING	OS_VERSION " UPnP/1.0 MiniUPnPd"
...
struct upnphttp {
	int socket;
	struct in6_addr clientaddr;	/* client address */ /* IPv6modification, IPv6 only */
	int state;
	char HttpVer[16];
	/* request */
	char * req_buf;
...
void
BuildResp_upnphttp(struct upnphttp *, const char *, int, int);

/* BuildResp2_upnphttp()
 * same but with given response code/message */
void
BuildResp2_upnphttp(struct upnphttp * h, int respcode,
                    const char * respmsg, const char * body,
                    int bodylen, int phase);
