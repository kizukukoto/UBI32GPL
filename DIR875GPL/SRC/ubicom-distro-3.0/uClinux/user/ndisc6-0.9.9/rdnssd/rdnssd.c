 ...
 * $Id: rdnssd.c 614 2008-03-23 09:54:25Z remi $
 */
...
static int rdnss_update (const struct in6_addr *addr, unsigned int ifindex, time_t expiry)
{
	...
	for (unsigned i = 0; i < servers.count; i++)
	{
		...
		inet_ntop (AF_INET6, &servers.list[i].addr, buf,
		           sizeof (buf));
		syslog (LOG_DEBUG, "%u: %48s expires at %u", i, buf,
...
static int worker (int pipe, const char *resolvpath, const char *username, const char *hookpath, char *interface)
{
	...
	/*Joe Huang : Set Socket option to bind with the interface, avoid icmp.c check RA what send by itself.*/
	...
	for (bool ready = false; termsig == 0;)
	{
		...
		if (ready)
		{
			...
			if (have_change) {
				...
				/*Joe Huang: Copy merge_hook() here to make sure hookpath can be executed after write resolve.conf*/
		...
		if (my_ppoll (&pfd, 1, servers.count ? &ts : NULL, &emptyset) < 0)
		{
			if (errno == EINTR)
				continue;
		...
		if (pfd.revents)
		{
			...
			have_change=src->process (sock, interface);
...
static int manager (int pipe, const char *hookpath)
{
	...
	while (termsig == 0)
	{
		...
		if (my_ppoll(&pfd, 1, NULL, &emptyset) <= 0)
			continue;
...
rdnssd (const char *username, const char *resolvpath, const char *hookpath, char *interface)
{
	...
	/*Joe Huang : Add a new opts - interface for socket option*/

	if(strlen(interface) != 0)
		printf("username=%s, resolvpath=%s, hookpath=%s, interface=%s\n",username,resolvpath,hookpath,interface);
	else
		printf("username=%s, resolvpath=%s, hookpath=%s\n",username,resolvpath,hookpath);
	rval = pipe(pfd);
	if (rval == -1) {
...
*/
	...
	switch (worker_pid)
	{
		...
		default:
			close(pfd[1]);
			prepare_fd(pfd[0]);
		/*Joe Huang:Using vfork would NOT do manager(), move to worker() for child*/
...
{
	const char *username = "root", *hookpath = NULL,
