...
struct link_util *get_link_kind(const char *id)
{
	...
	for (l = linkutil_list; l; l = l->next)
		if (strcmp(l->id, id) == 0)
			return l;
/* id is passed in as an argument of cmd "ip link type xxx", however Usage does not have
 * an option of type. So it seems it has no use except causing confusion.
 */
#ifdef IPROUTE2_LINK_USELESS_TYPE 
