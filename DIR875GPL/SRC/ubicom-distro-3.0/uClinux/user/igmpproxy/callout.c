...
void age_callout_queue(int elapsed_time) {
    ...
    for (ptr = queue; ptr; ptr = queue, i++) {
        if (ptr->time > elapsed_time) {
            ...
            queue = queue->next;
            IF_DEBUG log_msg(LOG_DEBUG, 0, "About to call timeout %d (#%d)", ptr->id, i);
    ...
    if (!queue) {
        ...
        while (ptr) {
            if (delay < ptr->time) {
                ...
                ptr->time -= node->time;
                IF_DEBUG {
                    log_msg(LOG_DEBUG, 0, "Created timeout %d (#%d) - delay %d secs", 
                        node->id, i, node->time);
...
int timer_clearTimer(int  timer_id) {
    ...
    while (ptr) {
        if (ptr->id == timer_id) {
            ...
            if (ptr->data)
                free(ptr->data);
            IF_DEBUG log_msg(LOG_DEBUG, 0, "deleted timer %d (#%d)", ptr->id, i);
            free(ptr);
            IF_DEBUG debugQueue();
            return 1;
        }
    ...
    IF_DEBUG {
        log_msg(LOG_DEBUG, 0, "failed to delete timer %d (#%d)", timer_id, i);
        debugQueue();
    }
    return 0;
}
...
static void debugQueue() {
    ...
    IF_DEBUG {
        for (ptr = queue; ptr; ptr = ptr->next) {
            log_msg(LOG_DEBUG, 0, "(Id:%d, Time:%d) ", ptr->id, ptr->time);
        }
    }
}
