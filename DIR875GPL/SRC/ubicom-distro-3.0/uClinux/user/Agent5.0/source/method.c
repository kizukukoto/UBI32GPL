...
static struct fault_struct cpe_method_fault[] = {
    ...
    {9014, "Download failure: unable to join multicast grouip"},
...
static int common_acs_body(struct session *ss)
{
    ...
    if(r && r->data_len > 0) {
        ...
        push_buffer(&(ss->outbuf), "%x\r\n%s\r\n", r->data_len, data);
// 2011.04.14
        push_buffer(&(ss->outbuf2_data), "%s", data);
    }
