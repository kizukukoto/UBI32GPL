...
)
{
	...
	if(isMerlin(gLibInfo.pLibDevArray[devNum]->swDevID)) {
	    if(((regOffset & 0xffff) >= 0x7800) && ((regOffset & 0xffff) <= 0x78fc)) {
    	        mSleep(1); 
	    }
	}
#endif
...
)
{
	...
	if(hwMemWriteBlock(devIndex, bytesWrite, length, &(physAddr)) == -1) {
		uiPrintf("Error: TESTT   OSmemWrite failed call to hwMemWriteBlock()\n");
		return;
    }
}
...
#else
    devMap.DEV_MEMORY_ADDRESS = pdevInfo->pdkInfo->dma_mem_addr;
