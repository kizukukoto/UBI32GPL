...
#define MAX_NUM_PDGAINS_PER_CHAN_OP_LOOP 2
...
typedef struct _rawDataPerPDGAIN {
	...
	A_UINT16	numVpd;
	A_UINT16	*Vpd;     /* pdadc */
    A_UINT16	*pcdac;   /* pcdac for pwr meas (open loop pwr cntl only) */
	A_INT16		*pwr_t4;  /* power measurement x4 */
    A_INT16     *pwr_t4_pal_on; /* power measurement x4, w/PAL ON */
...
A_BOOL setup_raw_dataset_gen5(A_UINT32 devNum, RAW_DATA_STRUCT_GEN5 *pRawDataset_gen3, A_UINT16 myNumRawChannels, A_UINT16 *pMyRawChanList, A_UINT32 swDeviceID);
A_BOOL AllocMemPerChan(A_UINT32 devNum, RAW_DATA_STRUCT_GEN5  *pRawDataset_gen3, A_UINT16 numChannels);
