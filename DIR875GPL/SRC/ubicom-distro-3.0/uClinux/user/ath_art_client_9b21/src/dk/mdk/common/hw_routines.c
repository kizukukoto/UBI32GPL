...
/**
 * - Function Name: envInit
 * - Description  : performs environment initialization when running MDK 
 *                  client. For the windows NT hardware environment, need to
 *                  open the driver and perform other initialization required
 *                  by it
 * - Arguments
 *     -      
...
#ifdef OCTEON 
 		 pdevInfo->pdkInfo->dma_mem_addr = cliInfo.dma_mem_addr;	
#else
 		 pdevInfo->pdkInfo->dma_mem_addr = cliInfo.memPhyAddr;	
#endif
	...
	)
{
	if((address >= pdevInfo->pdkInfo->memPhyAddr) &&
            (address < (A_UINT_PTR)((A_UCHAR *)(pdevInfo->pdkInfo->memPhyAddr) + pdevInfo->pdkInfo->memSize)))
		return A_OK;
	else 
		return A_ENOENT;
}
...
(
	A_UINT16 devIndex,
    A_UINT_PTR address                    /* the address to read from */
)
{
#ifdef SIM
	A_UINT8 value;
...

{
	...
	if (A_OK == checkRegSpace(pdevInfo, address))
	{
		...
		q_uiPrintf("Read Reg addr %x Val %x\n", pMem, *(volatile int *)pMem);
		temp = (*(volatile int *)pMem);
...
(
	A_UINT16 devIndex,
    A_UINT_PTR address,                    /* the address to write */
    ...
    for (i = 0; i < WLAN_MAX_DEV; i++) {
        if(pdevInfo = globDrvInfo.pDevInfoArray[i])	{
			...
			if((*pPhysAddr >= startPhysAddr) &&
				...
				pMem = (A_UINT8 *) (pdevInfo->pdkInfo->memVirAddr + (*pPhysAddr - pdevInfo->pdkInfo->dma_mem_addr));
    ...
    for (i = 0; i < WLAN_MAX_DEV; i++) {
        if(pdevInfo = globDrvInfo.pDevInfoArray[i]) {
			...
			if((physAddr >= startPhysAddr) &&
				...
				/* get the virtual pointer to start and read */
				pMem = (A_UINT8 *) (pdevInfo->pdkInfo->memVirAddr + (physAddr - pdevInfo->pdkInfo->dma_mem_addr));
