...
*/
void dump_struct_server(struct server *s)
{
	char a1[64] = "", a2[64] = "", a3[64] = "";

	if (s->addr.in.sin_family != AF_INET) {
		cprintf("not socke family\n");
		return;
	}
	inet_ntop(AF_INET, &s->addr.in.sin_addr, a1, 64);
	inet_ntop(AF_INET, &s->addr.in.sin_addr, a2, 64);
	inet_ntop(AF_INET, &s->source_addr.in.sin_addr, a3, 64);
	cprintf("%p, addr addr, %s, source_addr: %s, domain: %s, 0x%08X\n",
		s, a1, a2, s->domain, s->addr.in.sin_addr.s_addr);
}

extern int extract_name(HEADER *header, size_t plen, unsigned char **pp, 
			char *name, int isExtract);
static struct server *replace_server_as(HEADER *header, int qlen, struct daemon *daemon, struct server *ref)
{
  unsigned char buf[1024], *p;
  char **m;
  struct server *s, *server2;
  int ansp, nameoffset, q, qdcount = ntohs(header->qdcount);
  
  if (daemon->res_hook == NULL)
	return ref;
  if ((server2 = daemon->res_hook->servers) == NULL)
	return ref;
  m = daemon->res_hook->namematch;
 ...
 //       if (!(ansp = skip_questions(header, qlen)))
   //             return 0; /* bad packet */
  	...
  	for (q=0; q<qdcount; q++) {
		//cprintf("extract name\n");
		if (!extract_name(header, qlen, &p, buf, 1))
			return 0; /* bad packet */
		//cprintf("extract name ok:%s\n", buf);
		for (m = daemon->res_hook->namematch; *m != NULL; m++) {
			//cprintf("strstr(%s, %s)?\n", buf, *m);
...
/* parser format resolve.conf2.
 * eg: cat /tmp/res.cfg
 * 	namematch .com
 * 	namematch .idv.tw
 *	nameserver 168.95.1.1
 *	nameserver 168.192.1.1
 * */
//struct server *fmt_struct_serve(const char *file, struct server *ref)
int load_res_hook(struct daemon *daemon, struct server *ref)
{
	FILE *fd;
	char buf[256], *p;
	struct server *s = NULL;
	char **m = daemon->res_hook->namematch;
	
	if (daemon->res_hook == NULL)
		return 0;
	for (;*m != NULL; m++)
		free(*m);	/* clear old */
	if ((fd = fopen(daemon->res_hook->path, "r")) == NULL)
		return -1;
	while (fgets(buf, sizeof(buf), fd) != NULL) {
		buf[strlen(buf) - 1] = '\0';
		if (strncmp("namematch", buf, 9) == 0) {
			p = buf + 10;
			cprintf("match %s\n", p);
			*m = strdup(p);
			m++;
		} else if(strncmp("nameserver", buf, 10) == 0) {
			p = buf + 11;
			cprintf("server [%s]\n", p);
			if (s == NULL) {
				s = malloc(sizeof(struct server));
				memcpy(s, ref, sizeof(struct server));
				inet_pton(AF_INET, p, &s->addr.in.sin_addr);
				cprintf("head [%p]: %s\n",s,  inet_ntoa(s->addr.in.sin_addr));
				s->next = s;
			} else {
				s->next = malloc(sizeof(struct server));
				memcpy(s->next, ref, sizeof(struct server));
				inet_pton(AF_INET, p, &s->next->addr.in.sin_addr);
				cprintf("next [%p]: %s\n", s->next, inet_ntoa(s->next->addr.in.sin_addr));
				s->next->next = s;
				s = s->next;
			}
		}
	}
	daemon->res_hook->servers = s?s->next:0;
	cprintf("[return %p]\n", s?s->next:0);
	return 0;
}
			  ...
			  HEADER *header, size_t plen, time_t now, struct frec *forward)
{
  ...
  unsigned short flags = 0, start_replaced = 0;
  unsigned short gotname = extract_request(header, plen, daemon->namebuff, NULL);
  struct server *start = NULL, *orig_start;
  ...
  orig_start = start;
  if ((start = replace_server_as(header, plen, daemon, start)) != orig_start) {
	cprintf("start replaced\n");
	start_replaced = 1;  
  }
  ...
  if (!flags && forward)
	...
	{ 
	  ...
	  if (type == (start->flags & SERV_TYPE) &&
	    ...
	    {
     		//cprintf("%s: %d, sendto fd: %d, addr:%s\n", __FUNCTION__, __LINE__,
		//	start->sfd->fd, inet_ntoa(start->addr.in.sin_addr));
	      ...
	      else
		{
		  ...
		  forwarded = 1;
		  /*
		   * FIXME: dns should be as round-robin serach.
		   * If nameserver 1 from res.cfg is out-of working ...
		   * */
		  //forward->sentto = start;
		  //forward->sentto = orig_start;
		  forward->sentto = start_replaced? orig_start : start;
