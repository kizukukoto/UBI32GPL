...
struct daemon *read_opts(int argc, char **argv, char *compile_opts)
{
  ...
  daemon->log_fac = -1;
  daemon->res_hook = NULL;
  
  if (access("/tmp/res.cfg", F_OK) == 0) {
	daemon->res_hook = safe_malloc(sizeof(struct resolv_hook));
	memset(daemon->res_hook, 0, sizeof(struct resolv_hook));
	daemon->res_hook->path = strdup("/tmp/res.cfg");
  }
