...
int brctl_main(int argc UNUSED_PARAM, char **argv)
{
	...
	USE_FEATURE_BRCTL_FANCY(
		...
		"setpathcost\0" "setportprio\0" "setbridgeprio\0" "setguestzone\0"
#if 1
		"setwebfileaccessport\0"
		"setwebfileaccesssslport\0"
#endif		
	)
	...
	enum { ARG_addbr = 0, ARG_delbr, ARG_addif, ARG_delif
		USE_FEATURE_BRCTL_FANCY(,
		   ...
		   ARG_setpathcost, ARG_setportprio, ARG_setbridgeprio, ARG_setguestzone,
#if 1
		   ARG_setwebfileaccessport,
		   ARG_setwebfileaccesssslport,
#endif		   
		)
	...
	while (*argv) {
		...
		if (key == ARG_show) { /* show */
			...
			arm_ioctl(args, BRCTL_GET_BRIDGES,
						(unsigned long) bridx, MAX_PORTS,0);
			...
			for (i = 0; i < num; i++) {
				...
				arm_ioctl(args, BRCTL_GET_PORT_LIST,
							(unsigned long) ifidx, MAX_PORTS,0);
				xioctl(fd, SIOCDEVPRIVATE, &ifr);
				tabs = 0;
		...
		if ((unsigned)(key - ARG_setageing) < 4) { /* time related ops */
			...
			arm_ioctl(args, ops[key - ARG_setageing], str_to_jiffies(*argv), 0, 0);
			goto fire;
		}
		if (key == ARG_setpathcost
		 ...
		 || key == ARG_setbridgeprio
			...
			arm_ioctl(args, ops[key - ARG_setpathcost], arg1, arg2, 0);
		}
		if (key == ARG_setguestzone) { /* setguestzone */
			int err;
			int ifindex;
			...
			printf("br_cmd_setguestzone: %s_%s\n",brif_name, *argv);
	
			ath_name=strtok(brif_name, ":");
			if(ath_name==NULL)
			{
			...
			arm_ioctl(args, BRCTL_SET_GUEST_ZONE, ifindex, br_ip, gstzone);
			xioctl(fd, SIOCDEVPRIVATE, &ifr);
		}
#if 1		
		if (key == ARG_setwebfileaccessport) { 
			unsigned long port;
			port = atol(*argv);
			arm_ioctl(args, BRCTL_SET_WEB_FILE_ACCESS_PORT, port, 0, 0);
			xioctl(fd, SIOCDEVPRIVATE, &ifr); // here maybe more code
			
		
		}		
		
		if (key == ARG_setwebfileaccesssslport) { 
			unsigned long port;
			port = atol(*argv);
			arm_ioctl(args, BRCTL_SET_WEB_FILE_ACCESS_SSL_PORT, port, 0, 0);
			xioctl(fd, SIOCDEVPRIVATE, &ifr); // here maybe more code
