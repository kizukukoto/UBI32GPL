 ...
 */
int get_dhcp_packet_type(struct dhcpMessage *p);
int get_dhcp_packet_type(struct dhcpMessage *p)
{
	uint8_t *op;
...
static int init_sockets(char **client_ifaces, int num_clients,
			char *server_iface, int forced_server_sport, int *fds)
{
	int i, n;

	/* talk to real server on bootps */
	//fds[0] = udhcp_listen_socket(/*INADDR_ANY,*/ SERVER_PORT, server_iface);
	fds[0] = udhcp_listen_socket(/*INADDR_ANY,*/ forced_server_sport, server_iface);
	n = fds[0];
...
static RelayHook putenv_ip_mac = udhcp_relay_hook_putenv_ip_mac;
...
static void pass_to_client(struct dhcpMessage *p, int packet_len, int *fds)
{
	...
	/* Matt 10/27 add this hook at pass_to_server function
	 * Fred 10/28 move this hook from pass_to_server to pass_to_client function
	 * */
	if (putenv_ip_mac)
		putenv_ip_mac(p, 0);
...
//static RelayHook hook = NULL;
static RelayHook hook_ssid = udhcp_relay_hook_ssid;
...
int dhcprelay_main(int argc, char **argv)
{
	...
	uint32_t our_ip;
	int forced_server_sport = SERVER_PORT;
	char **servers;
	int num_servers = 0;
	...
	if (argc >= 4) {
		int x;
		if (argc == 5)
			forced_server_sport = atoi(argv[4]);
		servers = get_client_devices(argv[3], &num_servers);
		x = num_servers;
		while (x--)
			printf("servers[%s]\n", servers[x]);
		if (!inet_aton(servers[0], &server_addr.sin_addr))
	...
	max_socket = init_sockets(client_ifaces, num_sockets, argv[2], forced_server_sport, fds);
...
//maybe set dhcp_msg.flags |= BROADCAST_FLAG too?
				if (hook_ssid)
					packlen = hook_ssid(&dhcp_msg, packlen);
				if (num_sockets > 0) {
					/* if @argc = 3, use boardcast and @servers was not init(NULL) */
					if (num_servers >= i) {
						inet_aton(servers[i - 1], &server_addr.sin_addr);
					} else {
						/* last list entry at @argv[3] */
						inet_aton(servers[num_servers - 1], &server_addr.sin_addr);
					}
				}
