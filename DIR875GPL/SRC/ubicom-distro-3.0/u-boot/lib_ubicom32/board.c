...
void delay_time(int seconds)
{
	 ...
	 * Date: 2010.11.12
	 */
	etime = get_ticks() + (uint64_t)(seconds) * get_tbclk();
...
void board_init (void)
{
	...
	UBICOM32_GPIO_SET_PIN_OUTPUT(STATUS_LED);//status led output enable
	gpio_set_value(STATUS_LED, 0);
	UBICOM32_GPIO_SET_PIN_OUTPUT(POWER_LED);//status led output enable
	gpio_set_value(POWER_LED, 1);

#if defined(IP8000)
	// Switch reset G4P14 (142) , for DIR-827 switch chip v1.1
	UBICOM32_GPIO_SET_PIN_OUTPUT(142);
...
void athrs16_reg_init(void);
	 ...
	 * Date: 2010.11.12
	 */
//	delay_time(5); Maybe not needed anymore
    ...
    UBICOM32_GPIO_SET_PIN_INPUT(PUSH_BUTTON);
    gpio_set_value(PUSH_BUTTON, 1);
    delay_time(1);

	if (gpio_get_value(PUSH_BUTTON) == 0)//0=push reset button
	{
		//UBICOM32_GPIO_SET_PIN_OUTPUT(STATUS_LED);//status led output enable
		httpd(0, NULL);
	}
