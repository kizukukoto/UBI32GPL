					   ...
					   char *buf, int inst)
{
	...
	link = phy_reg_read(0, AR8327_PORT_TO_PHY(inst), 0x11);
        ...
        if((link & (1 << 10)) != (1 << 10))
        {
                printk(KERN_INFO "ar8327 switch chip wan disconnect-link=0x%x\n", link);
        }
#endif
	return sprintf(buf, "%d\n", ((link & (1 << 10)) == (1 << 10)) ? 1 : 0);
}
...
u16_t phy_addr_t=0;
/*
 * ar8327_handle_port_phyregval_read
 */
static int ar8327_handle_port_phyregval_read(struct switch_device *dev,
					   char *buf, int inst)
{
	u16_t link;
	...
	//return sprintf(buf, "0x11=0x%x 0x1=0x%x 0xa=0x%x 0x4=0x%x 0x5=0x%x \n",link ,phy_reg_read(0, AR8327_PORT_TO_PHY(inst), MII_BMSR), phy_reg_read(0, AR8327_PORT_TO_PHY(inst), MII_STAT1000), phy_reg_read(0, AR8327_PORT_TO_PHY(inst), MII_ADVERTISE), phy_reg_read(0, AR8327_PORT_TO_PHY(inst), MII_LPA) );
	return sprintf(buf, "%x\n",phy_reg_read(0, AR8327_PORT_TO_PHY(inst), phy_addr_t));	
}

/*
 * ar8327_handle_port_status_write
 */
static int ar8327_handle_port_phyregval_write(struct switch_device *dev,
					    char *buf, int inst)
{
	/*
	...
	if (!(dev->port_mask[0] & (1 << inst) & ~(1 << 0))) {
        ...
        phy_reg_write(0, AR8327_PORT_TO_PHY(inst), phy_addr_t, simple_strtoul(buf, NULL, 0));

	return 0;
}

/*
 * ar8327_handle_port_phyreg_read
 */
static int ar8327_handle_port_phyreg_read(struct switch_device *dev,
					   char *buf, int inst)
{
	/*
	 * validate port
...
/*
 * ar8327_handle_port_phyreg_write
 */
static int ar8327_handle_port_phyreg_write(struct switch_device *dev,
					    char *buf, int inst)
{
	/*
	 * validate port (not for CPU port)
	...
	if (!(dev->port_mask[0] & (1 << inst) & ~(1 << 0))) {
        ...
        phy_addr_t = simple_strtoul(buf, NULL, 0);
					    ...
					    char *buf, int inst)
{
	...
	if (strncmp(buf, "1000FD", 6) == 0) {
		...
		phy_reg_write(0, AR8327_PORT_TO_PHY(inst), 0x9, 0x0);
		phy_reg_write(0, AR8327_PORT_TO_PHY(inst), 0x0, 0x3300);	
		phy_reg_write(0, AR8327_PORT_TO_PHY(inst), 0x0, 0x2100);	
printk("set 100FD\n");		
		return 0;
...
static const struct switch_handler ar8327_switch_handlers_port[] = {
	...
	{
		.name	= "phy_reg_val",
		.read	= ar8327_handle_port_phyregval_read,
		.write	= ar8327_handle_port_phyregval_write,
	},
	{
		.name	= "phy_reg",
		.read	= ar8327_handle_port_phyreg_read,
		.write	= ar8327_handle_port_phyreg_write,
	},
#endif
	{
		.name	= "media",
