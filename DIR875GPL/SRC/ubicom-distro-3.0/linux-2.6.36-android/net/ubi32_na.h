...
struct ubicom_na_ipv4_create {
	int src_interface_num;
	int dest_interface_num;
	int protocol;
	uint32_t flags;
	uint32_t src_ip;
	int32_t src_port;
...
struct ubicom_na_ipv4_sync {
	...
	unsigned long int delta_jiffies;		/* Time in Linux jiffies to be added to the current timeout to keep the connection alive */
};
